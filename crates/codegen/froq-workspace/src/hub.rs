...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        session_handler_resolver: Option<froq_computer_hub_sdk::SessionHandlerResolver>,
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(froq_version::VERSION)
...
struct CallCompletedGuard {
    ...
    outcome: froq_file_utils::events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: froq_file_utils::events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: froq_file_utils::events::ToolOutcome) {
...
impl ToolServerHandler for SessionRoutedToolHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let hub_session = ctx
            .extensions
            .get::<froq_tool_runtime::SessionContext>()
            .map(|s| s.0.clone());
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        ...
                        _guard.set_outcome(froq_file_utils::events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(froq_file_utils::events::ToolOutcome::Error);
...
fn parse_server_id(id: &str) -> Result<froq_tool_protocol::ServerId, ClientError> {
    froq_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    use futures::StreamExt;
    use froq_tool_runtime::{SessionContext, ToolCallId};
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![froq_tool_runtime::ContentBlock::Text {
    ...
    use froq_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    impl froq_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use froq_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_counts<T>(mut stream: froq_tool_runtime::ToolStream<T>) -> (usize, usize, bool) {
        let mut progress = 0;
        let mut terminal = 0;
    ...
    async fn wait_until(
        ...
        pred: impl Fn(&froq_tool_protocol::ToolServerStatusPayload) -> bool,
        timeout: Duration,
    ) -> froq_tool_protocol::ToolServerStatusPayload {
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use froq_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use froq_tools::computer::types::{TaskKind, TaskSnapshot};
