...
use url::Url;
use uuid::Uuid;
use froq_computer_hub_sdk::pool::HubConnectionPool;
use froq_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &froq_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
) -> anyhow::Result<froq_computer_hub_sdk::ToolHarness> {
