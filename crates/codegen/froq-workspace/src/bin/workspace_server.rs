...
use url::Url;
use froq_workspace::config::WorkspaceServerMetadata;
use froq_workspace::daemonize;
use froq_workspace::diag_server;
use froq_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<froq_tool_protocol::ServerId>()
        .err()
        .map(|e| format!("{INVALID_SERVER_ID_MARKER} {id:?}: {e}"))
}
#[derive(Parser)]
#[command(name = "froq-workspace-server")]
...
/// `/usr/local/bin/froq-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = froq_computer_hub_sdk::DonatingLogLayer::new_inert();
    ...
    let direct_otlp = match std::env::var("GROK_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match froq_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use froq_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("GROK_SANDBOX_PROFILE").ok() {
            ...
            None if froq_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            ...
            sandbox.install();
            let active = froq_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = froq_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let auth_provider = froq_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(
        hub_url = %url,
        cwd = %cwd.display(),
    ...
    let status_config = froq_workspace::StatusConfig::from_env();
    ...
    froq_workspace::init_metrics();
    let ws_handle = froq_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = froq_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            froq_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    froq_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        let args = Args::try_parse_from(["froq-workspace-server"]).unwrap();
        assert!(!args.capabilities);
        let args = Args::try_parse_from(["froq-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        ...
        let args = Args::try_parse_from(["froq-workspace-server", "--project-lsp-trusted", "true"])
            .unwrap();
        assert!(args.project_lsp_trusted);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["froq-workspace-server", "--capabilities"])
    ...
    fn ready_file_is_accepted_as_a_deprecated_no_op() {
        let args =
            Args::try_parse_from(["froq-workspace-server", "--ready-file", "/tmp/x.ready"]).unwrap();
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["froq-workspace-server", "--flag-from-the-future"])
            .err()
