...
//!
//! Moved from `froq-shell/src/session/worktree.rs` into the workspace
...
use froq_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
// Canonical in froq-workspace-types; re-exported for existing paths.
...
const WORKTREE_LOG: &str = "froq_worktree";

/// Map a [`WorktreeType`] to the fast-worktree crate's `CreationMode`.
pub(crate) fn to_creation_mode(t: WorktreeType) -> froq_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => froq_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => froq_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => froq_fast_worktree::CreationMode::GitCheckout,
    }
}
...
/// `worktrees.db` (`froq_fast_worktree::resolve_grok_home`), so checkout dirs and
/// the metadata DB always live under the same `.grok` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `froq_config::grok_home()`,
/// so worktree paths also agree with trust/hooks and other grok-home paths.
fn grok_home() -> std::path::PathBuf {
    froq_fast_worktree::resolve_grok_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to froq_fast_worktree::WorkingTreeMode
    ...
    // Use froq-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(froq_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        froq_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        froq_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        froq_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        froq_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        froq_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        froq_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use froq-fast-worktree -- it handles copying from any worktree path
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(froq_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &froq_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use froq_config_types::WorktreeKindMaxAge;
...
fn resolve_worktree_auto_gc_local() -> froq_fast_worktree::ResolvedWorktreeAutoGc {
    use froq_config_types::WorktreeAutoGcSettings;
...
pub fn worktree_db_rebuild() -> Result<froq_fast_worktree::RebuildReport> {
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = froq_fast_worktree::ListFilter {
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[froq_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    fn worktree_db_fixture(
        ...
        let record = WorktreeRecord {
            ...
            status: froq_fast_worktree::WorktreeStatus::Alive,
