//! AcpFsAdapter: implements `froq-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates froq-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use froq_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps froq-shell's ACP gateway to satisfy froq-tools' AsyncFileSystem.
///
