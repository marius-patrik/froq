...
use froq_computer_hub_sdk::ToolHarness;
use froq_tools::types::output::ToolRunResult;
use froq_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use froq_workspace_types::rpc::WorkspaceRpc;
pub use froq_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use froq_workspace_types::rpc::code_nav::{
...
pub use froq_workspace_types::rpc::fs::{
...
pub use froq_workspace_types::rpc::git::{
...
pub use froq_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use froq_workspace_types::rpc::hunks::{
...
pub use froq_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use froq_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use froq_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use froq_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use froq_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &froq_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: froq_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use froq_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &froq_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: froq_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use froq_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: froq_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: froq_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &froq_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: froq_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: froq_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> froq_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => froq_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => froq_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<froq_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = froq_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                froq_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &froq_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<froq_hooks::discovery::HookRegistry> {
    let value = serde_json::to_value(wire).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
    let mut registry: froq_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<froq_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<froq_codebase_graph::QueryResult, froq_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<froq_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](froq_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: froq_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<froq_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](froq_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(froq_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = froq_tool_protocol::ToolId::new(name).map_err(|e| {
                    froq_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = froq_tool_runtime::ToolCallContext::default();
                ctx.call_id =
                    froq_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    /// request types are defined in `froq-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/froq-main");
        let window_b = std::path::PathBuf::from("/repos/froq-main-2");
        assert_eq!(
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            froq_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hunk_to_wire_serializes_identically() {
        use froq_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use froq_hunk_tracker::FileContentEntry;
        use froq_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use froq_hunk_tracker::SessionSummary;
        use froq_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = froq_hooks::config::HookSpec {
            ...
            event: froq_hooks::event::HookEventName::PreToolUse,
        ...
        let mut registry = froq_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use froq_hooks::event::HookEventName as E;
    ...
    /// can't depend on `froq-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use froq_hooks::config::HookSpec;
