...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<froq_computer_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<froq_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<froq_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<froq_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn froq_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            froq_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = froq_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// [`LocalTerminalBackend`]: froq_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<froq_computer_hub_sdk::SharedAuthProvider>,
...
impl WorkspaceSessionContextFactory {
    ...
    pub fn with_auth(auth: froq_computer_hub_sdk::SharedAuthProvider, api_base_url: String) -> Self {
        Self {
            auth: Some(auth),
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(froq_tools::computer::local::LocalFs)
            as Arc<dyn froq_tools::computer::types::AsyncFileSystem>;
        let notification_handle = froq_tools::notification::ToolNotificationHandle::noop();
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    froq_computer_hub_sdk::AuthCredential::Bearer { token, .. } => {
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            froq_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = froq_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("froq-workspace/{version}"),
    );
    ...
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-froq-Token-Auth".to_string(), "froq-cli".to_string());
        headers.insert(
...
fn build_web_fetch_config() -> froq_tools::implementations::grok_build::web_fetch::WebFetchConfig
{
    use froq_tools::implementations::grok_build::web_fetch::{WebFetchConfig, WebFetchParams};
    ...
    if froq_config::env_bool("GROK_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
...
mod tests {
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use froq_tools::types::resources::{State, WebCitationCounter};
