...
use std::sync::Arc;
use froq_computer_hub_mcp_adapter::McpBridgeHandle;
use froq_mcp::servers::McpState;
use froq_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use froq_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use froq_hunk_tracker::HunkTrackerHandle;
use froq_tool_protocol::ToolId;
use froq_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: froq_hunk_tracker::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, froq_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<froq_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<froq_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<froq_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<froq_hooks::error::HookError>,
    ...
    pub(crate) auth_provider: Option<froq_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<froq_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) local_registry: froq_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn froq_tools::implementations::lsp::LspBackend>>,
    ...
    pub(crate) upload_queue: Option<std::sync::Arc<froq_file_utils::queue::UploadQueue>>,
    ...
    /// [`EventWriter::noop()`](froq_file_utils::events::EventWriter::noop) and
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, froq_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<froq_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<froq_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](froq_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> froq_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<froq_file_utils::events::EventWriter> {
    ...
    pub fn auth_provider(&self) -> Option<&froq_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](froq_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<froq_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, froq_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> froq_file_utils::events::EventWriter {
    use froq_file_utils::events::EventWriter;
    if !enabled {
...
mod tests {
    ...
    use dashmap::DashMap;
    use froq_file_utils::events::{Event, EventWriter};
