...
///
/// ```
/// use froq_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
pub const RESTORE_CODE_LOG: &str = "froq_restore_code";
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = froq_tools::util::grok_home::grok_home().join("worktrees");
...
pub fn git_rewind_enabled() -> bool {
    froq_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
