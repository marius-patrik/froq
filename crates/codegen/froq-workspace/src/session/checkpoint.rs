...
use froq_hunk_tracker::{HunkId, HunkTrackerSnapshot, HunkTurnDelta};
...
pub(crate) fn rewind_hunks_enabled() -> bool {
    froq_config::env_bool("GROK_WORKSPACE_REWIND_HUNKS").unwrap_or(false)
}
...
pub(crate) fn rewind_durable_enabled() -> bool {
    froq_config::env_bool("GROK_WORKSPACE_REWIND_DURABLE").unwrap_or(false)
}
impl WorkspaceSession {
    ...
    pub(crate) async fn restore_hunk_checkpoints(&self, target_prompt_index: usize) {
        ...
        let mut file_states: HashMap<std::path::PathBuf, froq_hunk_tracker::FileHunkStateSnapshot> =
            HashMap::new();
...
impl WorkspaceHandle {
    ...
    ) -> Option<tokio::task::JoinHandle<froq_file_utils::queue::EnqueueOutcome>> {
        match boundary {
            TurnBoundary::Start {
