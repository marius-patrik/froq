...
use froq_mcp::servers::parse_mcp_qualified_name;
use froq_paths::AbsPathBuf;
use froq_tools::implementations::grok_build::web_fetch::{
    DomainMatcher, domain::normalize_domain,
};
use froq_tools::types::resources::resolve_model_path;
...
fn permission_mode_artifact_str(mode: froq_telemetry::enums::PermissionMode) -> &'static str {
    use froq_telemetry::enums::PermissionMode;
    match mode {
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            froq_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
...
mod tests {
    ...
    fn manager_with_recording_client_remember(
        ...
        let (gateway, receiver) = froq_acp_lib::acp_gateway::<acp::AgentSide, _>(client);
