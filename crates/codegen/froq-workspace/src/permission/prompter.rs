...
use froq_file_utils::events::{Event, EventWriter, PermissionDecision};
use froq_mcp::servers::parse_mcp_qualified_name;
use froq_tools::implementations::grok_build::web_fetch::domain_from_url;
...
/// Canonical definition lives in `froq_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `froq-mcp` can
...
/// reached `froq_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
