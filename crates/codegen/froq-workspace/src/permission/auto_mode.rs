...
pub fn mcp_access_detail(name: &str, input: &serde_json::Value) -> String {
    ...
    froq_tools::util::truncate_line(&format!("{name} {compact}"), MCP_ACCESS_DETAIL_MAX_LEN)
        .into_owned()
}
...
pub fn permission_decision_args(access: &AccessKind, access_detail: Option<&str>) -> String {
    ...
    froq_tools::util::truncate_str_with_marker(&raw, CLASSIFIER_TURN_MAX_LEN).into_owned()
}
...
fn classifier_reason(v: &serde_json::Value) -> Option<String> {
    v.get("reason")
        ...
        .filter(|r| !r.is_empty())
        .map(|r| froq_tools::util::truncate_line(&r, CLASSIFIER_REASON_MAX_LEN).into_owned())
}
