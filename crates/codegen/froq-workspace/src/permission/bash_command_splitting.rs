...
///
/// ```bash
/// froq_API_KEY='froq-some-key' cargo run --bin froq-pager
/// ```
///
...
///   prefix: ["froq_API_KEY=froq-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "froq-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "froq_API_KEY='froq-some-key' cargo run --bin froq-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["froq_API_KEY=froq-some-key".to_owned()],
        ...
        let redirection_command = "cargo build --bin froq-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package froq-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
