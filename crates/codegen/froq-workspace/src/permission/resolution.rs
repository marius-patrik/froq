...
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    froq_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    if let Some(global_path) = froq_config::user_grok_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match froq_config::load_config_file(&global_path) {
    ...
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match froq_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[froq_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
    ...
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &froq_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// [`froq_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = froq_config::claude_managed_settings_path() else {
...
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = froq_config::requirements_layers();
...
mod tests {
    ...
    /// `froq_config::grok_home()` is a process-wide `OnceLock`, so under
...
"#,
        ...
        let global_live = froq_config::user_grok_home()
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            froq_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            froq_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
