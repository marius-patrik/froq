...
// permissions), consider extracting to a shared helper (e.g., in froq-hooks
...
// =============================================================================
//
// `froq-shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in froq-shell (inaccessible from here at runtime). They also
    ...
    let Some(config_path) = froq_config::user_grok_home().map(|g| g.join("config.toml")) else {
        return false;
    };
