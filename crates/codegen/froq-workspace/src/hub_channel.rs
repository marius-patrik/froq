...
pub use froq_workspace_client::consume_stream_terminal;

/// Extract a `WorkspaceEvent` from a custom `ToolNotificationFrame`.
pub fn extract_workspace_event(
    frame: &froq_tool_protocol::ToolNotificationFrame,
) -> Option<WorkspaceEvent> {
    use froq_tool_protocol::WireToolNotification;
...
mod tests {
    use super::*;
    use froq_tool_protocol::{ToolId, ToolNotificationFrame, WireToolNotification};
    ...
    // consume_stream_terminal tests live in froq-workspace-client.
}
