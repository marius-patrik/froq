...
use froq_computer_hub_mcp_adapter::{
...
use froq_mcp::rmcp;
use froq_mcp::servers::{McpClient, parse_mcp_qualified_name};
use froq_tool_protocol::ToolId;
use froq_tool_runtime::{ToolCallContext, ToolStream, TypedToolOutput};
...
impl McpTransport for McpClientTransportAdapter {
    async fn initialize(&self) -> Result<McpServerInfo, froq_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
            .map_err(|e| froq_computer_hub_mcp_adapter::McpError::Transport(e.to_string()))?;
        let info = service.peer_info().ok_or_else(|| {
            froq_computer_hub_mcp_adapter::McpError::Transport("no peer info after init".into())
        })?;
        Ok(McpServerInfo {
    ...
    async fn list_tools(
        &self,
    ) -> Result<Vec<McpToolDefinition>, froq_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn call_tool(
        ...
        name: &str,
        arguments: Value,
    ) -> Result<McpCallResult, froq_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn close(&self) -> Result<(), froq_computer_hub_mcp_adapter::McpError> {
...
/// Extract a server name from an [`McpError`](froq_mcp::servers::McpError),
/// falling back to `"unknown"`.
pub(crate) fn server_name_from_mcp_error(e: &froq_mcp::servers::McpError) -> &str {
    e.server_name().unwrap_or("unknown")
}

/// Bridge config factory for MCP bridge connections.
pub(crate) fn make_bridge_config(
    session_id: froq_tool_protocol::SessionId,
    server_name: &str,
) -> McpBridgeConfig {
    McpBridgeConfig {
        session_id,
...
mod tests {
    use super::*;
    use froq_computer_hub_mcp_adapter::{McpBridge, McpError};
