...
use std::sync::Arc;
use froq_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: froq_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn froq_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub viewer_ctx: Option<froq_tool_runtime::WorkspaceViewerContext>,
    ...
    pub tools: Option<Vec<froq_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(froq_tools::registry::proto_convert::ToolConfigEntryError),
}
...
impl WorkspaceBindConfig {
    ...
    /// [`froq_tool_runtime::WorkspaceBindMetadata`] (same type the emitter
    /// serializes); `tool_config` is a consumer-only raw escape hatch read
    /// separately.
    pub fn from_metadata(metadata: &serde_json::Value) -> Self {
        let wire: froq_tool_runtime::WorkspaceBindMetadata =
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = froq_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match froq_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    .manifest_version, running_version = froq_version::VERSION,
...
impl WorkspaceConfig {
    ...
    pub fn new_for_proxy(
        ...
        auth_provider: froq_computer_hub_sdk::SharedAuthProvider,
...
/// `*.json` hook files. Maps 1:1 to [`froq_hooks::discovery::HookSource`]
