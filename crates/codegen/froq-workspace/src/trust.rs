...
//!
//! The store is rooted at [`froq_config::user_grok_home`] — the **Option**
...
impl TrustStore {
    ...
    ///
    /// Resolves via [`froq_config::user_grok_home`], never
    /// [`froq_config::grok_home`], so it never falls back to a cwd-relative
    ...
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(froq_config::user_grok_home())
    }
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = froq_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match froq_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_sources_from_user_grok_home() {
        ...
        assert_eq!(
            ...
            froq_config::user_grok_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    fn register_grok_worktree(
        ...
        use froq_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
