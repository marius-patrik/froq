...
use std::sync::Arc;
use froq_hunk_tracker::{HunkTrackerActor, HunkTrackerHandle, TrackingMode};
use froq_tool_protocol::ToolServerStatusPayload;
use froq_tool_protocol::turn_hook::TurnHookOutcome;
...
use froq_file_utils::events::types::CancellationCategory;
use froq_file_utils::events::{Event, SessionRelationship, TurnOutcomeLabel};
use froq_file_utils::queue::EnqueueOutcome;
use froq_tool_protocol::turn_hook::{AfterTurnAckPayload, AfterTurnAckStatus};
...
impl WorkspaceHandle {
    ...
    pub async fn trace_donation_reporter(
        ...
        froq_computer_hub_sdk::HubDonatingReporter,
        froq_computer_hub_sdk::TraceDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    /// teardown.
    ///
    /// [`LogDonationSender`]: froq_computer_hub_sdk::LogDonationSender
    pub async fn log_donation_layer(
        &self,
        service_name: &str,
    ) -> Option<(
        froq_computer_hub_sdk::LogDonationSender,
        froq_computer_hub_sdk::LogDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    pub async fn metric_donation_reporter(
        &self,
        service_name: &str,
    ) -> Option<froq_computer_hub_sdk::MetricDonationPump> {
        self.shared
            .hub_handle
            .lock()
            .await
    ...
    /// pre-spawned [`UploadQueue`](froq_file_utils::queue::UploadQueue).
    ///
    ...
    pub(crate) fn new_with_data_collection(
        ...
        upload_queue: Arc<froq_file_utils::queue::UploadQueue>,
    ...
    fn build(
        config: WorkspaceConfig,
        workspace_home: std::path::PathBuf,
        upload_queue: Option<Arc<froq_file_utils::queue::UploadQueue>>,
        ...
        let local_registry = froq_computer_hub_sdk::LocalRegistry::new();
        ...
        let (hook_registry, hook_load_errors) = {
            use froq_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn froq_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                froq_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            let servers =
                froq_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
                    sourced,
            ...
            if servers.is_empty() {
                ...
                use froq_tools::implementations::lsp::{
                ...
                let mgr = Arc::new(tokio::sync::Mutex::new(LspManager::new(
                    ...
                    true,
                    froq_tools::notification::ToolNotificationHandle::noop(),
                )));
        ...
        let session_event_writers: Arc<
            dashmap::DashMap<String, froq_file_utils::events::EventWriter>,
    ...
    /// The [`ToolServer`](froq_computer_hub_sdk::ToolServer) for this
    ...
    pub fn hub_server(&self) -> Option<froq_computer_hub_sdk::ToolServer> {
    ...
    pub async fn hub_server_blocking(&self) -> Option<froq_computer_hub_sdk::ToolServer> {
    ...
    pub fn create_session_with_config(
        ...
        session_id: impl Into<String>,
        cwd: Option<std::path::PathBuf>,
        tool_config: Option<froq_tools::registry::types::ToolServerConfig>,
        capability: CapabilityMode,
        viewer_ctx: Option<froq_tool_runtime::WorkspaceViewerContext>,
    ...
    fn create_session_with_tracker_inner(
        ...
        let system_notify_channel = system_notifications
            .then(froq_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: froq_tools::registry::types::ToolServerConfig,
    ...
    async fn resolve_and_swap_session_toolset_locked(
        ...
        let _ = self
            .shared
            .events
            .send(froq_workspace_types::WorkspaceEvent::ToolsChanged {
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<froq_tools::registry::types::ToolServerConfig>,
    ...
    pub async fn on_before_turn(
        &self,
        session_id: &str,
        payload: &froq_tool_protocol::turn_hook::BeforeTurnPayload,
    ) {
    ...
    pub async fn on_after_turn(
        &self,
        session_id: &str,
        payload: &froq_tool_protocol::turn_hook::AfterTurnPayload,
    ) {
    ...
    pub async fn compute_turn_injections(
        &self,
        session_id: &str,
        request: &froq_tool_protocol::turn_hook::TurnHookRequest,
    ) -> froq_tool_protocol::turn_hook::HookReply {
        use froq_tool_protocol::turn_hook::{HookReply, TurnHookRequest};
        match request {
    ...
    pub fn cancel_tool_call(&self, session_id: &str, call_id: &str) {
        self.shared.activity_tracker.tool_call_completed(
            call_id,
            Some(session_id),
            froq_file_utils::events::ToolOutcome::Cancelled,
        );
    ...
    pub fn hook_registry(&self) -> froq_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[froq_hooks::error::HookError] {
    ...
    pub fn get_or_create_codebase_index(
        &self,
        cwd: std::path::PathBuf,
    ) -> (Arc<froq_codebase_graph::IndexManagerHandle>, bool) {
        self.shared.codebase_indexes.lock().get_or_create(cwd)
    }
    pub fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<Arc<froq_codebase_graph::IndexManagerHandle>> {
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(froq_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(froq_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(froq_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    async fn emit_environment_artifact(
    ...
    ) -> Option<froq_file_utils::queue::EnqueueOutcome> {
        ...
        let env = {
            ...
            .in_span(
                fastrace::Span::root(
                    ...
                    trace_parent.unwrap_or_else(froq_tracing::local_or_random_span_ctx),
                )
                .with_properties(|| {
                    [
                        ("session_id", session_id.to_owned()),
        ...
        let outcome = upload_queue
            ...
            .await;
        match &outcome {
            froq_file_utils::queue::EnqueueOutcome::Failed { reason: _ } => {
                dc_log!(
                    warn,
                    session_id = %session_id,
                    error_category = "enqueue_failed",
    ...
    pub async fn start_session_mcp_servers(
        ...
        use froq_computer_hub_mcp_adapter::McpBridge;
        use froq_computer_hub_sdk::ToolServerHandler as _;
        use froq_mcp::servers::MCP_TOOL_NAME_DELIMITER;
        use froq_tool_protocol::SessionId;
        let tool_server = {
        ...
        let mcp_results: Vec<
            Result<froq_mcp::servers::McpClient, froq_mcp::servers::McpError>,
        > = tokio::task::spawn_blocking(move || {
            use std::collections::HashMap;
            use froq_mcp::oauth_config::McpOAuthConfigMap;
            use froq_mcp::servers::{McpClientTimeoutOverrides, McpMetaConfigMap};
            ...
            rt_handle.block_on(froq_mcp::servers::start_mcp_servers(
                configs,
                ...
                &oauth_config_map,
                &event_writer,
                froq_mcp::servers::OauthInteractivity::Interactive,
            ))
        })
        .await
        ...
        for result in mcp_results {
            match result {
                Ok(client) => {
                    ...
                    let transport: Arc<dyn froq_computer_hub_mcp_adapter::McpTransport> =
                        ...
                        Ok(handle) => {
                            for handler in handle.bridge.handlers() {
                                    ...
                                    .await
                                {
                                    ...
                                    froq_tool_protocol::ToolId::new(&qualified_name)
                                {
    ...
    pub async fn teardown_session_mcp(&self, session_id: &str) {
        ...
        let sid = match froq_tool_protocol::SessionId::new(session_id) {
            Ok(s) => s,
            Err(_) => return,
        };
        let mut tool_ids = session.mcp_tool_ids.lock().await;
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<froq_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<froq_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
    ...
    pub(crate) fn session_bind_resolver(
        &self,
        catalog: Arc<Vec<Arc<dyn froq_computer_hub_sdk::ToolServerHandler>>>,
        rpc_tool_id: froq_tool_protocol::ToolId,
    ) -> froq_computer_hub_sdk::SessionHandlerResolver {
        let weak_shared = Arc::downgrade(&self.shared);
        Arc::new(
            move |sid: froq_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                let bind_parent = params
                    ...
                    .unwrap_or_else(froq_tracing::local_or_random_span_ctx);
                ...
                async move {
                    let Some(shared) = weak_shared.upgrade() else {
                        WORKSPACE_BIND_FAILED_TOTAL
                            ...
                            .inc();
                        return Err(
                            froq_tool_runtime::ToolError::service_unavailable(
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || froq_tools::registry::types::ToolServerConfig {
                        tools: vec![],
                    ...
                    Ok(froq_computer_hub_sdk::ResolvedSessionHandlers {
                        handlers,
                        unserved_tool_ids,
                        resolve_error,
                    })
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let (template_handlers, rpc_tool_id) = {
            ...
            let tool_names: Vec<String> = handlers
                ...
                .map(|h| h.tool_id().as_str().to_owned())
                .collect();
            let rpc_handler: Arc<dyn froq_computer_hub_sdk::ToolServerHandler> =
        ...
        let catalog: Arc<Vec<Arc<dyn froq_computer_hub_sdk::ToolServerHandler>>> =
        ...
        let (activity_notify_handle, activity_notify_rx) =
            froq_tools::notification::types::ToolNotificationHandle::channel();
        ...
        let listener_task = tokio::spawn(async move {
            while let Some(notification) = notification_rx.recv().await {
                match notification {
                    froq_computer_hub_sdk::HubNotification::ToolsChanged {
                        added,
                        removed,
                        updated,
                        ..
            ...
            loop {
                match rx.recv().await {
                    Ok(event) => {
                        ...
                        let frame = froq_tool_protocol::ToolNotificationFrame::custom(
                            froq_tool_protocol::ToolId::new(
        ...
        let status_publisher_task = tokio::spawn(async move {
            ...
            async fn send_status(
                conn: &froq_computer_hub_sdk::HubConnection,
                ...
                let req = froq_tool_protocol::JsonRpcRequest {
                    jsonrpc: froq_tool_protocol::JsonRpcVersion,
                    id: froq_tool_protocol::JsonRpcId::from_request_id(&request_id),
                    session_id: None,
                    method: froq_tool_protocol::Method::ToolServerStatus
                        .as_wire_str()
                        .to_owned(),
                    params,
                };
...
fn build_session_routed_handlers(
    toolset: &froq_tools::registry::types::FinalizedToolset,
    ws: &WorkspaceHandle,
) -> Vec<Arc<dyn froq_computer_hub_sdk::ToolServerHandler>> {
    ...
    for def in toolset.tool_definitions() {
        ...
        let mut desc = froq_tool_types::ToolDescription::new(
        ...
        match crate::hub::SessionRoutedToolHandler::new(
            ...
            Ok(handler) => {
                handlers.push(Arc::new(handler) as Arc<dyn froq_computer_hub_sdk::ToolServerHandler>)
            }
            Err(e) => {
                tracing::warn!(
                    tool = %def.function.name,
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &froq_tools::notification::types::ToolNotification,
) {
    use froq_tools::notification::types::ToolNotification;
    match notification {
...
pub(crate) async fn run_activity_feed(
    ...
    mut rx: tokio::sync::mpsc::UnboundedReceiver<
        froq_tools::notification::types::ToolNotification,
    >,
) {
...
pub async fn connect_local_workspace(
    cwd: std::path::PathBuf,
    hub_url: url::Url,
    auth: froq_computer_hub_sdk::SharedAuthProvider,
    ...
    let tool_config = froq_agent::workspace_grok_build_toolset();
    ...
    let trace_source: Arc<dyn froq_file_utils::queue::TraceExportSource> = Arc::new(
        crate::upload::WorkspaceTraceExportSource::new(proxy_storage.clone()),
    );
    let upload_queue = Arc::new(froq_file_utils::queue::UploadQueue::spawn(
        &workspace_home,
        trace_source,
        froq_file_utils::queue::UploadRetryPolicy::default(),
    ));
    ...
    upload_queue.cleanup_orphans(froq_file_utils::queue::DEFAULT_MAX_AGE);
...
///    otherwise falls back to `~/.grok` (see [`froq_config::grok_home`]).
pub fn resolve_workspace_home() -> std::path::PathBuf {
    ...
    froq_config::grok_home().join("workspace")
}
...
/// back [`EventWriter::noop()`](froq_file_utils::events::EventWriter::noop)
...
async fn enqueue_workspace_tool_definitions(
    upload_queue: &froq_file_utils::queue::UploadQueue,
    session_id: &str,
    object_path: &str,
    bytes: &[u8],
) -> froq_file_utils::queue::EnqueueOutcome {
    use froq_file_utils::queue::EnqueueOutcome;
    let outcome = upload_queue
        .enqueue_bytes_blocking(
            bytes,
...
fn turn_outcome_label(outcome: froq_tool_protocol::turn_hook::TurnHookOutcome) -> TurnOutcomeLabel {
    use froq_tool_protocol::turn_hook::TurnHookOutcome;
    match outcome {
...
fn rewind_all_outcomes_from_env() -> bool {
    froq_config::env_bool("GROK_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
struct SessionToolHandle {
    tool_id: froq_tool_protocol::ToolId,
    desc: froq_tool_types::ToolDescription,
...
impl SessionToolHandle {
    fn new(
        ...
        workspace: WorkspaceHandle,
        session_id: String,
    ) -> Result<Self, froq_tool_protocol::IdError> {
        Ok(Self {
            tool_id: froq_tool_protocol::ToolId::new(tool_name)?,
            desc,
            workspace,
            session_id,
        })
...
impl froq_tool_runtime::ToolDyn for SessionToolHandle {
    ...
    ) -> froq_tool_runtime::ToolStream<froq_tool_runtime::TypedToolOutput> {
        use froq_tool_runtime::{ToolError, ToolErrorKind, ToolStreamItem, terminal_only};
...
impl WorkspaceHandle {
    ...
    pub fn create_local_harness(
        &self,
        session_id: &str,
    ) -> WorkspaceResult<froq_computer_hub_sdk::ToolHarness> {
        let session = self
            .session(session_id)
            .ok_or_else(|| WorkspaceError::SessionNotFound(session_id.to_string()))?;
        let toolset = session.toolset();
        let registry = froq_computer_hub_sdk::LocalRegistry::new();
        for def in toolset.tool_definitions() {
            let tool_name = def.function.name.clone();
            let desc = froq_tool_types::ToolDescription::new(
                tool_name.clone(),
                def.function.description.clone().unwrap_or_default(),
            );
            match SessionToolHandle::new(tool_name, desc, self.clone(), session_id.to_string()) {
                Ok(tool) => {
                    registry.register_dyn(Arc::new(tool) as Arc<dyn froq_tool_runtime::ToolDyn>);
                }
                Err(e) => {
                    tracing::warn!(
                        tool = %def.function.name,
        ...
        let session_id = froq_tool_protocol::SessionId::new(session_id.to_string())
            .map_err(|e| WorkspaceError::HubError(format!("invalid session id: {e}")))?;
        Ok(froq_computer_hub_sdk::ToolHarness::local_only_with(
            registry,
            session_id,
            froq_tool_runtime::TypedExtensions::default(),
        ))
    }
}
impl WorkspaceHandle {
    ...
    pub fn new_minimal(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: froq_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    use std::sync::Arc;
    use froq_tools::registry::types::ToolServerConfig;
    use froq_tools::types::tool::ToolKind;
    use froq_workspace_types::WorkspaceEvent;
    ...
    fn make_handle_inner(
        ...
        let config = WorkspaceConfig {
            ...
            plugin_discovery_config: Default::default(),
            hub_config: None,
            auth_provider: Some(Arc::new(froq_computer_hub_sdk::AuthCredential::bearer(
                "test-token",
            ))),
            server_metadata: None,
            status_config,
    ...
    impl froq_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> froq_tools::types::tool::ToolNamespace {
            froq_tools::types::tool::ToolNamespace::MCP
        }
        fn description_template(&self) -> &str {
            "bash cco stub"
        }
    }
    impl froq_tool_runtime::Tool for BashCcoStub {
        type Args = serde_json::Value;
        type Output = froq_tools::types::output::ToolOutput;
        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new(BASH_CCO_STUB_NAME, "bash cco stub")
        }
        async fn run(
            &self,
            _ctx: froq_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<froq_tools::types::output::ToolOutput, froq_tool_runtime::ToolError>
        {
            let output = BASH_CCO_STUB_STDOUT.as_bytes();
            Ok(froq_tools::types::output::ToolOutput::Bash(
                froq_tools::types::output::BashOutput {
                    output: output.to_vec(),
                    output_for_prompt:
                        froq_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    pub(crate) fn assert_bash_cco_terminal(typed: &froq_tool_runtime::TypedToolOutput) {
        use froq_tool_runtime::ToolOutput as _;
        let resp = typed
    ...
    pub(crate) async fn drain_terminal_ok(
        mut stream: impl futures::Stream<
            Item = froq_tool_runtime::ToolStreamItem<froq_tool_runtime::TypedToolOutput>,
        > + Unpin,
    ) -> froq_tool_runtime::TypedToolOutput {
        use futures::StreamExt;
        use froq_tool_runtime::ToolStreamItem;
    ...
    async fn local_harness_preserves_bash_chat_completion_output() {
        use froq_tool_runtime::ToolCallContext;
        let handle = make_handle();
        register_bash_cco_stub(&handle);
        let harness = handle.create_local_harness("main").expect("local harness");
        let tool_id = froq_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id");
        let stream = harness
    ...
    async fn rebind_explicit_to_explicit_with_in_flight_call_defers_then_corrects() {
        use froq_file_utils::events::ToolOutcome;
    ...
    async fn toolset_terminal(
        toolset: &Arc<froq_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn froq_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<froq_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    ///
    /// [`TerminalRunRequest`]: froq_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> froq_tools::computer::types::TerminalRunRequest {
        froq_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: froq_tools::notification::ToolNotificationHandle::noop(),
            tool_call_id: tool_call_id.to_string(),
            display_command: None,
            auto_background_on_timeout: false,
            foreground_block_budget: None,
            kind: froq_tools::computer::types::TaskKind::Bash,
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> froq_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        fn build_session_context(
            ...
            backend: Arc<dyn froq_tools::computer::types::TerminalBackend>,
        ) -> froq_tools::registry::types::SessionContext {
            self.inner
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                froq_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> froq_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn assert_hunk_tracker_stops(tracker: &froq_hunk_tracker::HunkTrackerHandle) {
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(10);
    ...
    async fn restarted_workspace_recreates_session_and_reports_lost_task() {
        ...
        let froq_tools::types::output::ToolOutput::TaskOutput(
            froq_tool_types::TaskOutputOutput::TaskNotFound(msg),
        ) = &result.output
        else {
    ...
    async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
        use froq_file_utils::events::ToolOutcome;
        use froq_tool_protocol::turn_hook::{AfterTurnPayload, BeforeTurnPayload, TurnHookOutcome};
        ...
        handle.shared().session_event_writer(sid).emit(
            froq_file_utils::events::Event::McpToolCallStarted {
    ...
    async fn before_turn_hooks_sync_session_yolo_mode() {
        use froq_tool_protocol::turn_hook::{BeforeTurnPayload, TurnHookRequest};
        let handle = make_handle();
        ...
        assert_eq!(
            reply,
            froq_tool_protocol::turn_hook::HookReply::default(),
    ...
    async fn before_turn_yolo_transition_emits_yolo_toggled_event() {
        use froq_tool_protocol::turn_hook::BeforeTurnPayload;
        let (handle, home) = make_handle_with_events();
        let sid = "sess-yolo";
        let _session = handle
    ...
    fn turn_outcome_label_maps_every_variant() {
        use froq_file_utils::events::TurnOutcomeLabel;
        use froq_tool_protocol::turn_hook::TurnHookOutcome;
        assert!(matches!(
    ...
    impl froq_file_utils::queue::TraceExportSource for UnreachableSource {
        fn resolve(&self) -> froq_file_utils::TraceExportConfig {
            froq_file_utils::TraceExportConfig {
                bucket_url: None,
                service_account_key: None,
                upload_method: froq_file_utils::UploadMethod::Proxy {
    ...
    fn spawn_test_queue(home: &std::path::Path) -> Arc<froq_file_utils::queue::UploadQueue> {
        let policy = froq_file_utils::queue::UploadRetryPolicy {
            initial_delay: std::time::Duration::from_secs(3600),
            ..Default::default()
        };
        Arc::new(froq_file_utils::queue::UploadQueue::spawn(
            home,
    ...
    fn make_queue_backed_handle_with(
        ...
        let auth: froq_computer_hub_sdk::SharedAuthProvider = Arc::new(
            froq_computer_hub_sdk::auth::AuthCredential::bearer("test-token"),
        );
        ...
        let source: Arc<dyn froq_file_utils::queue::TraceExportSource> =
            Arc::new(crate::upload::WorkspaceTraceExportSource::new(proxy));
        let policy = froq_file_utils::queue::UploadRetryPolicy {
            max_attempts: 1,
            ..Default::default()
        };
        let queue = Arc::new(froq_file_utils::queue::UploadQueue::spawn(
            home.path(),
            source,
            policy,
        ));
    ...
    async fn environment_artifact_enqueued_when_queue_present() {
        ...
        assert!(
            matches!(
                outcome,
                Some(froq_file_utils::queue::EnqueueOutcome::Enqueued)
            ),
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("froq-workspace-test-cwd-override");
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = froq_hooks::config::HookSpec {
                name: "injected".into(),
                event: froq_hooks::event::HookEventName::SessionStart,
                handler_type: froq_hooks::config::HandlerType::Command,
    ...
    fn workspace_shared_auth_provider_uses_workspace_config() {
        ...
        let service_auth: froq_computer_hub_sdk::SharedAuthProvider = Arc::new(
            froq_computer_hub_sdk::auth::AuthCredential::bearer("froq-service-token"),
        );
        let hub_auth: froq_computer_hub_sdk::SharedAuthProvider = Arc::new(
            froq_computer_hub_sdk::auth::AuthCredential::bearer("hub-token"),
        );
        let hub_cfg = crate::hub::HubConfig {
    ...
    async fn fork_session_inherits_viewer_ctx_from_parent() {
        ...
        let parent = handle
            .create_session_with_tracker_and_viewer_ctx(
                ...
                froq_hunk_tracker::HunkTrackerHandle::noop(),
                None,
                CapabilityMode::All,
                Some(froq_tool_runtime::WorkspaceViewerContext {
    ...
    fn handler_names(resolved: &froq_computer_hub_sdk::ResolvedSessionHandlers) -> Vec<String> {
        resolved
            .handlers
            .iter()
            .map(|h| h.tool_id().as_str().to_owned())
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        let resolved = resolver(
            froq_tool_protocol::SessionId::new("bind-e2e-strict").unwrap(),
        ...
        assert!(
            reason.contains(froq_version::VERSION),
    ...
    async fn strict_rpc_only_bind_fails_closed_with_resolve_error_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            froq_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap(),
            Some(serde_json::json!({
                "metadata": {
    ...
    async fn strict_bind_with_explicit_toolset_serves_it_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            froq_tool_protocol::SessionId::new("bind-e2e-tools").unwrap(),
            Some(serde_json::json!({
    ...
    async fn lax_bind_without_metadata_uses_default_catalog_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            froq_tool_protocol::SessionId::new("bind-e2e-lax").unwrap(),
            None,
        )
        .await
        .expect("bind must succeed");
    ...
    async fn rejected_rebind_config_keeps_resolve_error_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-rejected").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!({
                "metadata": {"tools": [{"id": "GrokBuild:read_file"}]},
    ...
    async fn explicit_empty_toolset_rebind_never_swaps_session_tools() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!({
                "metadata": {"tools": [{"id": "GrokBuild:read_file"}]},
    ...
    async fn strict_rebind_with_corrected_toolset_heals_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-heal").unwrap();
        let first = resolver(
            sid.clone(),
    ...
    async fn owner_toolset_survives_concurrent_consumer_shaped_rebinds() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-consumer-storm").unwrap();
    ...
    async fn restored_server_first_bind_ordering_decides_capability_and_toolset() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-restore-read-first").unwrap();
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-restore-write-first").unwrap();
        resolver(
            sid.clone(),
            Some(serde_json::json!({"metadata": {"capability_mode": "read_write"}})),
        )
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-restore-owner-first").unwrap();
    ...
    async fn bind_flow_rebinds_keep_backend_and_task_alive_end_to_end() {
        ...
        let handle = make_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = froq_tool_protocol::SessionId::new("bind-e2e-bg").unwrap();
    ...
    async fn compute_turn_injections_after_returns_skipped_ack_without_queue() {
        use froq_tool_protocol::turn_hook::{AfterTurnPayload, TurnHookOutcome, TurnHookRequest};
        let handle = make_handle();
    ...
    async fn compute_turn_injections_before_runs_turn_start_and_replies_noop() {
        use froq_tool_protocol::turn_hook::{BeforeTurnPayload, HookReply, TurnHookRequest};
        let handle = make_handle();
        let reply = handle
            .compute_turn_injections(
                "main",
    ...
    fn make_handle_with_queue_routing(
        ...
        WorkspaceHandle,
        Arc<froq_file_utils::queue::UploadQueue>,
        tempfile::TempDir,
    ) {
        use froq_computer_hub_sdk::auth::{AuthCredential, AuthProvider};
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let config = WorkspaceConfig {
            root_cwd: cwd,
    ...
    async fn wait_enqueued(queue: &froq_file_utils::queue::UploadQueue, want: u64) {
    ...
    async fn enqueue_workspace_tool_definitions_reports_enqueued_at_session_root() {
        ...
        assert_eq!(outcome, froq_file_utils::queue::EnqueueOutcome::Enqueued);
        assert_eq!(
            queue
                .stats()
                .enqueued
    ...
    async fn two_phase_drain_no_queue_marks_draining_and_returns_zero() {
        ...
        assert_eq!(
            snap.status,
            froq_tool_protocol::ToolServerLifecycleStatus::Draining
        );
        assert!(
