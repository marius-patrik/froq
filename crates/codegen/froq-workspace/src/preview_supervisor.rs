...
//! spawns the unchanged `/usr/local/bin/froq-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/froq-preview-proxy";
...
impl PreviewArgs {
    ...
    /// `froq-preview-proxy/src/cli.rs`). Absent options and a false
...
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT froq_tty_utils::detach_command: the proxy must stay in
...
/// `froq-preview-proxy`'s `/__control/activity` route).
...
/// the default in `froq-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match froq-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
