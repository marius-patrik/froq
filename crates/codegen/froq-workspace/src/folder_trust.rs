...
//! `project_scope_allowed`, the loader filters) lives in `froq-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `froq-shell`
...
use froq_config_types::{BoolFlag, RemoteSettings};
...
///
/// Kept local (not in `froq-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    ...
    if std::env::var(froq_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    let user = froq_config::load_from_disk().ok();
    let managed = froq_config::load_managed_config().ok();
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = froq_agent::repo::RepoDirChain::resolve(cwd);
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = froq_config::load_config_file(&path) else {
            continue;
        };
    ...
    if !froq_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    ...
    if !froq_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
mod tests {
    ...
    // tests (the workspace crate has no `serial_test`/`froq-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(froq_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn is_local_build_honors_test_version_override() {
        ...
        {
            let _sim = EnvVarGuard::set(froq_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
        ...
        let _unset = EnvVarGuard::unset(froq_version::TEST_VERSION_ENV);
