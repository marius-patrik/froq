...
use froq_computer_hub_sdk::ToolServerHandler;
use froq_tools::computer::types::TaskKind;
use froq_tools::implementations::grok_build::scheduler::interval::interval_to_human;
use froq_tools::implementations::grok_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use froq_tools::registry::types::FinalizedToolset;
use froq_tools::types::resources::Terminal;
use froq_workspace_types::rpc::workspace::{
    BackgroundTaskSnapshotWire, ScheduledTaskSnapshotWire, TasksSnapshotResponse,
};
use froq_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
) -> Vec<froq_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use froq_tools::computer::types::TaskKind;
    use froq_tools::types::resources::Terminal;
    use froq_tools::types::tool::ToolKind;
    use froq_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &froq_tools::registry::types::FinalizedToolset,
) -> Vec<froq_workspace_types::rpc::workspace::TodoSummaryWire> {
    use froq_tools::implementations::grok_build::todo::{TodoState, TodoStatus};
    use froq_tools::types::resources::State;
    use froq_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use froq_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use froq_workspace_types::rpc::search::FuzzyStatusReq;
        use froq_workspace_types::rpc::skills::DiscoverPluginsReq;
        use froq_workspace_types::rpc::workspace::{
        ...
        use froq_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<froq_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use froq_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use froq_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    ) -> Option<froq_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use froq_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use froq_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                "GrokBuild:read_file",
                Some(froq_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(froq_tool_runtime::SessionContext("main".to_owned()));
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use froq_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
