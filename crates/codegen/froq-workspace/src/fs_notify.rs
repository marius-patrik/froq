...
//! FsNotify adapter functions bridging [`froq_fsnotify`] events to
...
//! sent to the [`IndexManagerHandle`](froq_codebase_graph::IndexManagerHandle).
//! The [`refresh_codebase_graph_after_head_change`] helper handles
//! git HEAD changes by diffing `ORIG_HEAD..HEAD`.

use std::path::{Path, PathBuf};

use froq_fsnotify::{FsEvent, FsEventKind};
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> froq_codebase_graph::FileEvent {
    use froq_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`froq_fsnotify::FsEventKind`] to the wire-type
/// [`froq_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> froq_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => froq_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => froq_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => froq_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => froq_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => froq_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](froq_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<froq_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
    cancel: tokio_util::sync::CancellationToken,
    codebase_index: Option<std::sync::Arc<froq_codebase_graph::IndexManagerHandle>>,
) {
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    froq_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<froq_codebase_graph::FileEvent> {
    let mut parts = line.splitn(3, '\t');
    let status = parts.next()?.trim();
    let path = parts.next()?;

    match status.chars().next()? {
        'A' => Some(froq_codebase_graph::FileEvent::created(repo_root.join(path))),
        'D' => Some(froq_codebase_graph::FileEvent::removed(repo_root.join(path))),
        'R' | 'C' => {
            let new_path = parts.next()?;
            Some(froq_codebase_graph::FileEvent::renamed(
                repo_root.join(path),
                repo_root.join(new_path),
            ))
        }
        _ => Some(froq_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &froq_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<froq_workspace_types::WorkspaceEvent>,
) {
    ...
    froq_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(froq_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            froq_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: froq_workspace_types::FsEventKind,
) -> froq_codebase_graph::FileEvent {
    use froq_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        froq_workspace_types::FsEventKind::Created => FileEventKind::Created,
        froq_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        froq_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        froq_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn workspace_event_kind_round_trip() {
        use froq_workspace_types::FsEventKind as WsKind;
        assert_eq!(
    ...
    fn codebase_graph_event_mapping() {
        use froq_codebase_graph::FileEventKind;
