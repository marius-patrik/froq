...
//! `froq-agent` and `froq-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `froq-tools` for external
...
// workspace crate without a direct froq-tools dependency.
pub use froq_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use froq_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use froq_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use froq_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`froq_agent::prompt::skills::list_skills`] with
...
pub async fn discover_skills(root_cwd: &Path, config: &SkillsConfig) -> Vec<Value> {
    ...
    let skills = froq_agent::prompt::skills::list_skills(
        Some(&cwd_str),
        config,
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = froq_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
...
///
/// Delegates to [`froq_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = froq_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    ...
    match froq_config::load_config_file(&config_path) {
...
mod tests {
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        ...
        // break discovery.
        let src = froq_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: froq_workspace_types::rpc::agents_md::AgentConfigFile =
