...
use froq_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource};
...
async fn shared_dedupes_by_directory() {
    ...
    let Ok(a) = froq_fsnotify::shared(path.clone(), FsConfig::default()) else {
        eprintln!("skipping: OS watcher unavailable (resource limit?)");
        return;
    };
    let before = froq_fsnotify::stats();
    let b = froq_fsnotify::shared(path.clone(), FsConfig::default()).unwrap();
    ...
    let c = froq_fsnotify::shared(other.path().to_path_buf(), FsConfig::default()).unwrap();
    assert!(!Arc::ptr_eq(&a, &c), "different dirs must not share");
    assert_eq!(
        froq_fsnotify::stats().created_total - after.created_total,
        1,
    ...
    drop(a);
    drop(b);
    let d = froq_fsnotify::shared(path, FsConfig::default()).unwrap();
    assert_eq!(
        Arc::strong_count(&d),
        1,
...
///
/// ```bash
/// cargo test -p froq-fsnotify --test integration \
...
async fn shared_watcher_scaling_demo() {
    ...
    for _ in 0..SESSIONS {
        let Ok(src) = froq_fsnotify::shared(path.clone(), FsConfig::default()) else {
            eprintln!("skipping: OS watcher unavailable (resource limit?)");
            return;
        };
        handles.push(src);
