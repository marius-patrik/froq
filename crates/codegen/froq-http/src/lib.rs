...
//! `froq_sampler::shared_http` (one HTTP/2 pooled client plus
...
// `OriginClientInfo` is owned by `froq-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `froq-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `froq-telemetry` consumes
// the same type via `froq_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use froq_sampler::OriginClientInfo;
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn froq_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(froq_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
