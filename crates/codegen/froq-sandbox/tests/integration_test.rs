//! Integration tests for froq-sandbox.
//!
...
fn test_sandbox_logger() {
    use froq_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!froq_sandbox::should_restrict_child_network());
}
