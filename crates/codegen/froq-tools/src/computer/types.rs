...
/// (`froq-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (froq-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in froq-tools, spawns processes)
/// - `AcpTerminalBackend` (in froq-shell, calls ACP protocol)
#[async_trait::async_trait]
