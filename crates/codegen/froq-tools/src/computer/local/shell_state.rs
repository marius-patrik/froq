...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match froq_config::shell::detect_unix_shell_kind() {
            froq_config::shell::UnixShellKind::Bash => Self::Bash,
            froq_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }

    /// Resolved absolute path to the shell binary. Falls back from `$SHELL` →
    /// `which` → common dirs → `/bin/<name>`. Result is cached process-wide
    /// in `froq_config::shell::unix_shell_path`. See that function for
    /// the full cascade. Returns `&'static str`.
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            Self::Bash => froq_config::shell::UnixShellKind::Bash,
            Self::Zsh => froq_config::shell::UnixShellKind::Zsh,
        };
        froq_config::shell::unix_shell_path(kind)
    }
