...
//!
//! `ToolSearchIndex` is defined in `froq-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `froq-shell`
...
/// in `froq-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `froq-shell` after MCP initialization.
#[derive(Clone)]
