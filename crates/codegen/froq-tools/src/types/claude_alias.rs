...
//!
//! Two consumers read it independently. The hook matcher (`froq-hooks`) needs the
//! Grok tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`froq-agent`) needs the [`ToolKind`] a `tools:`
...
/// Every distinct Grok name the table references, for the `froq-agent` drift-check
