...
impl Resources {
    ...
    pub fn require<T: Send + Sync + 'static>(&self) -> Result<&T, froq_tool_runtime::ToolError> {
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), froq_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn froq_tool_runtime::ToolDispatch>` so it can
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn froq_tool_runtime::ToolDispatch>);
#[derive(Debug, Clone)]
...
pub trait ManagedGatewayToolCaller: Send + Sync {
    async fn call_tool(
        ...
        caller: &str,
    ) -> Result<ManagedGatewayToolCallResponse, froq_tool_runtime::ToolError>;
}
#[derive(Clone)]
...
///
/// Stamped onto [`froq_tool_runtime::ToolCallContext::extensions`] by
...
/// depending on `froq-mcp` directly.  Follows the same pattern as
