...
/// Generate `impl From<$err> for froq_tool_runtime::ToolError` that wraps the
/// error as an execution failure tagged with the given tool ID.
macro_rules! impl_runtime_error_from {
    ($($err:ty => $tool_id:literal),+ $(,)?) => {
        $(
            impl From<$err> for froq_tool_runtime::ToolError {
                fn from(err: $err) -> Self {
                    froq_tool_runtime::ToolError::execution(
                        froq_tool_protocol::ToolId::new($tool_id).expect("valid static tool id"),
                        err.to_string(),
                    )
                }
            }
