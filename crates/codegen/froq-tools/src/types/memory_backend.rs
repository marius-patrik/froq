...
//!
//! `MemoryBackend` is defined in `froq-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `froq-tools`.
/// Mirrors `froq_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "froq_memory";
