...
/// this crate: instead of `froq-tools` referencing an out-of-tree tool
/// pack, the pack calls
...
impl ToolConfig {
    ...
    /// `froq_tool_runtime::Tool::id()`. Use this for built-in tools known
    ...
    where
        T: crate::types::tool_metadata::ToolMetadata + froq_tool_runtime::Tool + Default,
    {
        Self::from(&T::default())
    }
...
impl<T: crate::types::tool_metadata::ToolMetadata + froq_tool_runtime::Tool> From<&T>
    for ToolConfig
{
    fn from(tool: &T) -> Self {
        Self {
            id: format!(
                "{}:{}",
                tool.tool_namespace(),
                froq_tool_runtime::Tool::id(tool).as_str()
            ),
            params: None,
            name_override: None,
            params_name_overrides: None,
...
pub struct SessionContext {
    ...
    /// Auth provider which returns a froq_computer_hub_sdk::AuthCredential. Can be used by
    ...
    pub auth_provider: Option<froq_computer_hub_sdk::SharedAuthProvider>,
...
pub async fn drain_value_stream(
    mut stream: froq_tool_runtime::ToolStream<froq_tool_runtime::TypedToolOutput>,
) -> Result<serde_json::Value, froq_tool_runtime::ToolError> {
    use futures::StreamExt;
    while let Some(item) = stream.next().await {
        match item {
            froq_tool_runtime::ToolStreamItem::Progress(_) => continue,
            froq_tool_runtime::ToolStreamItem::Terminal(result) => {
...
fn stream_no_terminal_error() -> froq_tool_runtime::ToolError {
...
struct DispatchParts {
    ...
    lr_handle: Arc<dyn froq_computer_hub_core::ToolHandle>,
...
struct ToolEntry {
    ...
    parse_input: Box<
        dyn Fn(serde_json::Value) -> Result<ToolInput, froq_tool_runtime::ToolError> + Send + Sync,
    >,
    /// Registers this tool into a `LocalRegistry` using the concrete type.
    /// Captured at `register::<T>()` time when T is known.
    register_in_local: Box<dyn Fn(&froq_computer_hub_sdk::LocalRegistry) + Send + Sync>,
}
...
pub struct FinalizedToolset {
    ...
    local_registry: froq_computer_hub_sdk::LocalRegistry,
    ...
    workspace_viewer_ctx: Option<froq_tool_runtime::WorkspaceViewerContext>,
}
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct ToolRegistryBuilder {
    ...
    shared_local_registry: Option<froq_computer_hub_sdk::LocalRegistry>,
}
...
impl ToolRegistryBuilder {
            ...
            + 'static,
    {
        ...
        let id = froq_tool_runtime::Tool::id(&tool).as_str().to_string();
        let kind = tool.kind();
        ...
        self.tools.insert(
            ...
            ToolEntry {
                ...
                register_in_local: Box::new(|lr: &froq_computer_hub_sdk::LocalRegistry| {
    ...
    pub fn with_local_registry(mut self, registry: froq_computer_hub_sdk::LocalRegistry) -> Self {
...
///
/// Implements the canonical `froq_tool_runtime::ToolDispatch` trait so the
...
#[async_trait::async_trait]
impl froq_tool_runtime::ToolDispatch for InnerDispatchForToolset {
    async fn call(
        &self,
        ...
        let result = self
            ...
            .and_then(|output| {
                let value = serde_json::to_value(&output).map_err(|e| {
                    froq_tool_runtime::ToolError::custom("output_encoding", e.to_string())
                })?;
                Ok(froq_tool_runtime::TypedToolOutput::from_value(
                    tool_id.clone(),
                    value,
                ))
            });
        froq_tool_runtime::terminal_only(result)
    }
}
impl FinalizedToolset {
    ...
    pub fn empty_for_test() -> Self {
        Self {
            ...
            local_registry: froq_computer_hub_sdk::LocalRegistry::new(),
    ...
    pub fn local_registry(&self) -> &froq_computer_hub_sdk::LocalRegistry {
        &self.local_registry
    }
    ...
    fn tool_not_found_error(tool_name: &str) -> froq_tool_runtime::ToolError {
        let tid = froq_tool_protocol::ToolId::new(tool_name)
            .unwrap_or_else(|_| froq_tool_protocol::ToolId::new("unknown").expect("valid"));
        froq_tool_runtime::ToolError::not_found(tid, format!("Tool not found: {tool_name}"))
    }
    pub async fn try_parse(
        &self,
        tool_name: &str,
        tool_params: &serde_json::Value,
    ) -> Result<ToolInput, froq_tool_runtime::ToolError> {
    ...
    async fn call_raw(
        ...
        parent_ctx: froq_tool_runtime::ToolCallContext,
    ) -> Result<crate::types::output::ToolOutput, froq_tool_runtime::ToolError> {
        ...
        let mut ctx = froq_tool_runtime::ToolCallContext::new(parent_ctx.call_id.clone());
        ...
        if let Some(cwd) = parent_ctx.extensions.get::<froq_tool_runtime::Cwd>() {
            ctx.extensions.insert((*cwd).clone());
        }
        let tool_id = froq_tool_protocol::ToolId::new(&registry_id)
            .unwrap_or_else(|_| froq_tool_protocol::ToolId::new("unknown").expect("valid"));
        let lr_handle = self.local_registry.find(&tool_id).ok_or_else(|| {
            froq_tool_runtime::ToolError::not_found(
                tool_id,
        ...
        (output_converter)(value)
            .map_err(|e| froq_tool_runtime::ToolError::custom("output_decoding", e.to_string()))
    }
    ...
    ) -> Result<ToolRunResult, froq_tool_runtime::ToolError> {
        use futures::StreamExt;
        let mut stream = self.call_streaming(tool_name, tool_args, tool_call_id, cwd_override);
        while let Some(item) = stream.next().await {
            match item {
                froq_tool_runtime::ToolStreamItem::Progress(_) => continue,
                froq_tool_runtime::ToolStreamItem::Terminal(result) => return result,
            }
        }
        Err(stream_no_terminal_error())
    }
    ...
    ///
    /// [`ToolStream`]: froq_tool_runtime::ToolStream
    /// [`ToolStreamItem::Progress`]: froq_tool_runtime::ToolStreamItem::Progress
    pub fn call_streaming(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> froq_tool_runtime::ToolStream<ToolRunResult> {
        use futures::StreamExt;
        ...
        Box::pin(async_stream::stream! {
            ...
            froq_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; } }; let
            ...
            match item { froq_tool_runtime::ToolStreamItem::Progress(p) => { yield
            froq_tool_runtime::ToolStreamItem::Progress(p); }
            froq_tool_runtime::ToolStreamItem::Terminal(Err(e)) => { yield
            froq_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; }
            froq_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => { let run_result
            = this.finalize_output(typed.value, & output_converter,
            effective_tool_name). await; yield
            froq_tool_runtime::ToolStreamItem::Terminal(run_result); return; } } }
            yield
            froq_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> Result<DispatchParts, froq_tool_runtime::ToolError> {
        let (registry_id, output_converter, reverse_params) = {
            let tools = self.tools.read();
            let entry = tools
                .iter()
        ...
        let rt_call_id = froq_tool_protocol::ToolCallId::new(tool_call_id)
            .unwrap_or_else(|_| froq_tool_protocol::ToolCallId::new_v7());
        let mut ctx = froq_tool_runtime::ToolCallContext::new(rt_call_id);
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        ctx.extensions.insert(
            crate::types::resources::InvokingToolParamNames::from_reverse_params(&reverse_params),
        ...
        if let Some(cwd) = cwd_override {
            ctx.extensions.insert(froq_tool_runtime::Cwd(cwd));
        }
        if let Some(ref version) = contract_version {
            ctx.extensions
                .insert(froq_tool_runtime::BehaviorVersion(version.clone()));
        }
    ...
    ///
    /// The tool must implement `froq_tool_runtime::Tool + ToolMetadata`.
    ...
    ) -> Result<(), froq_tool_runtime::ToolError>
    where
        T: froq_tool_runtime::Tool + ToolMetadata + std::fmt::Debug + Send + Sync + 'static,
        T::Output: serde::Serialize,
    {
        let mut tools = self.tools.write();
        if tools.iter().any(|t| t.client_name == name) {
            return Err(froq_tool_runtime::ToolError::invalid_arguments(format!(
                "Tool already registered: {name}"
            )));
        }
        let description = tool.description_template().to_string();
        let kind = tool.kind();
        let registry_id = froq_tool_runtime::Tool::id(&tool).as_str().to_owned();
    ...
    pub fn unregister_tools_by_prefix(&self, prefix: &str) -> usize {
        ...
        let to_remove: Vec<_> = tools
            ...
            .filter_map(|t| froq_tool_protocol::ToolId::new(&t.registry_id).ok())
            .collect();
    ...
    pub fn unregister_tool_by_name(&self, name: &str) -> bool {
        ...
        let tool_id = tools
            ...
            .and_then(|t| froq_tool_protocol::ToolId::new(&t.registry_id).ok());
        let before = tools.len();
...
mod tests {
    ...
    impl froq_tool_runtime::Tool for FakeMcpTool {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new("fake_mcp").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new("fake_mcp", &self.description)
        }
        async fn run(
            &self,
    ...
    impl froq_tool_runtime::Tool for NonStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new("non_streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new("non_streaming_stub", "non-streaming stub")
        }
        async fn run(
            &self,
            _ctx: froq_tool_runtime::ToolCallContext,
    ...
    impl froq_tool_runtime::Tool for StreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new("streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new("streaming_stub", "streaming stub")
        }
        async fn run(
            &self,
            _ctx: froq_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            _ctx: froq_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> froq_tool_runtime::ToolStream<String> {
            Box::pin(futures::stream::iter(vec![
                froq_tool_runtime::ToolStreamItem::Progress(froq_tool_runtime::ToolProgress::Text {
                    text: "progress-1".into(),
                }),
                froq_tool_runtime::ToolStreamItem::Terminal(Ok("terminal-value".to_string())),
            ]))
        }
    }
        ...
        while let Some(item) = stream.next().await {
            match item {
                froq_tool_runtime::ToolStreamItem::Progress(p) => {
                    ...
                    assert!(matches!(p, froq_tool_runtime::ToolProgress::Text { .. }));
                    progress_count += 1;
                }
                froq_tool_runtime::ToolStreamItem::Terminal(result) => {
    ...
    impl froq_tool_runtime::Tool for NoTerminalStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new("no_terminal_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new("no_terminal_stub", "no-terminal stub")
        }
        async fn run(
            &self,
            _ctx: froq_tool_runtime::ToolCallContext,
    ...
    async fn prepare_dispatch_stamps_workspace_viewer_ctx_when_present() {
        let (toolset, _tmp) =
            toolset_with_viewer_ctx(Some(froq_tool_runtime::WorkspaceViewerContext {
        ...
        let wvc = parts
            .ctx
            .extensions
            .get::<froq_tool_runtime::WorkspaceViewerContext>()
    ...
    async fn bash_streaming_progress_emitted_via_finalized_toolset_when_gate_on() {
        ...
        while let Some(item) = stream.next().await {
            match item {
                froq_tool_runtime::ToolStreamItem::Progress(_) => progress_count += 1,
                froq_tool_runtime::ToolStreamItem::Terminal(r) => {
