...
impl ToolRegistryBuilder {
    ...
    pub fn new() -> Self {
        ...
        b.register::<grok_build::ManageTokensTool>();
    ...
    pub fn call_streaming(
        ...
        Box::pin(async_stream::stream! {
            let parts = match this.prepare_dispatch(
                ...
                Ok(parts) => parts,
                Err(e) => {
                    yield froq_tool_runtime::ToolStreamItem::Terminal(Err(e));
                    return;
                }
            };
            let DispatchParts {
            ...
            while let Some(item) = inner.next().await {
                match item {
                    froq_tool_runtime::ToolStreamItem::Progress(p) => {
                        yield froq_tool_runtime::ToolStreamItem::Progress(p);
                    }
                    froq_tool_runtime::ToolStreamItem::Terminal(Err(e)) => {
                        yield froq_tool_runtime::ToolStreamItem::Terminal(Err(e));
                        return;
                    }
                    froq_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                        let run_result = this
                            .finalize_output(typed.value, &output_converter, effective_tool_name)
                            .await;
                        yield froq_tool_runtime::ToolStreamItem::Terminal(run_result);
                        return;
                    }
                }
            }
            yield froq_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
