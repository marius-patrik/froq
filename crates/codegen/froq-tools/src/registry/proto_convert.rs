//! Conversion from the gRPC wire config types (`froq-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`froq_tools_api::config_validation`] so every consumer (this
...
pub use froq_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`froq_tools_api::ToolConfigEntry`] to a runtime
/// [`ToolConfig`].
///
...
pub fn tool_config_from_entry(
    index: usize,
    entry: froq_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let froq_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params = froq_tools_api::config_validation::parse_params_json(
        index,
        &id,
        params_json.as_deref(),
    )?;
    froq_tools_api::config_validation::validate_name_override(
        index,
        &id,
...
pub fn tool_server_config_from_entries(
    entries: Vec<froq_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> froq_tools_api::ToolConfigEntry {
        froq_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
