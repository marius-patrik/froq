//! ToolBridge: adapter that wraps `froq-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, froq_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, froq_tool_runtime::ToolError> {
        use crate::implementations::grok_build::scheduler::types::{
            SchedulerCommand, SchedulerHandle,
        };
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    froq_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                froq_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
        reply_rx
            .await
            .map_err(|_| {
...
mod tests {
    ...
    impl froq_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
