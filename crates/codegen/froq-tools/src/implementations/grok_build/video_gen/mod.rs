...
const froq_VIDEO_BASE_MODEL: &str = "grok-imagine-video";
const froq_VIDEO_QUALITY_MODEL: &str = "grok-imagine-video-1.5-preview";
...
pub use froq_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> froq_file_utils::s3::S3StaticCredentials {
        froq_file_utils::s3::S3StaticCredentials {
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, froq_tool_runtime::ToolError> {
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|e| {
                froq_tool_runtime::ToolError::invalid_arguments(format!(
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), froq_tool_runtime::ToolError>(())
        })?;
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, froq_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(froq_tool_runtime::ToolError::new(
                froq_tool_runtime::ToolErrorKind::Custom,
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(froq_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, froq_tool_runtime::ToolError> {
        let response = self.download_http.get(url).send().await.map_err(|e| {
            froq_tool_runtime::ToolError::invalid_arguments(format!("Failed to download video: {e}"))
        })?;
    ...
    async fn finish_zdr_video(
        ...
        let config = self.zdr_video_output_s3.as_ref().ok_or_else(|| {
            froq_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, froq_tool_runtime::ToolError> {
        ...
        let upload_url = froq_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, froq_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = froq_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
async fn resolve_image_reference(value: &str) -> Result<String, froq_tool_runtime::ToolError> {
    ...
    if value.starts_with("data:image/") {
        let comma = value.find(',').ok_or_else(|| {
            froq_tool_runtime::ToolError::invalid_arguments("malformed data URL in image reference")
        })?;
    ...
    let (_w, _h, mime) =
        crate::util::image_validate::validate_image_bytes(&raw_bytes).map_err(|e| {
            froq_tool_runtime::ToolError::invalid_arguments(format!("invalid image reference: {e}"))
        })?;
...
fn validate_one_of(
    ...
    Err(froq_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), froq_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), froq_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, froq_tool_runtime::ToolError> {
    let absolute_path = client
        .writer
        .save(session_folder, video_bytes, None)
        .await
        .map_err(|e| froq_tool_runtime::ToolError::invalid_arguments(e.to_string()))?;
...
) -> Result<MediaGenOutput, froq_tool_runtime::ToolError> {
    match outcome {
...
impl froq_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
impl froq_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    fn image_and_reference_payload_fields_are_serialized() {
        let payload = GenerateVideoPayload {
            model: froq_VIDEO_QUALITY_MODEL,
            prompt: "animate",
    ...
    async fn image_to_video_rejects_bad_duration() {
        ...
        let err = froq_tool_runtime::Tool::run(
            &tool,
