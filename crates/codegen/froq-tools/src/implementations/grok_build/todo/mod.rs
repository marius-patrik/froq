...
impl froq_tool_runtime::Tool for TodoWriteTool {
    type Args = TodoWriteInput;
    type Output = TodoWriteOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
    ...
    ) -> Result<TodoWriteOutput, froq_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn name_and_description() {
        ...
        assert_eq!(froq_tool_runtime::Tool::id(&tool).as_str(), "todo_write");
    ...
    async fn replace_mode_creates_items() {
        ...
        let output = expect_success(
            froq_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
                .await
                .unwrap(),
        );
    ...
    async fn replace_clears_previous_state() {
        ...
        froq_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input1)
            .await
            .unwrap();
        ...
        let output = expect_success(
            froq_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input2)
                .await
                .unwrap(),
        );
    ...
    async fn merge_with_lost_state_uses_id_fallback() {
        ...
        let output = expect_success(
            froq_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input)
                .await
                .unwrap(),
        );
        assert_eq!(output.todos.len(), 1);
