...
pub use froq_tools_api::slash_commands::UPDATE_GOAL_TOOL_NAME;
...
impl froq_tool_runtime::ToolOutput for UpdateGoalOutput {}
...
impl froq_tool_runtime::Tool for UpdateGoalTool {
    type Args = UpdateGoalInput;
    type Output = UpdateGoalOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new(UPDATE_GOAL_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<UpdateGoalOutput, froq_tool_runtime::ToolError> {
...
pub fn render_ack_into_output(
    ...
    match ack {
        ...
        UpdateGoalAck::Rejected { reason, detail } => Err(froq_tool_runtime::ToolError::custom(
            reason.error_code(),
            detail,
        )),
    }
