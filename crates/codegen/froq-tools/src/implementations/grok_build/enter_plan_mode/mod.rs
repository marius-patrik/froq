    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        Expr::Value(ToolRequirement::Tool {
            namespace: crate::types::tool_metadata::ToolMetadata::tool_namespace(&ExitPlanModeTool)
                .to_string(),
            id: froq_tool_runtime::Tool::id(&ExitPlanModeTool).to_string(),
            if_params: None,
        })
    }
}

impl froq_tool_runtime::Tool for EnterPlanModeTool {
    type Args = EnterPlanModeInput;
    type Output = EnterPlanModeOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("enter_plan_mode").expect("valid tool id")
    }
    ...
    ) -> Result<EnterPlanModeOutput, froq_tool_runtime::ToolError> {
