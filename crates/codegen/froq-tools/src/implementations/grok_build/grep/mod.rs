...
fn grep_timeout() -> Duration {
    Duration::from_secs(grep_timeout_secs(froq_tty_utils::is_wsl()))
}
...
static GREP_CAPABILITIES: LazyLock<froq_tool_protocol::ToolCapabilities> =
...
impl froq_tool_runtime::Tool for GrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("grep").expect("valid tool id")
    }
    ...
    async fn execute(
    ...
    ) -> froq_tool_runtime::ToolStream<GrepSearchOutput> {
        ...
        let Some(spec) = admitted_spec else {
            return froq_tool_runtime::terminal_only(self.run(ctx, input).await);
        };
    ...
    async fn run(
        ...
        ctx: froq_tool_runtime::ToolCallContext,
        input: GrepSearchInput,
    ) -> Result<GrepSearchOutput, froq_tool_runtime::ToolError> {
...
fn grep_progress_stream(
    ctx: froq_tool_runtime::ToolCallContext,
    input: GrepSearchInput,
    spec: &'static froq_tool_protocol::StreamingSpec,
    span: tracing::Span,
) -> froq_tool_runtime::ToolStream<GrepSearchOutput> {
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Early(out)) => {
                ...
                yield froq_tool_runtime::ToolStreamItem::Terminal(Ok(out));
                return;
            }
            Err(e) => {
        ...
        if timed_out {
            ...
            if stdout_buf.is_empty() {
                yield froq_tool_runtime::ToolStreamItem::Terminal(Ok(grep_timeout_output(secs)));
            } else {
                ...
                output.exit_code = -1;
                yield froq_tool_runtime::ToolStreamItem::Terminal(Ok(output));
            }
            return;
        }
...
async fn prepare_grep(
    ctx: &froq_tool_runtime::ToolCallContext,
    input: &GrepSearchInput,
) -> Result<GrepStep, froq_tool_runtime::ToolError> {
...
struct BodyStreamer<'a> {
    spec: &'a froq_tool_protocol::StreamingSpec,
...
impl<'a> BodyStreamer<'a> {
    fn new(spec: &'a froq_tool_protocol::StreamingSpec, config: &'a GrepFormatConfig) -> Self {
        Self {
            spec,
            config,
            body: String::new(),
    ...
    fn feed(&mut self, bytes: &[u8]) -> Vec<froq_tool_runtime::ToolProgress> {
    ...
    fn finish(&mut self) -> Option<froq_tool_runtime::ToolProgress> {
    ...
    fn push_line(&mut self, line: &[u8]) -> Option<froq_tool_runtime::ToolProgress> {
        ...
        froq_tool_runtime::stream_chunk(
            self.spec,
            self.body.as_bytes(),
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        let tool = GrepTool;
        assert_eq!(froq_tool_runtime::Tool::id(&tool).as_str(), "grep");
    ...
    async fn tool_grep_no_matches() {
        ...
        let tool = GrepTool;
        let output = froq_tool_runtime::Tool::run(
            &tool,
    ...
    async fn deny_read_globs_exclude_denied_files() {
        ...
        let run = |resources: Resources, input| async {
            let out =
                froq_tool_runtime::Tool::run(&GrepTool, test_ctx(resources.into_shared()), input)
                    .await
                    .unwrap();
    ...
    fn read_grep_delta(p: &froq_tool_runtime::ToolProgress) -> String {
        match p {
    ...
    async fn grep_streaming_body_matches_card_body() {
        ...
        let mut terminal: Option<Result<GrepSearchOutput, froq_tool_runtime::ToolError>> = None;
    ...
    async fn grep_streaming_suppressed_when_gate_off() {
        ...
        let tool = GrepTool;
        let mut stream =
            froq_tool_runtime::Tool::execute(&tool, ctx, make_grep_input("findme")).await;

        let mut progress = 0usize;
        let mut terminal: Option<Result<GrepSearchOutput, froq_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                froq_tool_runtime::ToolStreamItem::Progress(_) => progress += 1,
                froq_tool_runtime::ToolStreamItem::Terminal(r) => {
                    assert!(terminal.is_none(), "more than one Terminal yielded");
                    terminal = Some(r);
                }
    ...
    async fn grep_streaming_body_matches_card_body_when_truncated() {
        ...
        let tool = GrepTool;
        let mut stream =
            froq_tool_runtime::Tool::execute(&tool, test_ctx(resources.into_shared()), input).await;
