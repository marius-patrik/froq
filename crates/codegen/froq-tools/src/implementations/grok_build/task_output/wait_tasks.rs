...
use froq_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            froq_tool_types::build_wait_tasks_description(&froq_tool_types::WaitTasksToolNaming {
...
/// [`froq_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl froq_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
