...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let grok_build_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: froq_tool_runtime::Tool::id(&BashTool).as_str().to_string(),
    ...
    let grok_build_concise_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: froq_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
    ...
    let opencode_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: froq_tool_runtime::Tool::id(&OpenCodeBashTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
...
pub(crate) fn task_output_requires_expr() -> Expr<ToolRequirement> {
    ...
    let task_tool = Expr::Value(ToolRequirement::Tool {
        ...
        id: froq_tool_runtime::Tool::id(&TaskTool).as_str().to_string(),
        if_params: None,
    });
...
impl TaskOutputTool {
    async fn run_single_task(
    ...
    ) -> Result<TaskOutputOutput, froq_tool_runtime::ToolError> {
        let contract_version = ctx
            .extensions
            .get::<froq_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
        ...
        let waits = froq_tool_types::task_output_waits(timeout_ms);
...
pub(crate) use froq_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&froq_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            froq_tool_types::build_task_output_description(&froq_tool_types::TaskOutputToolNaming {
...
/// [`froq_tool_types::build_task_output_description`] builder so the CLI and
...
impl froq_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    async fn multi_task_ids_poll_returns_multi_result_mode_poll() {
        let resources = resources_with_terminal(None);
        let tool = TaskOutputTool;
        let out = froq_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
            TaskOutputToolInput {
