...
use froq_tool_types::{TaskOutputOutput, TaskOutputToolInput};
...
impl froq_tool_runtime::Tool for GetTerminalCommandOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("get_terminal_command_output").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        froq_tool_runtime::Tool::run(&TaskOutputTool, ctx, input).await
    }
}
