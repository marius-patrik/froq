...
//! (in `froq-tools`) and the subagent coordinator (in `froq-shell`).
//!
//! ## Resource types
//!
...
use froq_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
/// Re-export of [`froq_tool_types::is_not_sentinel`] for existing call sites.
pub use froq_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `froq-shell`'s subagent coordinator.
...
/// the cross-crate drain loop in `froq-shell` relies on
...
pub type MonitorEventBuffer = froq_interjection_core::EventQueue<MonitorEventNotification>;
...
/// froq-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in froq-chat-state (via `RunningSubagentSummary`)
...
///
/// Set by froq-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by froq-shell at turn start.
