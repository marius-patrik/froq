...
use regex::Regex;
use froq_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<froq_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| froq_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(froq_tool_protocol::ToolScope::Write),
        streaming: Some(froq_tool_protocol::StreamingSpec {
...
pub struct BashParams {
    ...
    /// grok-build opts up to 10h via froq-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl froq_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<froq_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => froq_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production grok-build sets it to 10h in froq-shell's
...
/// [`froq_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = froq_config::shell::chain_separator();
...
impl froq_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: froq_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> froq_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield froq_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield froq_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield froq_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, froq_tool_runtime::ToolError> {
        ...
        let ampersand = froq_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(froq_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background && !background_enabled {
            return Err(froq_tool_runtime::ToolError::invalid_arguments(
...
mod tests {
    ...
    fn read_chunk_progress(p: &froq_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            froq_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let froq_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = froq_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, froq_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                froq_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn foreground_command_success() {
        ...
        let result = froq_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = froq_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            froq_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = froq_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(froq_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod versioned_background_check {
        ...
        ) -> froq_tool_runtime::ToolCallContext {
            let mut ctx =
                froq_tool_runtime::ToolCallContext::new(froq_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(froq_tool_runtime::BehaviorVersion(
