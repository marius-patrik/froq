...
pub struct ImageGenClient {
    ...
    /// [`froq_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| froq_IMAGINE_MODEL.to_owned());
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`froq_IMAGINE_MODEL`]). Driven by the remote
...
impl ImageGenConfig {
    ...
    /// default quality model ([`froq_IMAGINE_MODEL`]) is used.
...
impl froq_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(froq_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
