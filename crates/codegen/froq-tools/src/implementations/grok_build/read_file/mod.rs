...
/// `PlainText` / `Append`.
static READ_FILE_CAPABILITIES: LazyLock<froq_tool_protocol::ToolCapabilities> =
...
async fn handle_pptx(
    file_bytes: Vec<u8>,
    path: &std::path::Path,
) -> Result<ReadFileOutput, froq_tool_runtime::ToolError> {
...
impl froq_tool_runtime::Tool for ReadFileTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;
    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("read_file").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn execute(
        ...
        input: ReadFileInput,
    ) -> froq_tool_runtime::ToolStream<ReadFileOutput> {
        ...
        let Some(spec) = admitted_spec else {
            ...
            return Box::pin(async_stream::stream! {
                yield froq_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input)
                . await);
            });
        };
...
if let Some(p) =
            froq_tool_runtime::stream_chunk(spec, & content[..window_end], window_end
            as u64, & mut last_total, false,) { yield
            froq_tool_runtime::ToolStreamItem::Progress(p); } window_start =
            window_end; } } yield
            froq_tool_runtime::ToolStreamItem::Terminal(Ok(output)); } Err(e) => yield
...
mod tests {
    ...
    async fn read_file_basic() {
        ...
        let result = froq_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
            .await
            .unwrap();
        match result {
    ...
    async fn legacy_read_file_not_found_returns_exact_historical_message() {
        ...
        let result = froq_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        let expected = format!(
            "Failed to read file: {}",
    ...
    async fn current_read_file_not_found_returns_structured_not_found() {
        ...
        let result = froq_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap();
        match result {
            ReadFileOutput::FileNotFound(msg) => {
...
";
    ...
    async fn run_read_file_on(filename: &str, content: &[u8]) -> ReadFileOutput {
        ...
        froq_tool_runtime::Tool::run(&ReadFileTool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap()
    }
    #[tokio::test]
    ...
    async fn execute_collect(
        ...
        let mut stream = froq_tool_runtime::Tool::execute(&ReadFileTool, ctx, input).await;
