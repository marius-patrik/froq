...
//!
//! - **`froq-tools`** — tool blocks on a oneshot, formats the result.
//! - **`froq-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`froq-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `froq-tools`.
