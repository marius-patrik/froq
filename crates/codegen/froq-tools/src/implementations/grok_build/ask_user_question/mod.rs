...
/// session-owned coordinator (in froq-shell), which performs an ACP
...
impl AskUserQuestionTool {
    ...
    ) -> Result<AskUserQuestionOutput, froq_tool_runtime::ToolError> {
...
impl froq_tool_runtime::Tool for AskUserQuestionTool {
    type Args = AskUserQuestionInput;
    type Output = AskUserQuestionOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("ask_user_question").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        if sender.0.send(request).is_err() {
            return Err(froq_tool_runtime::ToolError::execution(
                froq_tool_protocol::ToolId::new("ask_user_question").expect("valid"),
        ...
        match result {
            ...
            Err(UserQuestionError::TransportError(msg)) => {
                Err(froq_tool_runtime::ToolError::execution(
                    froq_tool_protocol::ToolId::new("ask_user_question").expect("valid"),
...
mod tests {
    ...
    fn tool_is_read_only() {
        assert!(froq_tool_runtime::Tool::capabilities(&AskUserQuestionTool).is_read_only);
    }
    ...
    async fn fallback_ask_single_question() {
        ...
        let result =
            froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "test-call"), input)
                .await
                .unwrap();
    ...
    async fn fallback_sends_notification() {
        ...
        froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "call-q"), input)
            .await
            .unwrap();
    ...
    async fn blocking_round_trip_accepted() {
        ...
        let handle = tokio::spawn({
            ...
            async move {
                froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-1"), input)
                    .await
            }
        });
    ...
    async fn blocking_round_trip_cancelled() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-4"), input)
                    .await
            }
        });
    ...
    async fn answer_before_timeout_still_succeeds() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-ok"), input)
                    .await
            }
        });
    ...
    async fn short_params_timeout_fires_with_cancel_text() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-short"), input)
                    .await
            }
        });
    ...
    async fn channel_drop_returns_error() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-5"), input)
                    .await
            }
        });
    ...
    async fn transport_error_not_cancel() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                froq_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-6"), input)
                    .await
            }
        });
