...
pub use froq_tools_api::slash_commands::WORKFLOW_TOOL_NAME;
...
impl froq_tool_runtime::ToolOutput for WorkflowToolOutput {}
...
impl froq_tool_runtime::Tool for WorkflowTool {
    type Args = WorkflowToolInput;
    type Output = WorkflowToolOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new(WORKFLOW_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
    ...
    ) -> Result<WorkflowToolOutput, froq_tool_runtime::ToolError> {
        ...
        match ack_rx.await {
            ...
            Ok(WorkflowLaunchAck::Rejected { code, detail }) => {
                Err(froq_tool_runtime::ToolError::custom(code, detail))
            }
            Err(_) => Err(froq_tool_runtime::ToolError::custom(
