...
use froq_tool_types::{KillTaskOutput, KillTaskToolInput};
...
impl froq_tool_runtime::Tool for KillTerminalCommandTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("kill_terminal_command").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        froq_tool_runtime::Tool::run(&KillTaskTool, ctx, input).await
    }
}
