...
use froq_tool_types::{KillTaskOutput, KillTaskResult, KillTaskToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for KillTaskTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            froq_tool_types::build_kill_task_description(&froq_tool_types::KillTaskToolNaming {
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        let task_tool = Expr::Value(ToolRequirement::Tool {
            ...
            id: froq_tool_runtime::Tool::id(&TaskTool).to_string(),
            if_params: None,
        });
        let mut arms =
...
/// [`froq_tool_types::build_kill_task_description`] builder so the CLI and
...
impl froq_tool_runtime::Tool for KillTaskTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("kill_task").expect("valid tool id")
    }
    ...
    ) -> Result<KillTaskOutput, froq_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn make_ctx_with_version(
        ...
        ctx.extensions
            .insert(froq_tool_runtime::BehaviorVersion(version.to_owned()));
        ctx
    }
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(froq_tool_runtime::Tool::id(&tool).as_str(), "kill_task");
