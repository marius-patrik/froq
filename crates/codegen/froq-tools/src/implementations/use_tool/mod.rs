...
async fn dispatch_local_mcp(
    ...
    let tool_id = froq_tool_protocol::ToolId::new(tool_name).map_err(|_| {
        froq_tool_runtime::ToolError::invalid_arguments(format!("invalid tool name: '{tool_name}'"))
    })?;
...
fn is_local_tool_id_rejection(err: &froq_tool_runtime::ToolError, tool_name: &str) -> bool {
    err.kind == froq_tool_runtime::ToolErrorKind::InvalidArguments
    ...
    if let Some(source) = gateway_source {
            ...
            && let Some(dispatch) = dispatch.clone()
        {
            match dispatch_local_mcp(dispatch, tool_name, tool_input.clone(), ctx.clone()).await {
                ...
                Err(err)
                    if err.kind != froq_tool_runtime::ToolErrorKind::NotFound
...
impl froq_tool_runtime::Tool for UseTool {
    type Args = UseToolInput;
    type Output = ToolOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("use_tool").expect("valid tool id")
    }
...
mod tests {
    ...
    impl froq_tool_runtime::ToolDispatch for MockToolDispatch {
        async fn call(
            &self,
            ...
            froq_tool_runtime::terminal_only(Ok(froq_tool_runtime::TypedToolOutput::from_value(
                tool_id, value,
            )))
        }
    }
    ...
    #[async_trait::async_trait]
    impl froq_tool_runtime::ToolDispatch for CapturingDispatch {
        async fn call(
            &self,
            tool_id: froq_tool_protocol::ToolId,
            ...
            if !matches!(
                ...
                return froq_tool_runtime::terminal_only(Err(
                    froq_tool_runtime::ToolError::not_found(tool_id, "Tool not found"),
                ));
            }
    ...
    fn ctx_capturing() -> (froq_tool_runtime::ToolCallContext, SharedArgs) {
    ...
    #[async_trait::async_trait]
    impl froq_tool_runtime::ToolDispatch for NotFoundDispatch {
        async fn call(
            &self,
            tool_id: froq_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            froq_tool_runtime::terminal_only(Err(froq_tool_runtime::ToolError::not_found(
                tool_id,
                "Tool not found",
            )))
        }
    ...
    #[async_trait::async_trait]
    impl froq_tool_runtime::ToolDispatch for InvalidArgumentsDispatch {
        async fn call(
            &self,
            _tool_id: froq_tool_protocol::ToolId,
            _args: serde_json::Value,
            _ctx: froq_tool_runtime::ToolCallContext,
        ) -> froq_tool_runtime::ToolStream<froq_tool_runtime::TypedToolOutput> {
            froq_tool_runtime::terminal_only(Err(froq_tool_runtime::ToolError::invalid_arguments(
    ...
    #[async_trait::async_trait]
    impl froq_tool_runtime::ToolDispatch for ErrorToolDispatch {
        async fn call(
            &self,
            _tool_id: froq_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            let tid = froq_tool_protocol::ToolId::new(&self.error)
    ...
    fn new_ctx() -> froq_tool_runtime::ToolCallContext {
        let call_id = froq_tool_protocol::ToolCallId::new_v7();
        froq_tool_runtime::ToolCallContext::new(call_id)
    }

    fn ctx_with_dispatch(
        dispatch: impl froq_tool_runtime::ToolDispatch + 'static,
    ...
    async fn rejects_builtin_tool_names() {
        ...
        assert_eq!(err.kind, froq_tool_runtime::ToolErrorKind::InvalidArguments);
    ...
    fn ctx_capturing_with_resources(
        resources: crate::types::resources::SharedResources,
    ) -> (froq_tool_runtime::ToolCallContext, SharedArgs) {
        let args: SharedArgs = Arc::new(std::sync::Mutex::new(None));
        let mut ctx = new_ctx();
        ctx.extensions
