...
impl froq_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = froq_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(froq_tool_runtime::Cwd(cwd));
        let mut result = froq_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                froq_tool_runtime::ToolError::execution(
                    froq_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
        ...
        if matches!(output_mode, OutputMode::Content) && result.exit_code == 0 {
            ...
            let (fs, scheme) = {
                ...
                let scheme = params
                    .0
                    .build_scheme()
                    .map_err(froq_tool_runtime::ToolError::invalid_arguments)?;
                (fs, scheme)
            };
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(froq_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");
        assert_eq!(ToolMetadata::kind(&tool), ToolKind::Search);
        assert!(froq_tool_runtime::Tool::capabilities(&tool).is_read_only);
        assert!(matches!(
