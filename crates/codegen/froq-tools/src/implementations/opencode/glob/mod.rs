...
impl froq_tool_runtime::ToolOutput for GlobOutput {}
...
impl froq_tool_runtime::Tool for GlobTool {
    type Args = GlobInput;
    type Output = GlobOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("glob").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: GlobInput,
    ) -> Result<GlobOutput, froq_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = GlobTool;
        assert_eq!(froq_tool_runtime::Tool::id(&tool).as_str(), "glob");
    ...
    async fn gitignore_respected() {
        ...
        froq_test_utils::git::ensure_hermetic_git_on_path();
