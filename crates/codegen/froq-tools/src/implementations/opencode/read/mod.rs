...
impl froq_tool_runtime::Tool for ReadTool {
    type Args = ReadInput;
    type Output = ReadFileOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("read").expect("valid tool id")
    }
...
mod tests {
    ...
    async fn runtime_footer_tracks_renamed_offset() {
        ...
        let result = froq_tool_runtime::Tool::run(&ReadTool, ctx, input)
            .await
            .unwrap();
        match result {
