...
impl froq_tool_runtime::Tool for CodexListDirTool {
    type Args = CodexListDirInput;
    type Output = ListDirOutput;

    fn id(&self) -> froq_tool_protocol::ToolId {
        froq_tool_protocol::ToolId::new("list_dir").expect("valid tool id")
    }
    ...
    ) -> Result<ListDirOutput, froq_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn tool_lists_directory() {
        ...
        let ctx = froq_tool_runtime::ToolCallContext::default();
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = froq_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {
