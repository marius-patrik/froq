...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party xAI OAuth (`is_froq_auth()`:
    // AuthMode::Oidc + xAI issuer); the harness's froq_API_KEY is ApiKey mode
    ...
    //
    // Accounting (froq-file-utils upload queue):
