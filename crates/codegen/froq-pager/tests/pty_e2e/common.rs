...
pub(crate) use froq_pager_pty_harness::{
...
// `froq_pager_pty_harness::flows` (re-exported above).
...
/// the harness's default `froq_API_KEY` (ApiKey/BYOK mode, no auth.json entry)
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".grok")
        .join(froq_workspace::trust::TRUST_FILE_NAME);
    let store = froq_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&froq_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `froq_pager_pty_harness::flows`
// (re-exported above).
...
// unit tests in froq-hooks::discovery::tests. The PTY E2E test requires
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = froq_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
pub(crate) use froq_pager_pty_harness::host_clipboard::{
...
pub(crate) use froq_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
