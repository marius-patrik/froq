...
fn grok_home_override_path_helpers() {
    ...
    assert_eq!(
        froq_pager::util::pager_toml_path(),
        grok_home.join("pager.toml")
    );
    assert_eq!(
        froq_pager::util::display_grok_home_prefix(),
        "$GROK_HOME"
    );
    assert_eq!(
        froq_pager::util::display_user_grok_path("config.toml"),
        "$GROK_HOME/config.toml"
    );

    let memory_path = grok_home.join("memory/MEMORY.md");
    assert_eq!(
        froq_pager::util::abbreviate_path(&memory_path.display().to_string()),
    ...
    assert_eq!(
        froq_pager::clipboard::display_copy_path(&grok_home.join("last-copy.txt")),
        "$GROK_HOME/last-copy.txt"
    );

    assert!(froq_pager::util::is_under_user_grok_home(&memory_path));
    assert!(!froq_pager::util::is_under_user_grok_home(
