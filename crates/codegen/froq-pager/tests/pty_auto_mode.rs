...
//!
//! Uses `froq-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//!
//! Run with:
//! `cargo test -p froq-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use froq_pager_pty_harness::{PtyHarness, pager_binary};
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    let mut env = vec![
        ...
        ("TMUX".into(), "".into()),
        // Do not set froq_API_KEY — prefer OIDC entry in auth.json (pty_e2e pattern).
    ];
    ...
    // layer below requirements; "1"/"0" parse to on/off (froq_config::
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (froq-pager-pty-harness)");
