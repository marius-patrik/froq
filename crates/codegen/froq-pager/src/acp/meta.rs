...
pub struct NotificationMeta {
    ...
    /// (`"{sessionId}-{counter}"`, see `froq-shell util::event_id`). The
...
/// replayed notifications (`_meta.isReplay`, stamped by froq-shell's
...
pub mod user_prompt_meta {
    ...
    /// See [`froq_prompt_queue::COMBINED_DISPLAY_TEXTS_META`].
    pub const COMBINED_DISPLAY_TEXTS: &str = froq_prompt_queue::COMBINED_DISPLAY_TEXTS_META;
}
...
pub mod user_message_chunk_meta {
    ...
    /// ([`froq_shell::session::PromptOrigin::hide_user_echo_from_scrollback`]).
