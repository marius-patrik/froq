...
pub async fn spawn_grok_shell(
    ...
    memory_config: Option<froq_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    let (agent_config, models_manager) =
        froq_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        froq_agent::prompt::workspace_user::optional_workspace_user_dir();
                    froq_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    froq_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    froq_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
