...
use froq_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    // When available, add field: startup_announcements: Option<Vec<froq_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<froq_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<froq_shell::util::config::RemoteSettings>,
...
pub async fn connect_via_leader(
    ...
    froq_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = froq_shell::util::grok_home::grok_home().join("config.toml");
...
mod tests {
    ...
    /// (`agent::auth_method::tests::enterprise_byok_first_method_is_froq_api_key`
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use froq_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    #[test]
    fn startup_auth_froq_api_key_not_first_still_requires_login() {
        use froq_shell::agent::auth_method::{GROK_COM_METHOD_ID, froq_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(GROK_COM_METHOD_ID, "Grok", None),
            make_auth_method(froq_API_KEY_METHOD_ID, "xai.api_key", None),
        ];
