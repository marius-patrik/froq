...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: froq_voice::STT_LANGUAGE_DEFAULT.to_string(),
        }
    }
}
...
///
/// Delegates to [`froq_voice::canonicalize_stt_language`] so the pager and
...
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    froq_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = froq_shell::models::default_model();
...
mod tests {
            ...
            match (meta.key, &meta.kind) {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in froq-tools"
                    );
                }
                ...
                ("fork_secondary_model", SettingKind::DynamicEnum { default, .. }) => {
                    ...
                    assert_eq!(
                        ...
                        froq_shell::models::default_model(),
    ...
    /// Spot-check the delegation to `froq_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = froq_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = froq_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match froq_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: froq_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
