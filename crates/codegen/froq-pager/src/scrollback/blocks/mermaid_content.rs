...
//! the generic [`CodeBlockSpan`](froq_markdown::CodeBlockSpan) API) and
...
use froq_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](froq_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](froq_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a froq_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
/// dark. The render worker maps this to `froq_mermaid::MermaidTheme`; it
