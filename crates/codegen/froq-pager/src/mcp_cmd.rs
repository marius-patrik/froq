...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = froq_shell::util::config::load_mcp_server_configs_with_project(&cwd);
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    froq_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => froq_shell::util::config::user_config_path(),
        McpScope::Project => {
            froq_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn select_remove_site(
    ...
    use froq_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    froq_shell::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        froq_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = froq_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        froq_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = froq_shell::util::config::user_config_path();
