...
use froq_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use froq_shell::agent::auth_method::AuthMethodKind;
use froq_shell::agent::config::Config as AgentConfig;
use froq_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use froq_shell::util::config as cli_config;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<froq_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use froq_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<froq_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = froq_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut froq_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    froq_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        ...
        "Not signed in. To authenticate without a browser, run:\n  \
         ...
         Alternatively, set the froq_API_KEY environment variable \
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !froq_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !froq_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &froq_tools::types::compat::CompatConfig::default());
...
pub async fn run_single_turn(
    ...
    froq_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = froq_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = froq_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        froq_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    if track_active {
        let _ = froq_shell::active_sessions::register(
            froq_shell::active_sessions::ActiveSession {
    ...
    if track_active {
        ...
        let _ = froq_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = froq_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    notif: &froq_acp_lib::AcpArgsBox<acp::ExtNotification>,
    format: OutputFormat,
) -> ExtEvent {
    ...
    let Ok(froq_notif) = serde_json::from_str::<XaiNotif>(notif.request.params.get()) else {
        return ExtEvent::None;
    };

    match froq_notif.update {
...
mod tests {
    ...
    use super::*;
    use froq_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            froq_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> froq_acp_lib::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = froq_acp_lib::AcpArgs {
