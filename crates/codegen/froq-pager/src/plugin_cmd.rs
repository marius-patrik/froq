...
//!
//! Business logic lives in `froq_shell::plugin` (shared orchestration)
//! and lower crates (`froq-agent`, `froq-plugin-marketplace`). This
...
use froq_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use froq_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use froq_plugin_marketplace::SourceKind;
use froq_plugin_marketplace::git::SourceCacheLease;
use froq_shell::plugin::{self, RepoUpdateOutcome, UninstallError};
...
fn installed_plugins(
    repos: &[(
        &str,
        &froq_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = froq_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = froq_plugin_marketplace::load_sources(&config);
    sources.extend(froq_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in froq_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                froq_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &froq_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = froq_plugin_marketplace::git::default_cache_root();
            froq_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> froq_telemetry::events::InstallKind {
    if is_git {
        froq_telemetry::events::InstallKind::Git
    } else {
        froq_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: froq_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    froq_telemetry::session_ctx::log_event(froq_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = froq_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use froq_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &froq_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            froq_telemetry::session_ctx::log_event(
                froq_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
...
fn cmd_enable(name: &str) -> Result<()> {
    ...
    if let Err(e) = froq_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    froq_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = froq_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    froq_shell::config::add_disabled_plugin(name)
...
fn marketplace_add(
    ...
    use froq_shell::plugin::MarketplaceAddInput;
...
fn find_removal_source<'a>(
    sources: &'a [froq_plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a froq_plugin_marketplace::MarketplaceSource, String> {
    ...
    let local_input = match plugin::classify_marketplace_add_input(input, cwd) {
        froq_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &froq_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match froq_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use froq_plugin_marketplace::MarketplaceSource;
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = froq_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = froq_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
