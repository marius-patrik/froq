...
pub fn apply_voice_probe(report: &mut DiagnosticReport, emit_missing_issue: bool) {
    ...
    match froq_voice::input_device_info() {
        Ok(device) => {
        ...
        Err(err) => {
            let error = match err {
                froq_voice::VoiceError::Config(message) => message,
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(froq_sandbox::sandbox_profile_conflicts(workspace))
}
