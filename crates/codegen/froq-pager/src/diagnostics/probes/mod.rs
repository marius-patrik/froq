...
pub fn collect_startup_tui<'a>(
    ...
    let native_tool = startup_native_tool(is_wayland, || {
        froq_shell::util::clipboard::native_tool_name()
    });
    collect_common(
        terminal,
        runtime,
...
pub fn collect_doctor_tui<'a>(
    ...
    let is_wayland = crate::host::DisplayServer::current() == crate::host::DisplayServer::Wayland;
    let native_tool = froq_shell::util::clipboard::native_tool_name();
    DoctorProbeSnapshot {
        ...
        container_no_display: froq_shell::util::clipboard::is_containerized_without_display(),
...
pub fn collect_standalone<'a>(terminal: &'a TerminalContext) -> StandaloneDiagnosticSnapshot<'a> {
    ...
    let container_no_display = froq_shell::util::clipboard::is_containerized_without_display();
...
fn standalone_data_control(is_wayland: bool) -> TmuxProbeResult<bool> {
    ...
    match froq_shell::util::clipboard::probe_wayland_data_control() {
        froq_shell::util::clipboard::WaylandDataControlProbe::Available(value) => {
            TmuxProbeResult::Available(value)
        }
        froq_shell::util::clipboard::WaylandDataControlProbe::Unavailable => {
            TmuxProbeResult::Unavailable
        }
        froq_shell::util::clipboard::WaylandDataControlProbe::Error(error) => {
...
fn collect_common<'a>(
    ...
    let data_control =
        is_wayland && froq_shell::util::clipboard::wayland_data_control_supported();
    ProbeSnapshot {
        terminal,
