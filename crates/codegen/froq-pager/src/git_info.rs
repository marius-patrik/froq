...
pub fn worktree_label_index() -> std::collections::HashMap<PathBuf, String> {
    ...
    let Ok(db) = froq_fast_worktree::db::WorktreeDb::open_default() else {
        return map;
    };
    ...
    for rec in records {
        let Some(label) = rec
            .metadata
            .as_ref()
            .and_then(|m| m.get(froq_shell::session::worktree::META_KEY_LABEL))
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
        else {
            continue;
...
fn lookup_worktree_label(cwd: &Path) -> Option<String> {
    let db = froq_fast_worktree::db::WorktreeDb::open_default().ok()?;
    ...
    for ancestor in cwd.ancestors() {
        if let Ok(Some(record)) = db.get(&ancestor.to_string_lossy()) {
            let label = record
                .metadata
                .as_ref()?
                .get(froq_shell::session::worktree::META_KEY_LABEL)?
                .as_str()
                .filter(|s| !s.is_empty())
                .map(String::from);
            return label;
...
mod tests {
    ...
    fn cwd_git_info_lazy_non_repo_is_none() {
        let p = Path::new("/nonexistent-froq-git-info-lazy-test-zzz");
