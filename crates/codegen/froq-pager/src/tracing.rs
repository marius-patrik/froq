...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "froq_shell=info,froq_pager=trace,froq_tools=info,froq_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = froq_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = froq_telemetry::instrumentation::layer();
    let sampling_log_layer = froq_telemetry::sampling_log::layer();
    let hooks_log_layer = froq_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    froq_telemetry::debug_log::install_firehose(registry, "tui");
