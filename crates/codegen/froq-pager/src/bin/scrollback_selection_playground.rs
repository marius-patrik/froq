...
use froq_pager::scrollback::text_selection::{
...
use froq_pager::scrollback::types::slice_display_cols;
use froq_pager::scrollback::{RenderBlock, ScratchBuffer, ScrollbackPane, ScrollbackState};
...
fn selection_highlight_duration_ms() -> u64 {
    ...
    if froq_pager::appearance::cache::load_keep_text_selection().holds() {
        0
    } else {
        PLAYGROUND_FLASH_MS
    }
...
fn double_click_action_label() -> &'static str {
    if froq_pager::appearance::cache::load_keep_text_selection().selects_word() {
        "word_select"
    } else {
        "toggle_fold"
    }
