...
use froq_shell::session::repo_changes::UploadMethod;
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    froq_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
...
async fn run_upload(
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            froq_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
    ...
    let upload_config = froq_shell::session::repo_changes::TraceExportConfig {
...
async fn upload_with_retries(
    config: &froq_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
    ...
    (|| async {
        tokio::time::timeout(
            UPLOAD_TIMEOUT,
            froq_file_utils::gcs::upload_bytes(config, object_path, archive, "application/gzip"),
        )
        .await
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = froq_shell::auth::ensure_authenticated_or_noninteractive(
