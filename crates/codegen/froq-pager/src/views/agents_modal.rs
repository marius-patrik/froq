...
use froq_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use froq_shell::agent::config::AgentSelectionConfig;
use froq_tools::implementations::skills::discovery::extract_first_paragraph;
...
impl AgentsModalState {
    ...
    #[cfg(test)]
    fn search_viewport(&self, width: usize) -> froq_ratatui_textarea::SingleLineViewport {
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = froq_agent::discovery::discover(cwd);
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match froq_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => froq_config::grok_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".grok").join("personas"),
    }
}
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    froq_shell::config::load_effective_config()
        .ok()
        .and_then(|root| froq_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in froq-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    froq_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        froq_agent::config::PromptMode::Extend => "extend",
        froq_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use froq_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
