...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a froq_announcements::RemoteAnnouncement>,
...
pub(super) fn render_version_badge(
    ...
    let channel = froq_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", froq_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a froq_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a froq_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [froq_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a froq_shell::auth::GateInfo>,
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &froq_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn masked_auth_token_view(input: &str, cursor_byte: usize, width: usize) -> (String, usize) {
    ...
    let buffer =
        froq_ratatui_textarea::EditBuffer::from_parts(masked.display.as_str(), masked.cursor_byte);
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use froq_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = froq_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn long_ann() -> froq_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = froq_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
