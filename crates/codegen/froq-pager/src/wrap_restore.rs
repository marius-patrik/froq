...
//! [`froq_crash_handler::terminal`] (`RESTORE_SEQ`) — pinned mechanically by a
...
/// `froq_crash_handler::terminal::RESTORE_SEQ`'s ordering tests.
...
/// `froq_crash_handler::terminal::RESTORE_SEQ` for every element the two
...
mod tests {
    ...
    fn covers_and_orders_every_crash_handler_restore_seq_element() {
        let elements: Vec<Vec<u8>> = froq_crash_handler::terminal::RESTORE_SEQ
            .split(|&b| b == 0x1b)
