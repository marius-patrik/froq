...
//! mutates process-global env (proxy URLs, `froq_API_KEY`,
...
//!
//! ```bash
//! cargo test -p froq-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
use froq_shell::agent::mvp_agent::MvpAgent;
...
impl PagerLeaderCluster {
    ...
    async fn start() -> Self {
        ...
        let env = vec![
            ...
            crate::test_util::EnvVarGuard::set("GROK_froq_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("froq_API_KEY", "test-key-for-ci"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (froq_version), not
        ...
        // froq-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: froq-shell/tests/test_leader_soak.rs ("Real agent
