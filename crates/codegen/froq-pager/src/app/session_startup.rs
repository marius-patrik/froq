...
pub enum DeferredSessionStartup {
    ...
    ForeignResume {
        tool: froq_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = froq_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = froq_shell::util::grok_home::grok_home().join("sessions");
    let encoded = froq_shell::util::grok_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    froq_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = froq_shell::session::persistence::list_summaries(Some(cwd)).await?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &froq_shell::agent::config::Config,
) -> std::sync::Arc<froq_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(froq_shell::auth::AuthManager::new(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if froq_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = froq_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = froq_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    if let Some((false, source)) =
        froq_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = froq_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use froq_shell::agent::session_registry_client::SessionRegistryClient;
    use froq_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use froq_shell::session::restore::restore_session_with_storage;
    use froq_shell::util::grok_home::grok_home;
    ...
    let storage_client = froq_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: froq_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = froq_shell::util::grok_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = froq_shell::util::grok_home::grok_home()
            .join("sessions")
            .join(&encoded);
