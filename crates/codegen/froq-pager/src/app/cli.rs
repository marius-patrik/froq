...
pub struct AgentArgs {
    ...
    #[arg(long = "froq-api-base-url")]
    pub froq_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
...
pub struct PagerArgs {
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            froq_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
...
impl PagerArgs {
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                froq_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                froq_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<froq_sandbox::ProfileName>().ok()
                    != s.parse::<froq_sandbox::ProfileName>().ok() =>
            {
