...
//! inside [`froq_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `froq-pager __mermaid-render` (see
...
//!    via [`froq_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`froq_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `froq-mermaid`). The parent also rejects obviously-oversized
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, froq_mermaid::MermaidError> {
