...
) -> froq_workspace::foreign_sessions::RecentForeignSession {
    froq_workspace::foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                froq_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(froq_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                froq_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn marketplace_list_loaded_sanitizes_components_at_ingestion() {
    ...
    entry.components = Some(froq_hooks_plugins_types::PluginComponents {
        skills: vec![froq_hooks_plugins_types::ComponentItem {
            name: "evil\u{1b}[31mskill".into(),
            description: Some(format!("\u{7}{}", "d".repeat(300))),
        }],
        ..Default::default()
    });
    let response = froq_hooks_plugins_types::MarketplaceListResponse {
...
fn plugins_action_success_sets_result_notice_and_autoreload_preserves_it() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(froq_hooks_plugins_types::ActionOutcome {
...
fn uninstall_result_notice_is_footer_only_not_row_anchored() {
    ...
    let id = AgentId(0);
    {
        ...
        modal.last_plugins_action = Some(froq_hooks_plugins_types::PluginsAction::Uninstall {
...
fn confirmation_required_builds_plugins_confirmation_with_confirmed_true() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(froq_hooks_plugins_types::ActionOutcome {
                status: froq_hooks_plugins_types::OutcomeStatus::ConfirmationRequired,
    ...
    match &modal.modal_message {
        Some(ModalMessage::Confirmation {
            ...
            assert_eq!(
                action,
                &ConfirmationAction::Plugins(froq_hooks_plugins_types::PluginsAction::Uninstall {
                    plugin_id: "user/ab12/gone".into(),
                    confirmed: true,
                })
            );
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = froq_shell::agent::config::ModelSwitchIncompatibleAgentError {
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = froq_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    let settings = froq_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = froq_shell::util::config::RemoteSettings {
...
fn test_gate() -> froq_shell::auth::GateInfo {
    froq_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(froq_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(froq_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
