...
fn plugin_cta_catalog_loaded_sanitizes_components_at_ingestion() {
    ...
    let response = froq_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![froq_hooks_plugins_types::MarketplaceScanResult {
            source_name: froq_plugin_marketplace::OFFICIAL_SOURCE_NAME.into(),
            source_kind: "git".into(),
            source_url_or_path: froq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
            plugins: vec![entry],
            error: None,
        }],
    };
...
mod cta_e2e {
    ...
    fn figma_candidate() -> froq_hooks_plugins_types::MarketplacePluginEntry {
        let mut entry = cta_entry("figma", "not_installed");
    ...
    fn dismissed_plugin_suppressed_on_next_recompute() {
        ...
        let dismissed = froq_shell::config::dismissed_plugin_ctas_in_file(&config_path);
        let matched =
        ...
        froq_shell::config::add_dismissed_plugin_cta_to_file("figma", &config_path)
