...
fn e2e_compact_auth_failure_holds_prompt_and_resubmits_after_login() {
    ...
    use froq_shell::extensions::notification::{RetryState, SessionUpdate as XaiSessionUpdate};
...
fn auth_complete_preserves_show_resolved_model_when_absent() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::AuthComplete {
            request_seq: 1,
            meta: Some(serde_json::to_value(froq_shell::auth::AuthMeta::default()).unwrap()),
        }),
        &mut app,
    );
