...
fn cta_entry(name: &str, status: &str) -> froq_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: froq_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = froq_shell::util::grok_home::grok_home();
    let encoded = froq_shell::util::grok_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: froq_tools::implementations::grok_build::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    froq_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    froq_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = froq_acp_lib::AcpArgs {
