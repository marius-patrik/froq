...
use super::*;
use froq_shell::sampling::error::is_free_usage_exhausted_error;
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: froq_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    let mut app = test_app_with_agent();
    let before = agent_scrollback_len(&app);
    let usage = froq_shell::extensions::notification::PromptUsage {
        totals: froq_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        froq_shell::extensions::notification::PromptUsage {
            totals: froq_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: froq_telemetry::events::SuperGrokUpsell::FreeUsagePaywall,
            }
        )
    ));
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use froq_shell::extensions::notification::{RetryState, SessionUpdate};
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: froq_telemetry::events::SuperGrokUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
