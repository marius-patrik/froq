...
///
/// Delegates to the shared implementation in `froq_tools::reminders`.
...
fn format_cron_prompt(prompt: &str, task_id: &str, human_schedule: &str) -> String {
    froq_tools::reminders::format_scheduled_task_prompt(prompt, task_id, human_schedule)
}
...
pub(super) fn maybe_drain_queue(agent: &mut AgentView) -> QueueDrain {
    ...
    match queued.kind {
        QueueEntryKind::Prompt => {
            ...
            let multi = froq_prompt_queue::is_combined(&queued.combined_texts);
            ...
            let effects = if let Some(mut blocks) = queued.wire_blocks {
                ...
                if let Some(acp::ContentBlock::Text(tb)) = blocks.first_mut() {
                    ...
                    froq_prompt_queue::stamp_combined_display_texts(map, &combined_segs);
                } else {
                    tracing::debug!(
...
fn paint_or_reuse_combined_user_bubbles(
    ...
    let joined = froq_prompt_queue::join_texts(segments.iter().map(String::as_str));
    ...
    let page_flip_entry = if let Some(segments) = multi_segments {
        ...
        if rewindable {
            let restore = text.clone().unwrap_or_else(|| {
                froq_prompt_queue::join_texts(segments.iter().map(String::as_str))
            });
...
mod tests {
    ...
    fn drain_scroll_honors_page_flip_setting() {
        ...
        crate::appearance::cache::set_page_flip_on_send(
            froq_shell::agent::config::UiConfig::PAGE_FLIP_ON_SEND_DEFAULT,
        );
    }
