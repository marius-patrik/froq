...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = froq_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == froq_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = froq_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut froq_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = froq_shell::util::config::resolve_contextual_hints(
...
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = froq_shell::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();
