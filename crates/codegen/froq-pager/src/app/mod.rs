...
pub use froq_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
pub(crate) fn voice_mode_requirement_pin() -> Option<bool> {
    froq_config::load_merged_requirements().and_then(|req| {
        req.get("features")
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use froq_shell::agent::config::{BoolFlag, ConfigSource};
...
///
/// Uses [`froq_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `froq-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`froq-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&froq_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<froq_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<froq_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
pub async fn run(
    ...
    froq_tty_utils::redirect_native_stderr();
    ...
    let grok_com_config = match froq_shell::agent::config::Config::new_from_toml_cfg(
        &raw_config,
    ) {
    ...
    let refreshed_auth = froq_shell::auth::try_ensure_fresh_auth(&grok_com_config).await;
    let early_prefetch =
        froq_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    froq_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(froq_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = froq_shell::session::persistence::list_summaries(None).await?;
    ...
    cancel.cancel();
    froq_tty_utils::global_process_scope().kill_all();
...
fn disable_mouse_paste_raw() {
    froq_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(froq_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    froq_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        froq_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(froq_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                froq_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = froq_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let term = froq_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        froq_crash_handler::disable_terminal_escape_restore();
    })
}
...
mod tests {
    ...
    fn restore_runs_teardown_even_when_writer_failed() {
        ...
        let terminal = froq_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = froq_shell::util::config::RemoteSettings {
