#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn goal_updated_ignores_unknown_json_fields_via_serde() {
        ...
        let msg = AcpClientMessage::ExtNotification(froq_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
    ...
    #[test]
    fn workflow_updates_bypass_global_froq_highwater_and_use_run_revision() {
        let mut app = make_app_with_agent("sess-A");
        let id = AgentId(0);
        app.agents.get_mut(&id).unwrap().last_applied_froq_event_seq = Some(100);
        ...
        assert_eq!(app.agents[&id].last_applied_froq_event_seq, Some(100));
