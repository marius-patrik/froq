#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn reconnect_reload_failure_restores_pre_outage_transcript() {
        ...
        let id = AgentId(0);
        {
            ...
            agent.last_applied_froq_event_seq = Some(5);
            agent.begin_session_reload(1);
        }
        ...
        let _ = handle_ext_notification(&froq_model_switch_notif("sess-rc", "sess-rc-30"), &mut app);
        assert_eq!(
        ...
        assert_eq!(
            app.agents[&id].last_applied_froq_event_seq,
            Some(30),
        ...
        assert_eq!(
            agent.last_applied_froq_event_seq,
            Some(5),
    ...
    #[test]
    fn froq_session_update_dedup_drops_already_applied_event() {
        let mut app = make_app_with_agent("sess-xdup");
        let id = AgentId(0);

        assert!(handle_ext_notification(
            &froq_model_switch_notif("sess-xdup", "sess-xdup-10"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 1);
        assert_eq!(app.agents[&id].last_applied_froq_event_seq, Some(10));
    ...
    #[test]
    fn unhandled_froq_update_does_not_advance_cursor_or_highwater() {
        let mut app = make_app_with_agent("sess-ig");
        let id = AgentId(0);

        assert!(!handle_ext_notification(
            &froq_unhandled_notif("sess-ig", "sess-ig-7"),
            &mut app
        ));
        assert_eq!(
        ...
        assert_eq!(app.agents[&id].last_applied_froq_event_seq, Some(8));
    }
    ...
    #[test]
    fn direct_froq_event_does_not_shoot_down_delayed_acp_chunk() {
        let mut app = make_app_with_agent("sess-split");
        let id = AgentId(0);

        // Direct xAI emission stamped N+1 arrives first.
        assert!(handle_ext_notification(
            &froq_model_switch_notif("sess-split", "sess-split-21"),
            &mut app
        ));
        assert_eq!(app.agents[&id].last_applied_froq_event_seq, Some(21));
    ...
    #[test]
    fn froq_session_update_replay_gating_and_cursor() {
        ...
        let replay_meta = serde_json::json!({ "isReplay": true, "eventId": "sess-froq-7" });
        assert!(!handle_ext_notification(
