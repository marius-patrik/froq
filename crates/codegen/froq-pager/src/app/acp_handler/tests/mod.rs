...
use froq_shell::extensions::notification::RetryState;
use froq_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> froq_shell::extensions::notification::ImageCompressedEntry {
    froq_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_exit_plan_ext(
    ...
    froq_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<froq_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(froq_acp_lib::AcpArgs {
...
pub(super) fn froq_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
...
pub(super) fn froq_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
...
pub(super) fn froq_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
...
pub(super) fn froq_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
pub(super) fn froq_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
...
pub(super) fn froq_hook_execution_notif_with_runs(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<froq_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn froq_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
    froq_hook_execution_notif_for_prompt(session_id, event_name, None, is_replay)
}
...
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(froq_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(froq_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = froq_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: froq_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use froq_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = froq_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
