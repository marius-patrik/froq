...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        froq_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = froq_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = froq_shell::config::load_effective_config().ok();
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(froq_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(froq_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        froq_shell::config::load_merged_requirements(),
        froq_shell::config::load_from_disk().ok(),
        froq_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = froq_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = froq_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = froq_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use froq_shell::util::config::resolve_tips;
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = froq_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        froq_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<froq_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[froq_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = froq_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = froq_announcements::filter_expired(merged);
    ...
    if froq_announcements::prune_hidden_announcement_ids(
...
pub(super) fn pick_random_announcement(
    announcements: &[froq_announcements::RemoteAnnouncement],
) -> Option<froq_announcements::RemoteAnnouncement> {
...
/// `froq-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `froq-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
