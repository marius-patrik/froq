use super::*;
use froq_shell::sampling::error::format_rate_limited_user_message;
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
        ...
        })
    {
        tracing::debug!(
            ...
            last_applied = agent.last_applied_froq_event_seq,
    ...
    let changed = match session_notif.update {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        froq_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        froq_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        froq_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
                            blocked: true,
        ...
        XaiSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(froq_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        XaiSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if froq_shell::agent::chat_modes::process_chat_mode_enabled() {
    ...
    if let Some(agent) = app.agents.get_mut(&parent_id) {
        if let Some(seq) = meta.event_seq
            ...
            && !is_workflow_update
        {
            agent.last_applied_froq_event_seq = Some(seq);
        }
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(froq_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[froq_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &froq_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                froq_telemetry::session_ctx::log_event(
                    froq_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
            ...
            let is_free_usage = *rate_limited
                && froq_shell::sampling::error::is_free_usage_exhausted_error(reason);
            if is_credit_limit {
    ...
    if is_credit_limit {
        froq_telemetry::session_ctx::log_event(froq_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use froq_tools::types::SessionMode;
