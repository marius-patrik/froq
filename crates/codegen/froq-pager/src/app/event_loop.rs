...
/// [`decide`](froq_workspace::folder_trust::decide) precedence.
///
...
fn seed_trust_state(
    app: &mut AppView,
    remote: Option<&froq_shell::util::config::RemoteSettings>,
) {
    use std::io::IsTerminal;
    use froq_workspace::folder_trust::{
        TrustOutcome, decide, decide_inputs_with_interactive, feature_enabled,
    };
    use froq_workspace::trust::workspace_key;
...
pub(crate) async fn run(
    ...
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<froq_update::auto_update::UpdateAvailable>>,
    >,
    ...
    let launch_auto = froq_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let launch_effective_ui = froq_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(froq_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = froq_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match froq_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.privacy_notice_rollout = froq_config::env_bool("GROK_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
    ...
    app.privacy_banner_acked = froq_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            froq_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = froq_config::env_bool("GROK_PLUGIN_CTA")
    ...
    app.session_picker_grouped = std::env::var("GROK_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            froq_shell::config::load_effective_config()
                .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    froq_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<froq_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == froq_shell::agent::auth_method::froq_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = froq_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = froq_shell::config::load_merged_requirements();
    let user_config = froq_shell::config::load_from_disk().ok();
    let managed_config = froq_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match froq_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = froq_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > GROK_froq_API_BASE_URL env > default).
            let endpoints_base =
                froq_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .froq_api_base_url;
            app.voice_config =
                froq_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    ...
    app.zdr_access_enabled = froq_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        froq_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        froq_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        froq_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use froq_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = froq_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let grok_home = froq_tools::util::grok_home::grok_home();
            app.tip = froq_shell::util::tips::pick_and_advance(&app.tips, &grok_home);
        }
    }

    let hints = froq_shell::util::config::resolve_hints(
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        froq_shell::util::config::clamped_display_permission_mode(
            froq_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        froq_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = froq_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = froq_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<froq_voice::VoiceEvent>>;
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(froq_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
...
pub(crate) fn load_initial_ui_config() -> froq_shell::agent::config::UiConfig {
    use froq_shell::agent::config::UiConfig;
    let Ok(root) = froq_shell::config::load_effective_config() else {
