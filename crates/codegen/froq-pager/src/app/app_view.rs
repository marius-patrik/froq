...
/// [`froq_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<froq_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: froq_shell::util::config::WorktreeHintMode) -> Self {
        use froq_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        froq_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            froq_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`froq_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// (toolset) gate via [`froq_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => froq_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: froq_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`froq_shell::util::config::resolve_hints`];
    ...
    pub active_announcements: Vec<froq_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<froq_announcements::RemoteAnnouncement>,
    ...
    /// empty and inert outside `--minimal`; the `froq-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<froq_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        froq_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `froq_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    pub contextual_hints: froq_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<froq_shell::util::config::ContextualHintsRemote>,
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, froq_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `grok_build_access_gate`. `Some` = blocked.
    pub gate: Option<froq_shell::auth::GateInfo>,
    ...
    pub pending_gate_verification: Option<froq_shell::auth::GateInfo>,
    ...
    pub voice_config: froq_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `froq_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<froq_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<froq_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &froq_shell::util::config::RemoteSettings,
    ) -> Option<froq_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(froq_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &froq_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            froq_telemetry::session_ctx::log_event(
                froq_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: froq_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: froq_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: froq_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<froq_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && froq_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: froq_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(froq_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(froq_voice::VoiceCommand::PttRelease);
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<froq_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    froq_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        froq_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                froq_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::SuperGrokUpsellShown {
                                    source:
                                        froq_telemetry::events::SuperGrokUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`froq_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use froq_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = froq_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                froq_telemetry::session_ctx::log_event(
                    froq_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: froq_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            froq_telemetry::session_ctx::log_event(froq_telemetry::events::ContextualTip {
                tip: froq_telemetry::events::ContextualTipKind::ImageInput,
                action: froq_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = froq_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![froq_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![froq_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: froq_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_disables_billing_surface_for_team_users() {
        ...
        let meta = froq_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_enables_billing_surface_for_personal_users() {
        ...
        let meta = froq_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_restores_billing_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&froq_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&froq_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(froq_shell::auth::GateInfo {
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = froq_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(froq_workspace::foreign_sessions::RecentForeignSession {
                tool: froq_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn esc_owned_before_agent_covers_app_level_owners() {
        ...
        app.import_claude_modal = Some(
            crate::views::import_claude_modal::ImportClaudeModalState::new(
                froq_shell::claude_import::ImportPlan::default(),
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![froq_workspace::file_system::FuzzyMatchResult {
