...
use super::*;
use froq_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use froq_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use froq_workspace::session::git::RestoreDegree;
...
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<froq_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let froq_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use froq_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            froq_shell::active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use froq_acp_lib::AcpAgentMessage;
    use froq_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "x.ai/marketplace/action" => {
                        ...
                        let req: froq_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: froq_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> froq_shell::session::SessionInfoResponse {
    use froq_shell::session::acp_types::{ContextInfo, SessionInfoData};
    froq_shell::session::SessionInfoResponse {
...
fn format_session_info_session_auth_ignores_api_key_env() {
    ...
    assert!(!text.contains("Also present: froq_API_KEY"), "{text}");
...
fn format_session_info_api_key_without_env() {
    ...
    assert!(!text.contains("froq_API_KEY"), "{text}");
    assert!(
...
fn format_session_info_api_key_auth_notes_console_billing() {
    ...
    assert!(text.contains("Auth method: API key (froq_API_KEY)"), "{text}");
    assert!(
            text.contains("Manage account and credits: console.x.ai"),
            "{text}"
        );
