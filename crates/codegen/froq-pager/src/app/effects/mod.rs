...
use froq_acp_lib::{AcpAgentTx, acp_send};
use actions::{
...
use froq_shell::sampling::error::http_status_from_error;
use froq_shell::session::{ExtMethodResult, SessionInfoResponse};
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = froq_shell::active_sessions::register(froq_shell::active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = froq_tools::types::compat::CompatConfig::default();
            let mcp_servers = froq_shell::util::config::load_mcp_servers(
                &session_cwd,
                &compat,
            );
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = froq_shell::util::config::worktree_type();
                    ...
                    let mcp_servers = froq_shell::util::config::load_mcp_servers(
                        &session_cwd,
                        &froq_tools::types::compat::CompatConfig::default(),
                    );
                    let result = acp_send(
                            acp::NewSessionRequest::new(session_cwd.clone())
                                .mcp_servers(mcp_servers)
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            froq_workspace::foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &grok_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || froq_workspace::foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            use froq_shell::agent::session_registry_client::SessionRegistryClient;
            use froq_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = froq_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = froq_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = froq_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        froq_shell::session::restore::ProgressCallback,
                    > = {
                        use froq_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            let info = froq_shell::session::info::Info {
                                id: acp::SessionId::new(session_id),
                                cwd,
                            };
                            let history_path = froq_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use froq_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    let params = froq_shell::extensions::task::KillTaskRequest {
            ...
            tasks
                .spawn(async move {
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use froq_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            froq_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            froq_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    froq_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = froq_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = froq_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                froq_hooks_plugins_types::HooksListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                froq_hooks_plugins_types::PluginsListResponse,
                            >(inner.clone())
        ...
        Effect::HooksAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = froq_hooks_plugins_types::HooksActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
        ...
        Effect::PluginsAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = froq_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                froq_hooks_plugins_types::MarketplaceListResponse,
                            >(inner.clone())
            ...
            tasks
                .spawn(async move {
                    ...
                    for (name, old, new, source_url, rel_path) in outdated {
                        let action = froq_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path: source_url.clone(),
                            plugin_relative_path: rel_path.clone(),
                        };
                        let req_body = froq_hooks_plugins_types::MarketplaceActionRequest {
                            session_id: session_id.0.to_string(),
                            action,
                        };
            ...
            tasks
                .spawn(async move {
                    let plugin_name = plugin_relative_path
                        ...
                        .to_string();
                    let action = froq_hooks_plugins_types::MarketplaceAction::Install {
                        source_url_or_path,
                        plugin_relative_path,
                    };
                    let req_body = froq_hooks_plugins_types::MarketplaceActionRequest {
        ...
        Effect::ReloadPluginsForCta { agent_id, session_id, plugin_name } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = froq_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action: froq_hooks_plugins_types::PluginsAction::Reload,
                    };
                    let req = acp::ExtRequest::new(
                        "x.ai/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: froq_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use froq_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = froq_shell::agent::auth_method::has_froq_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text } => {
            use froq_shell::session::ClientType;
            use froq_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = froq_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    froq_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            froq_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionTitleFromDisk { agent_id, session_id, cwd } => {
            tasks
                .spawn(async move {
                    ...
                    let path = froq_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    let title = tokio::task::spawn_blocking(move || -> Option<
                            (String, bool),
                        > {
                            let raw = std::fs::read_to_string(path).ok()?;
                            let summary: froq_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent } => {
            ...
            tasks
                .spawn(async move {
                    use froq_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "x.ai/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !froq_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let grok_home = froq_shell::util::grok_home::grok_home();
                            let store = froq_shell::auth::read_auth_json(
                                    &grok_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = froq_shell::auth::GrokComConfig::default()
                                .auth_scope();
                            let auth = froq_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    froq_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            froq_shell::remote::fetch_settings_blocking(
                                &proxy_base,
                                &auth,
                                None,
                            )
...
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<froq_shell::extensions::notification::PromptUsage, String> {
    let request = acp::ExtRequest::new(
        "x.ai/session/usage",
        serde_json::value::to_raw_value(
                &serde_json::json!({
    ...
    let parsed: froq_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
async fn lookup_session_title(session_id: &acp::SessionId) -> Option<String> {
    let summaries = froq_shell::session::persistence::list_summaries(None)
        .await
        .ok()?;
    summaries
        .into_iter()
...
fn format_session_info(
    ...
    let model_display = froq_shell::session::model_display_name(
    ...
    let model_hash_line = if froq_shell::session::should_show_model_fingerprint(
    ...
    let backend_line = info
        ...
        .map(|b| format!("\n  API Backend: {b}"))
        .unwrap_or_default();
    let sandbox_line = froq_sandbox::profile_name()
        .map(|profile| format!("\n  Sandbox: {profile}"))
        .unwrap_or_default();
    ...
    let conversation_line = info
        ...
        .map(|id| format!("\n  Conversation ID: {id}"))
        .unwrap_or_default();
    let version_display = froq_version::display_version(
...
fn format_auth_lines(is_api_key_auth: bool, api_key_env_set: bool) -> String {
    if is_api_key_auth {
        let method = if api_key_env_set {
            "  Auth method: API key (froq_API_KEY)\n"
        } else {
            "  Auth method: API key\n"
        };
        return format!(
