...
pub(crate) struct SessionReload {
    ...
    last_applied_froq_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<froq_hooks_plugins_types::MarketplacePluginEntry>,
...
pub struct AgentView {
    ...
    /// ([`Self::last_applied_froq_event_seq`]) because the two streams are not
    ...
    /// froq-stream sibling of [`Self::last_applied_event_seq`] (see there for
    /// why the highwaters are split). Same drop rule, replay-exempt.
    pub last_applied_froq_event_seq: Option<u64>,
    ...
    pub context_state: Option<froq_shell::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<froq_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(froq_telemetry::events::CreditLimitChoice::PayAsYouGo);
            froq_telemetry::session_ctx::log_event(
                froq_telemetry::events::CreditLimitUpsellClicked {
                    surface: froq_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
                    choice,
                },
            );
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            froq_telemetry::session_ctx::log_event(
                froq_telemetry::events::SuperGrokUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use froq_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        froq_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = froq_tools::util::grok_home::grok_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &froq_tools::implementations::grok_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
