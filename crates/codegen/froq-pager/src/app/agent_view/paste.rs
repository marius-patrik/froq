                ...
                if let Err(msg) = self.prompt.insert_image(pasted) {
                    ...
                    if ctx.source.tip_showing() {
                        froq_telemetry::session_ctx::log_event(
                            froq_telemetry::events::ContextualTip {
...
pub(super) mod paste_key_tests {
    ...
    -> crate::views::question_view::QuestionViewState {
        let question = froq_tools::implementations::grok_build::ask_user_question::Question {
            question: "Pick one?".to_string(),
            options: vec![
                froq_tools::implementations::grok_build::ask_user_question::QuestionOption {
                    label: "A".to_string(),
    ...
    fn agent_completion_inserts_unreadable_file_url_as_path_text() {
        ...
        let completion = agent.complete_clipboard_attachment_paste(
            ctx,
            crate::app::actions::ProbedAttachment::NoRaster,
            Some("file:///definitely/missing/froq-primary-paste.png".to_owned()),
        );
        assert_eq!(
            completion,
            crate::app::actions::ClipboardPasteCompletion::Handled
        ...
        assert_eq!(
            agent.prompt.text(),
            "/definitely/missing/froq-primary-paste.png "
        );
        assert!(agent.prompt.images.is_empty());
    }
    #[test]
