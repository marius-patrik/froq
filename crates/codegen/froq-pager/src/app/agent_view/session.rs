...
impl AgentView {
    ...
    pub(crate) fn bind_session_id(&mut self, session_id: agent_client_protocol::SessionId) {
        if self.session.session_id.as_ref() != Some(&session_id) {
            ...
            self.last_applied_froq_event_seq = None;
    ...
    pub fn new(session: AgentSession, scrollback: ScrollbackState) -> Self {
        ...
        let mut view = Self {
            ...
            last_applied_froq_event_seq: None,
    ...
    pub(crate) fn begin_session_reload(&mut self, generation: u64) {
        ...
        self.session_reload = Some(SessionReload {
            ...
            last_applied_froq_event_seq: self.last_applied_froq_event_seq,
            saw_replay: false,
    ...
    fn apply_reload_outcome(&mut self, reload: SessionReload, success: bool) -> bool {
        ...
        if success && reload.saw_replay {
            ...
            self.last_applied_froq_event_seq = reload.last_applied_froq_event_seq;
            dropped_heavy = true;
        }
    ...
    pub fn apply_full_context_info(&mut self, next: froq_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            Some(snap) => {
                ...
                snap.usage_pct = froq_token_estimation::usage_percentage_u8(used, snap.total);
                snap.free_tokens = froq_token_estimation::free_tokens(snap.total, used);
            }
            None => {
                self.context_state = Some(froq_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
