...
impl AgentView {
    ...
    fn log_extensions_modal_action(
        &self,
        action: &str,
        input_method: froq_telemetry::events::ExtensionsInputMethod,
    ) {
    ...
    fn log_extensions_modal_action_with(
        ...
        if let Some(ref state) = self.extensions_modal {
            froq_telemetry::session_ctx::log_event(
                froq_telemetry::events::ExtensionsModalAction {
    ...
    pub(super) fn handle_extensions_modal_key(
            ...
            })
        {
            self.log_extensions_modal_action(
                "open_connectors",
                froq_telemetry::events::ExtensionsInputMethod::Keyboard,
            );
        ...
        if let Some(ch) = chrome_shortcut_ch {
                ...
                .and_then(|s| crate::views::extensions_modal::resolve_key(s.active_tab, ch))
            {
                self.log_extensions_modal_resolved_action(
                    ch,
                    &action,
                    froq_telemetry::events::ExtensionsInputMethod::Mouse,
                );
    ...
    fn execute_modal_button_action(
        ...
        match action {
            ButtonAction::HooksAction(hooks_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    state.modal_message = None;
                    if matches!(hooks_action, froq_hooks_plugins_types::HooksAction::Reload) {
            ...
            ButtonAction::PluginsAction(plugins_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    ...
                    if matches!(
                        plugins_action,
                        froq_hooks_plugins_types::PluginsAction::Reload
                    ) {
                        // Reload rebuilds the entire plugin registry -- show
                        // tab-level "Loading..." instead of a single-entry badge.
                        state.pending_action = Some("Reloading...".into());
                        ...
                        let label = if matches!(
                            plugins_action,
                            froq_hooks_plugins_types::PluginsAction::Update { .. }
                        ) {
                            "Updating..."
                        } else {
                            "Processing..."
            ...
            ButtonAction::MarketplaceAction(marketplace_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    ...
                    match &marketplace_action {
                        ...
                        froq_hooks_plugins_types::MarketplaceAction::Uninstall { .. } => {
                        ...
                        && let Some(hook) = data.hooks.get(idx)
                    {
                        ...
                        return self.prompt_extensions_confirm(
                            ...
                            crate::views::extensions_modal::ConfirmationAction::Hooks(
                                froq_hooks_plugins_types::HooksAction::Remove { path },
                            ),
                        );
                    }
                }
            ...
            ButtonAction::ToggleSelectedHook => {
                    ...
                    && let Some(hook) = data.hooks.get(idx)
                {
                    ...
                    if is_collapsed {
                        ...
                        let group_hooks: Vec<&froq_hooks_plugins_types::HookInfo> = data
                            .hooks
                            .iter()
                        ...
                        let action = froq_hooks_plugins_types::HooksAction::ToggleSource {
                            hook_names,
                            disable: any_enabled,
                        };
                        ...
                        let action = if hook.disabled {
                            froq_hooks_plugins_types::HooksAction::Enable {
                                hook_name: hook.name.clone(),
                            }
                        } else {
                            froq_hooks_plugins_types::HooksAction::Disable {
                                hook_name: hook.name.clone(),
                            }
                        };
                        return self.execute_modal_button_action(ButtonAction::HooksAction(action));
            ...
            ButtonAction::ToggleSelectedPlugin => {
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = if plugin.enabled {
                        froq_hooks_plugins_types::PluginsAction::Disable {
                            plugin_id: plugin.id.clone(),
                        }
                    } else {
                        froq_hooks_plugins_types::PluginsAction::Enable {
                            plugin_id: plugin.id.clone(),
                        }
                    };
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    ...
                    return self.prompt_extensions_confirm(
                        ...
                        crate::views::extensions_modal::ConfirmationAction::Plugins(
                            froq_hooks_plugins_types::PluginsAction::Uninstall {
                                plugin_id,
            ...
            ButtonAction::UpdateSelectedPlugin => {
                ...
                if let Some(ref state) = self.extensions_modal
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = froq_hooks_plugins_types::PluginsAction::Update {
            ...
            ButtonAction::InstallSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    ...
                    |source_url_or_path, plugin_relative_path| {
                        froq_hooks_plugins_types::MarketplaceAction::Install {
                            source_url_or_path,
            ...
            ButtonAction::UpdateSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    "Updating...",
                    |source_url_or_path, plugin_relative_path| {
                        froq_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path,
                            plugin_relative_path,
                        }
                    },
                    ...
                    if let TabDataState::Loaded(ref response) = state.marketplace_data {
                        ...
                        if let Some(source) = source {
                            return self.prompt_extensions_confirm(
                                ...
                                crate::views::extensions_modal::ConfirmationAction::Marketplace(
                                    froq_hooks_plugins_types::MarketplaceAction::RemoveSource {
                                        source_url_or_path: source.source_url_or_path.clone(),
                                    },
                                ),
                            );
    ...
    fn execute_selected_marketplace_plugin_action(
        ...
        make_action: impl FnOnce(String, String) -> froq_hooks_plugins_types::MarketplaceAction,
    ) -> InputOutcome {
        if let Some(ref mut state) = self.extensions_modal {
            use crate::views::extensions_modal::TabDataState;
            if let TabDataState::Loaded(ref response) = state.marketplace_data
    ...
    fn update_selected_marketplace_plugin_dispatches_update_and_sets_pending_state() {
        ...
        modal.marketplace_data =
            TabDataState::Loaded(froq_hooks_plugins_types::MarketplaceListResponse {
                sources: vec![froq_hooks_plugins_types::MarketplaceScanResult {
...
mod extensions_action_target_tests {
    ...
    fn plugin_info(name: &str, enabled: bool) -> froq_hooks_plugins_types::PluginInfo {
    ...
    fn plugins_toggle_and_uninstall_resolve_name_and_state() {
        ...
        modal.plugins_data = TabDataState::Loaded(froq_hooks_plugins_types::PluginsListResponse {
        ...
        match outcome {
            crate::app::app_view::InputOutcome::Action(
                crate::app::actions::Action::ExecutePluginsAction(
                    froq_hooks_plugins_types::PluginsAction::Update { plugin_id },
                ),
    ...
    fn skills_toggle_resolves_name_and_resulting_state() {
        let skill = froq_tools::implementations::skills::types::SkillInfo {
            name: "my-skill".into(),
            enabled: false,
            ..Default::default()
        };
    ...
    fn hooks_modal(hooks: Vec<froq_hooks_plugins_types::HookInfo>) -> ExtensionsModalState {
        let mut modal = ExtensionsModalState::new(ExtensionsTab::Hooks);
        modal.hooks_data = TabDataState::Loaded(froq_hooks_plugins_types::HooksListResponse {
            hooks,
    ...
    fn loading_data_yields_no_target() {
        ...
        for action in [
            ...
            ButtonAction::ToggleSelectedMcpServer,
            ButtonAction::InstallSelectedMarketplacePlugin,
            ButtonAction::PluginsAction(froq_hooks_plugins_types::PluginsAction::Reload),
        ] {
            let (target, enabled) = AgentView::extensions_action_target(&modal, &action);
...
mod extensions_modal_confirmation_tests {
    ...
    fn plugin_info(name: &str) -> froq_hooks_plugins_types::PluginInfo {
        froq_hooks_plugins_types::PluginInfo {
            name: name.into(),
            id: format!("user/abcd1234/{name}"),
            root: "/tmp/p".into(),
    ...
    fn hook_info(name: &str, source_dir: &str) -> froq_hooks_plugins_types::HookInfo {
        froq_hooks_plugins_types::HookInfo {
            name: name.into(),
            event: froq_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: froq_hooks_plugins_types::HookHandlerType::Command,
    ...
    fn marketplace_loaded() -> TabDataState<froq_hooks_plugins_types::MarketplaceListResponse> {
        TabDataState::Loaded(froq_hooks_plugins_types::MarketplaceListResponse {
            sources: vec![froq_hooks_plugins_types::MarketplaceScanResult {
                source_name: "test-source".into(),
                source_kind: "git".into(),
    ...
    fn all_prompt_cases() -> Vec<PromptCase> {
        ...
        plugins.plugins_data = TabDataState::Loaded(froq_hooks_plugins_types::PluginsListResponse {
        ...
        hooks.hooks_data = TabDataState::Loaded(froq_hooks_plugins_types::HooksListResponse {
        ...
        vec![
            ...
            PromptCase {
                ...
                expected: ConfirmationAction::Hooks(froq_hooks_plugins_types::HooksAction::Remove {
                    path: source.into(),
                }),
                row: 0,
            },
    ...
    fn plugin_y_sends_confirmed_false_so_server_can_gate_multi() {
        ...
        assert_eq!(
            ...
            Some(froq_hooks_plugins_types::PluginsAction::Uninstall {
                plugin_id: "user/abcd1234/my-plugin".into(),
                confirmed: false,
            })
        );
