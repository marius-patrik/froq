...
use froq_fast_worktree::WorktreeRecord;
...
async fn dispatch(command: WorktreeCommand, tx: &froq_acp_lib::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &froq_acp_lib::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &froq_acp_lib::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
