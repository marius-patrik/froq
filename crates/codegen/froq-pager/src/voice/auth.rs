...
use froq_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
/// `froq_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<froq_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        froq_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
