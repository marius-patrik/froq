...
pub(crate) use froq_telemetry::session_metrics::TraceUploadReason;
...
pub(crate) struct PromptTraceContext {
    ...
    pub(crate) upload_queue: Option<froq_file_utils::queue::UploadQueue>,
...
pub(crate) async fn complete_prompt_trace(
    ...
    >,
    turn_messages: Option<froq_chat_state::TurnCapture>,
...
pub(crate) fn parse_agent_profile_from_meta(
...
) -> Option<froq_agent::AgentDefinition> {
    let value = meta?.get("agentProfile")?;
    if value.is_object() {
        return match froq_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = % def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return froq_agent::discovery::by_name(name);
    }
    tracing::warn!(
