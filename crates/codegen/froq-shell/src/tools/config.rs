...
use froq_sampler::SamplerConfig;
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> froq_tools::implementations::grok_build::web_fetch::WebFetchParams {
        ...
        let allow_local = self
            .allow_local
            .or_else(|| froq_config::env_bool("GROK_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `froq_tools::registry::types::ToolsetConfig` which holds
...
mod tests {
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            froq_agent::config::AgentDefinition::plan(),
            froq_agent::config::AgentDefinition::explore(),
        ] {
