//! Notification bridge: translates `froq-tools` `ToolNotification` events
//! into `froq-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use froq_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use froq_tools::types::output::{BashOutput, ToolOutput};
use froq_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    pub task_wake_suppressed: froq_tools::reminders::task_completion::TaskWakeSuppressed,
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: froq_tools::notification::ScheduledTaskRemoved,
...
async fn emit_current_mode_update(
    config: &NotificationBridgeConfig,
    mode: froq_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == froq_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    froq_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    froq_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = froq_tools::reminders::wrap_reminder(&body);
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, froq_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, froq_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use froq_tools::computer::types::TaskKind;
    use froq_tools::types::TaskSnapshot;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<froq_acp_lib::AcpClientMessage>,
    ...
    fn make_test_config_full_raw() -> (
        ...
        let gateway = froq_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
    ...
    fn task_completed_will_wake(
        gateway_rx: &mut mpsc::UnboundedReceiver<froq_acp_lib::AcpClientMessage>,
    ) -> Option<bool> {
        while let Ok(msg) = gateway_rx.try_recv() {
            if let froq_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "x.ai/task_completed"
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(froq_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            froq_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            froq_tools::notification::types::BashOutputChunk {
                base: froq_tools::notification::types::BashNotificationBase {
        ...
        let broadcast_id = match gateway_rx.try_recv().expect("chunk must be broadcast") {
            froq_acp_lib::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        let (config, _gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
        let removed = froq_tools::notification::ScheduledTaskRemoved {
            task_id: "loop-1".into(),
            generation: "generation-a".into(),
            revision: 2,
        };
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
                ...
                assert!(
                    froq_persisted_event_id(&notif).is_some(),
    ...
    async fn acknowledged_scheduler_removal_appends_before_ack_and_broadcast() {
        ...
        assert!(matches!(
            gateway_rx.try_recv(),
            Ok(froq_acp_lib::AcpClientMessage::ExtNotification(_))
        ));
    }
    fn froq_persisted_event_id(
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            froq_tools::notification::types::BashExecutionBackgrounded {
                base: froq_tools::notification::types::BashNotificationBase {
        ...
        match persistence_rx.try_recv().expect("must persist") {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
                assert!(froq_persisted_event_id(&notif).is_some());
            }
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            froq_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
        ...
        let froq_acp_lib::AcpClientMessage::ExtNotification(fired) = fired else {
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        let notification =
            ToolNotification::PlanModeExited(froq_tools::notification::types::PlanModeExited {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let froq_acp_lib::AcpClientMessage::SessionNotification(args) = msg
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            froq_tools::notification::types::PlanModeEntered {
