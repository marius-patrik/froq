...
//!
//! The canonical error types now live in `froq_sampling_types::error`.
...
pub use froq_sampling_types::error::*;
...
pub const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Froq usage limit for now. Try again later.";
...
/// This stays in froq-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Grok subscription")
                    && crate::agent::auth_method::has_froq_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (froq_API_KEY). \
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                froq_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: froq_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != froq_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == froq_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = froq_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &froq_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    /// Helper: run a closure with froq_API_KEY temporarily set (or cleared).
    /// Cleans up even if the closure panics.
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        let prev = std::env::var("froq_API_KEY").ok();
        let prev_legacy = std::env::var("GROK_CODE_froq_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("froq_API_KEY");
            std::env::remove_var("GROK_CODE_froq_API_KEY");
            if let Some(k) = key {
                std::env::set_var("froq_API_KEY", k);
            }
        }
        ...
        unsafe {
            ...
            std::env::remove_var("GROK_CODE_froq_API_KEY");
            if let Some(v) = prev {
                std::env::set_var("froq_API_KEY", v);
            }
            if let Some(v) = prev_legacy {
                std::env::set_var("GROK_CODE_froq_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("froq-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
