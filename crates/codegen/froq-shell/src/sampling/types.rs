// Re-export all types from the standalone froq-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use froq_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `froq-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `froq-sampler` crate can reference them without a circular
// dep on `froq-shell`.

// Tests for the types now live in froq-sampling-types crate.

use froq_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `froq-sampling-types` or
/// `froq-tools`) so neither of those crates needs to depend on
