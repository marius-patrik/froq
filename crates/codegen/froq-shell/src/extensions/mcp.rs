...
// rmcp is quarantined in froq-mcp; see that crate's docs.
use froq_mcp::rmcp;
...
/// `froq_mcp::wire`), these methods are private to the agent↔client channel and
...
pub async fn notify_servers_updated(
    gateway: &froq_acp_lib::AcpAgentGatewaySender,
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<froq_tools::bridge::ToolBridge>,
    event_writer: Option<&froq_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(froq_file_utils::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = froq_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from froq-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `froq-mcp` directly.
...
impl froq_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<froq_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(froq_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<froq_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(froq_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(froq_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(froq_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
