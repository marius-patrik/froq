...
use froq_telemetry::id::agent_id;
...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = require_froq_auth_for_share(&agent.auth_manager)?;
...
async fn upload_share_data_to_gcs(
    ...
    if let Err(e) = froq_file_utils::gcs::upload_bytes_signed(
...
fn require_froq_auth_for_share(
    auth_manager: &crate::auth::AuthManager,
) -> Result<crate::auth::GrokAuth, acp::Error> {
    super::auth_gate::require_froq_auth(
        auth_manager,
...
mod tests {
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        // return true from is_froq_auth(). This is required for the share tests to
        // exercise the happy path through require_froq_auth_for_share.
        let auth = GrokAuth {
    ...
    fn share_works_outside_the_5m_early_invalidation_window() {
        ...
        assert!(require_froq_auth_for_share(&mgr).is_ok());
    }
    ...
    fn share_succeeds_inside_the_5m_early_invalidation_window() {
        ...
        let res = require_froq_auth_for_share(&mgr);
        assert!(
            res.is_ok(),
            "require_froq_auth_for_share must succeed for a still-valid buffered xAI token"
        );
    }
    ...
    fn share_fails_with_no_auth_at_all() {
        ...
        assert!(require_froq_auth_for_share(&mgr).is_err());
    }

    #[test]
    fn share_rejects_non_froq_auth_with_actionable_grok_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            GrokComConfig::default(),
        ...
        let non_xai = GrokAuth {
            ...
            key: "froq-test-key".into(),
            create_time: Utc::now(),
            ..Default::default()
        };
        mgr.hot_swap(non_xai);

        let err = require_froq_auth_for_share(&mgr)
