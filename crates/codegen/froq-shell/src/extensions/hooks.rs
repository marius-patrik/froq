...
use froq_hooks::event::{HookEventEnvelope, HookEventName};
use froq_hooks::matcher::HookMatcher;
use froq_hooks_plugins_types::{HookEvent, HookHandlerType, HookInfo};
...
pub fn hook_spec_to_info(spec: &froq_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: froq_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[froq_hooks::event::HookEventName] = &[
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern)
            if event.traits().matcher == froq_hooks::event::MatcherPolicy::Ignored =>
        {
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "x.ai/hooks/action" => {
            let req: froq_hooks_plugins_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use froq_hooks::event::{HookEventEnvelope, HookPayload};
