...
//! Delegates to `froq-plugin-marketplace` crate for scanning and install logic.

use agent_client_protocol as acp;
use froq_hooks_plugins_types::{
...
fn load_filtered_marketplace_sources() -> Vec<froq_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                froq_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                froq_plugin_marketplace::SourceKind::Local { path } => {
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            {
                Ok(outcome) => outcome,
                Err(e) => froq_hooks_plugins_types::ActionOutcome {
                    status: froq_hooks_plugins_types::OutcomeStatus::InternalError,
...
fn refresh_sources(
    sources: &[froq_plugin_marketplace::MarketplaceSource],
    source_url_or_path: Option<&str>,
) -> froq_hooks_plugins_types::ActionOutcome {
    let mut refreshed = 0;
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                froq_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let froq_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = froq_plugin_marketplace::git::default_cache_root();
            if let Err(e) = froq_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
    ...
    froq_hooks_plugins_types::ActionOutcome {
        status: froq_hooks_plugins_types::OutcomeStatus::Success,
        message: msg,
        requires_reload: false,
        requires_restart: false,
    }
...
async fn handle_update(
    ...
    use froq_plugin_marketplace::installer;
    use froq_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &froq_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            froq_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            froq_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match froq_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        froq_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = froq_plugin_marketplace::git::default_cache_root();
            match froq_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                froq_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = froq_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = froq_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = froq_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, froq_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(froq_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            froq_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match froq_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    froq_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &froq_plugin_marketplace::MarketplaceSource,
    ...
    let scan = froq_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = froq_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match froq_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: froq_plugin_marketplace::MarketplaceEntry,
...
async fn handle_add_source(url: &str) -> froq_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &froq_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if froq_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        froq_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = froq_config::grok_home().join("config.toml");
    let grok_home = froq_config::grok_home();
...
async fn handle_remove_source(source_url_or_path: &str) -> froq_hooks_plugins_types::ActionOutcome {
...
fn remove_source_locked(source_url_or_path: &str) -> froq_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use froq_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let is_official = froq_plugin_marketplace::is_official_source_url(source_url_or_path);
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a froq_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub fn purge_default_skills_installs(grok_home: &std::path::Path) {
    purge_default_skills_installs_impl(grok_home, || {
        froq_agent::plugins::install_registry::InstallRegistry::try_load_from(
            froq_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}
...
fn purge_default_skills_installs_impl(
    ...
    load_registry: impl FnOnce() -> Result<
        froq_agent::plugins::install_registry::InstallRegistry,
        froq_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = grok_home.join("config.toml");
...
pub fn ensure_official_marketplace_source(grok_home: &std::path::Path) {
    ...
    let toml_sources = froq_plugin_marketplace::load_sources(&parsed);
    let json_sources = froq_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&grok_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, froq_plugin_marketplace::SourceKind::Git { url, .. }
            if froq_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            froq_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                froq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = froq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<froq_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        froq_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            froq_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                froq_plugin_marketplace::SourceKind::Git { url, .. }
                    if froq_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            froq_plugin_marketplace::SourceKind::Git { url, .. }
                if url == froq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                froq_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                froq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"froq-official":{{"source":{{"source":"git","url":"{}"}}}}}}"#,
                froq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_extra_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"extraKnownMarketplaces":{{"froq-official":{{"source":{{"source":"git","url":"{}"}}}}}}}}"#,
                froq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use froq_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = froq_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            ...
            components: Some(froq_hooks_plugins_types::PluginComponents {
                skills: vec![froq_hooks_plugins_types::ComponentItem::new(
                    "code-review",
