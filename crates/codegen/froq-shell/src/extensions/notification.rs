...
/// froq-specific session notification (parallel to acp::SessionNotification)
...
impl PromptUsage {
    ...
    pub fn project_from_ledger(
        ledger: Option<&froq_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&froq_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &froq_chat_state::UsageTotals) -> Self {
        ...
        let froq_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
            reasoning_tokens,
...
impl From<&froq_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &froq_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<froq_hooks_plugins_types::HookInfo>,
...
pub struct CompactionSegmentFile {
    pub items: Vec<froq_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: froq_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<froq_chat_state::compaction_utils::CompactionAttempt>,
}
...
pub struct RecapRequestFile {
    ...
    pub model: String,
    /// Sampling request id sent to the proxy (`froq-recap-{uuid}`).
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_grok_req_id: "froq-recap-abc".into(),
        ...
        assert_eq!(parsed.x_grok_req_id, "froq-recap-abc");
        assert_eq!(
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use froq_chat_state::compaction_utils::CompactionAttempt;
