...
/// two-function shape in `froq-tools` (F29).
...
/// `froq-tools` — existing items keep their prior content when
...
/// binary. Wraps a `froq_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: froq_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: froq_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub struct RunArgs {
    ...
    /// `froq_shell::util::grok_home::grok_home()` when `None`.
...
/// Resolve the API key from `--api-key` → `$froq_API_KEY` →
...
pub async fn resolve_api_key(explicit: Option<&str>, grok_home: &Path) -> Result<String> {
    ...
    let env_key = std::env::var("froq_API_KEY").ok();
    ...
    Err(anyhow!(
        "no API key: pass --api-key, set froq_API_KEY, or run `grok login` to populate \
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `grok login` to re-authenticate, \
             or pass --api-key / set $froq_API_KEY to bypass auth.json."
        )),
    }
}
...
async fn build_sampler_client(
...
) -> Result<froq_sampler::SamplingClient> {
    let default_home;
    ...
    let config = froq_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..froq_sampler::SamplerConfig::default()
    };
    froq_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use froq_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(
            ...
            Some(froq_telemetry::id::agent_id().as_str())
        );
    }
    ...
    async fn resolve_api_key_precedence() {
        ...
        unsafe { std::env::set_var("froq_API_KEY", "from-env") };
        assert_eq!(
        ...
        unsafe { std::env::set_var("froq_API_KEY", "   ") };
        assert!(
            resolve_api_key(None, grok_home).await.is_err(),
    ...
    /// * `froq_API_KEY` — would return early from `resolve_api_key`.
    ...
    async fn run_rejects_missing_trace_path() {
        let prev = std::env::var("froq_API_KEY").ok();
        unsafe { std::env::remove_var("froq_API_KEY") };
        let tmp = tempfile::tempdir().expect("tempdir");
        let args = RunArgs {
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var("froq_API_KEY", v) },
            None => unsafe { std::env::remove_var("froq_API_KEY") },
        }
    }
