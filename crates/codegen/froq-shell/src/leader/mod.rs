...
//!
//! ```ignore
//! use froq_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
const CLIENT_LEADER_VERSION: &str = froq_version::VERSION;
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : froq_version::VERSION,
                            }
                        )),
                    );
                }
...
/// `froq_update::auto_update::resolve_restart_exe`.
///
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let leader_log = std::env::var("GROK_LEADER_LOG")
        ...
        .unwrap_or_else(|_| "froq_shell=info,froq_acp_lib=warn,froq_mcp=warn".into());
