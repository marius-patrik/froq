...
use froq_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use froq_workspace::permission::rules::parse_permission_rule;
use froq_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
///
/// The output JSON is the same shape that `froq-hooks` natively understands
...
mod tests {
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        ...
        refresh_marker_cache(true);
        let dir = tempfile::tempdir().unwrap();
        let env = froq_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = froq_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        let has_probe = |reg: &froq_hooks::discovery::HookRegistry| {
...
"#;
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        let resolved =
            froq_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use froq_workspace::permission::types::RequirementSource;
