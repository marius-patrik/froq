...
use froq_agent::plugins::git_install::{self, UpdateStatus};
use froq_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use froq_plugin_marketplace::git::{self, SourceCacheLease};
use froq_plugin_marketplace::{
...
fn marketplace_root_for_provenance(
    provenance: &froq_agent::plugins::install_registry::MarketplaceProvenance,
...
pub fn marketplace_require_sha() -> bool {
    froq_config::load_effective_config_disk_only()
        .map(|c| froq_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| froq_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &froq_workspace::permission::resolution::MarketplaceAllowlist,
...
mod tests {
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> froq_workspace::permission::resolution::MarketplaceAllowlist {
        froq_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = froq_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
