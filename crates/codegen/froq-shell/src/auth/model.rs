...
use super::is_froq_oauth2_issuer;
...
pub struct GrokAuth {
    ...
    /// credential first-party (`is_froq_auth`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl GrokAuth {
    ...
    pub fn is_froq_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::Oidc | AuthMode::External => self
                .oidc_issuer
                .as_deref()
                .is_some_and(is_froq_oauth2_issuer),
    ...
    pub fn is_managed_mcp_eligible(&self) -> bool {
        self.is_froq_auth() || self.auth_mode == AuthMode::WebLogin
    }
    ...
    /// (`is_froq_auth`), matching the built-in devbox login they replace.
    /// Plain API keys never do.
    pub fn is_session_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::WebLogin | AuthMode::Oidc => true,
            AuthMode::External => self.is_froq_auth(),
...
mod tests {
    ...
    #[test]
    fn is_froq_auth_matrix() {
        use crate::auth::froq_OAUTH2_ISSUER;
        ...
        assert!(with_issuer(AuthMode::Oidc, Some(froq_OAUTH2_ISSUER)).is_froq_auth());
        assert!(with_issuer(AuthMode::External, Some(froq_OAUTH2_ISSUER)).is_froq_auth());
        assert!(!with_issuer(AuthMode::Oidc, None).is_froq_auth());
        assert!(!with_issuer(AuthMode::External, None).is_froq_auth());
        assert!(!with_issuer(AuthMode::Oidc, Some("https://idp.acme.example")).is_froq_auth());
        assert!(!with_issuer(AuthMode::External, Some("https://idp.acme.example")).is_froq_auth());

        // ApiKey / WebLogin stay false even with an x.ai issuer set.
        assert!(!with_issuer(AuthMode::ApiKey, Some(froq_OAUTH2_ISSUER)).is_froq_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(froq_OAUTH2_ISSUER)).is_froq_auth());
    }
    ...
    fn is_session_auth_requires_first_party_for_external() {
        ...
        assert!(with_issuer(AuthMode::External, Some(froq_OAUTH2_ISSUER)).is_session_auth());
        ...
        // Plain API keys never do.
        assert!(!with_issuer(AuthMode::ApiKey, Some(froq_OAUTH2_ISSUER)).is_session_auth());
    }
