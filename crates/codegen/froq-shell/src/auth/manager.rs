...
use froq_telemetry::events::ManualAuthSurface;
...
pub struct AuthManager {
    ...
    power_listener: parking_lot::Mutex<Option<froq_system_power::SystemPowerListener>>,
...
impl AuthManager {
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        ...
        match &write_result {
            Ok(()) => froq_telemetry::unified_log::info(
            ...
            Err(e) => froq_telemetry::unified_log::error(
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        froq_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `froq_system_power::PowerState` for the canonical
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<froq_telemetry::events::ManualAuth> {
...
impl froq_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    ...
    non_empty_key(crate::agent::auth_method::read_froq_api_key_env().ok())
...
) -> froq_tools::types::SharedApiKeyProvider {
