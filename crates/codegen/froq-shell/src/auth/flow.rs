...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<GrokAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_froq_auth() && a.refresh_token.is_some())
}
...
pub async fn run_cli_login(
    ...
    let authenticated = if devbox {
        ...
        if config.grok_com_config.oauth2.is_none() {
            ...
            anyhow::bail!("Sign-in is not available for this deployment. Set froq_API_KEY instead.");
        }
        let grok_home = grok_home::grok_home();
...
pub struct LogoutResult {
    ...
    /// `true` if `froq_API_KEY` / `GROK_CODE_froq_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        froq_telemetry::external::set_identity(
            froq_telemetry::external::IdentityAttrs::default(),
        );
        froq_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_froq_api_key_env(),
    })
}
...
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via froq_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("froq_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::froq_OAUTH2_ISSUER;
    ...
    // A grok.com first-party (x.ai-issuer) OIDC session — `is_froq_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> GrokAuth {
        GrokAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(froq_OAUTH2_ISSUER.to_string()),
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        ...
        let cfg = GrokComConfig {
            auth_provider_command: Some("printf '%s' froq-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("froq-ext-token".to_string()));
    }
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = GrokAuth {
            ...
            oidc_issuer: Some(froq_OAUTH2_ISSUER.into()),
