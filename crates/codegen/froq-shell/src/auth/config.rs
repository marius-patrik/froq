...
pub enum PreferredAuthMethod {
    /// `froq_API_KEY` / auth.json `xai::api_key` / per-model BYOK (`xai.api_key`).
    ApiKey,
...
pub struct GrokComConfig {
    ...
    /// neither advertised nor accepted, so `froq_API_KEY`/per-model credentials
...
pub const froq_OAUTH2_ISSUER: &str = "https://auth.x.ai";
...
const froq_OAUTH2_LOCAL_ISSUER: &str = "http://localhost:22255";
...
pub fn froq_oauth2_issuer() -> &'static str {
    if use_local_auth() {
        froq_OAUTH2_LOCAL_ISSUER
    } else {
        froq_OAUTH2_ISSUER
    }
}
...
/// [`froq_OAUTH2_ISSUER`] directly so that local-dev sessions are still
/// treated as first-party xAI auth.
pub fn is_froq_oauth2_issuer(issuer: &str) -> bool {
    issuer == froq_OAUTH2_ISSUER || issuer == froq_OAUTH2_LOCAL_ISSUER
}
...
impl Default for GrokComConfig {
    fn default() -> Self {
        ...
        let oauth2 = if oidc.is_some() {
            ...
            Some(
                OAuth2ProviderConfig::from_env().unwrap_or_else(|| OAuth2ProviderConfig {
                    issuer: froq_oauth2_issuer().to_owned(),
        ...
        Self {
            ...
            token_header: "froq-cli".to_owned(),
            oidc,
            oauth2,
