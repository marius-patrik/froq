...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| froq_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub(crate) fn embedding_session_credentials(
    ...
    froq_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
        crate::util::is_froq_api_bearer_url,
        auth_credentials,
        api_key_provider,
    )
}
...
///
/// - `"grok-shell"`   — classic Grok CLI / TUI (froq-shell)
/// - `"grok-pager"`   — new Grok Pager / TUI (froq-pager)
...
///
/// to the underlying `froq-file-utils::StorageClient` via
...
///   - `user_token` (normal OIDC users via com scope) → sends Bearer + `X-froq-Token-Auth`
...
pub fn build_storage_client_for_proxy(
...
) -> froq_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn froq_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        froq_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(froq_version::VERSION, client_identifier)
...
/// Bridge that lets `StorageClient` (which lives in froq-file-utils)
...
impl froq_file_utils::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        froq_telemetry::external::set_identity(
            froq_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `froq-pager` and `froq-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> froq_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let grok_com_config = crate::auth::GrokComConfig::default();
    let exporter = froq_telemetry::otel_layer::OtelExporterConfig {
    ...
    froq_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use std::sync::Mutex;
    use froq_auth::AuthCredentialProvider;
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let mgr = make_manager(
            &dir,
            Some(make_auth("froq-session-token", ChronoDuration::hours(1))),
        );
        let api_key_provider: froq_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use froq_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("froq-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("froq-token-EX").as_str())
        );
