...
//!
//! 1. [`froq_telemetry::unified_log::warn`] for the local
...
//!
//! [`froq_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`froq_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`froq_sampler::SamplerConfig::attribution_callback`] at every
...
use froq_sampler::{Auth401AttributionCallback, SamplingConsumer};
use froq_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`froq_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `froq_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn froq_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `froq_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `froq-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `froq-shell` go through this helper -- the
...
mod tests {
    ...
    /// `ShellAttribution` implements `froq_tools::Auth401AttributionCallback`
