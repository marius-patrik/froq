...
/// `froq-telemetry`; re-exported here so existing call sites continue
/// to work.
pub use froq_telemetry::enums::PermissionMode;
...
/// `froq-workspace`.
use froq_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    // definition in `froq_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = froq_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
