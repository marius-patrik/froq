...
///
/// This is a lightweight duplicate of `froq_update::channel_name()` for
/// use in `froq-shell` which cannot depend on `froq-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        let current = semver::Version::parse(froq_version::VERSION).ok()?;
