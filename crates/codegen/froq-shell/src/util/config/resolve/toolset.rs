...
/// [`froq_config::env_bool`] parser (`GROK_TOOLS_FIND_BFS` /
...
/// [`froq_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = froq_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = froq_config::env_bool(primary).or_else(|| froq_config::env_bool(alias));
...
/// the shared [`froq_config::env_bool`] via `BoolFlag`). The secs env var
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            froq_tools::implementations::grok_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        froq_tools::implementations::grok_build::ask_user_question::response_timeout_env_secs(),
...
) -> froq_tools::implementations::grok_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use froq_tools::implementations::grok_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = froq_tools::implementations::grok_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
