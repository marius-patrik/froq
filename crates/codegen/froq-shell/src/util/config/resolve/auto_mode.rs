...
pub fn auto_mode_classifier_defaults(
    ...
    froq_workspace::permission::ClassifierPromptType,
    Option<froq_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(froq_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(froq_sampling_types::ReasoningEffort::Low)
    });
