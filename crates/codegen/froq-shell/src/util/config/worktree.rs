...
use froq_fast_worktree::CreationMode;

/// Worktree creation type configuration.
///
/// Mirrors the internal `CreationMode` enum from froq-fast-worktree but uses
...
pub fn resolve_worktree_auto_gc(
    raw_config: &TomlValue,
    remote: Option<&RemoteSettings>,
) -> froq_fast_worktree::ResolvedWorktreeAutoGc {
...
pub fn resolve_worktree_auto_gc_from_settings(
    ...
    use froq_workspace::worktree::worktree_auto_gc_layer_from_settings;
    ...
    froq_fast_worktree::resolve_worktree_auto_gc_from_layers(
        local_layer.as_ref(),
        remote_layer.as_ref(),
    )
}
...
"#;
    ...
    fn clear_auto_gc_env() {
        unsafe {
            ...
            std::env::remove_var(froq_fast_worktree::ENV_AUTO_GC_REBUILD);
        }
    }
    ...
    fn resolve_worktree_auto_gc_defaults_enabled() {
        ...
        assert!(p.enabled);
        assert_eq!(p.max_age_secs, froq_fast_worktree::DEFAULT_MAX_AGE_SECS);
        assert_eq!(
            p.min_interval_secs,
            froq_fast_worktree::DEFAULT_MIN_INTERVAL_SECS
        );
        assert!(!p.dry_run);
        assert_eq!(
        ...
        assert_eq!(
            ...
            froq_fast_worktree::DEFAULT_REBUILD_MIN_INTERVAL_SECS
        );
    }
    ...
    fn resolve_worktree_auto_gc_rebuild_env_and_toml() {
        clear_auto_gc_env();
        unsafe { std::env::set_var(froq_fast_worktree::ENV_AUTO_GC_REBUILD, "1") };
        let root: TomlValue = toml::from_str("").unwrap();
        let p = resolve_worktree_auto_gc(&root, None);
    ...
    fn resolve_worktree_auto_gc_env_disabled_wins_over_remote_and_local() {
        clear_auto_gc_env();
        unsafe { std::env::set_var(froq_fast_worktree::ENV_AUTO_GC, "0") };
        let root: TomlValue = toml::from_str(
            r#"
            [worktree.auto_gc]
            enabled = true
    ...
    fn resolve_worktree_auto_gc_local_wins_over_remote_ttl_and_env_dry_run_wins() {
        clear_auto_gc_env();
        unsafe { std::env::set_var(froq_fast_worktree::ENV_AUTO_GC_DRY_RUN, "1") };
        let root: TomlValue = toml::from_str(
            r#"
            [worktree.auto_gc]
            max_age_secs = 7200
    ...
    fn resolve_worktree_auto_gc_remote_ttl_clamped() {
        ...
        let p = resolve_worktree_auto_gc(&root, Some(&remote));
        assert_eq!(p.max_age_secs, froq_fast_worktree::MAX_AGE_SECS_MIN);
        assert_eq!(
            p.min_interval_secs,
            froq_fast_worktree::MIN_INTERVAL_SECS_MIN
        );
    }
    ...
    fn resolve_worktree_auto_gc_upper_clamp_via_resolve() {
        ...
        .unwrap();
        let p = resolve_worktree_auto_gc(&root, None);
        assert_eq!(p.max_age_secs, froq_fast_worktree::MAX_AGE_SECS_MAX);
        assert_eq!(
            p.min_interval_secs,
            froq_fast_worktree::MIN_INTERVAL_SECS_MAX
        );
    }
    ...
    fn resolve_worktree_auto_gc_defaults_manual_never() {
        ...
        assert!(
            !p.max_age_by_kind
                .contains_key(&froq_fast_worktree::WorktreeKind::Session),
    ...
    fn resolve_worktree_auto_gc_kind_map_local_wins_over_remote() {
        ...
        let p = resolve_worktree_auto_gc(&root, Some(&remote));
        use froq_fast_worktree::WorktreeKind;
        assert_eq!(
    ...
    fn resolve_worktree_auto_gc_env_max_age() {
        clear_auto_gc_env();
        unsafe { std::env::set_var(froq_fast_worktree::ENV_AUTO_GC_MAX_AGE, "7200") };
        let root: TomlValue = toml::from_str(
            r#"
            [worktree.auto_gc]
            max_age_secs = 86400
