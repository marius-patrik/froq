...
use froq_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use froq_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `froq-config-types` (config
...
pub use froq_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use froq_config_types::PoolConfig;
...
pub fn mcp_preferences_path() -> PathBuf {
    froq_config::grok_home().join("mcp_preferences.json")
}
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    froq_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = froq_mcp::credentials::McpCredentialStore::load_default() {
...
// `.mcp.json` discovery moved to `froq-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use froq_workspace::project_config::{
...
pub fn session_registry_from_env_opt() -> Option<bool> {
    froq_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = froq_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = froq_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = froq_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
