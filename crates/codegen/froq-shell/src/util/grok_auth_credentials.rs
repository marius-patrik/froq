...
/// User token (xAI users) sends `Bearer` + `X-froq-Token-Auth: froq-cli`.
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    obfstr::obfstr!("X-froq-Token-Auth"),
                    obfstr::obfstr!("froq-cli"),
                )
        } else {
            builder
        };
...
impl froq_auth::HttpAuth for GrokAuthCredentials {
