...
use froq_tools::util::truncate::estimate_tokens;
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = froq_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = froq_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = froq_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        grok_version: froq_version::VERSION.to_string(),
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = froq_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        froq_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `froq_agent::prompt::agents_md::read_agents_config_with_paths`
...
async fn list_permissions(cwd: &Path, project_trusted: bool) -> PermissionsReport {
    use froq_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[froq_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = froq_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        froq_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &froq_agent::plugins::PluginRegistry,
    skills_config: &froq_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = froq_agent::prompt::skills::list_skills_with_plugins(
...
fn skill_entry_source(s: &froq_agent::prompt::skills::SkillInfo) -> ConfigSource {
    use froq_tools::implementations::skills::types::SkillScope;
...
fn list_agents(
    ...
    let agents = froq_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[froq_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                froq_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                froq_agent::plugins::PluginScope::Project => Scope::Project,
                froq_agent::plugins::PluginScope::User => Scope::User,
                froq_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: froq_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    froq_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = froq_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
mod tests {
    use super::*;
    use froq_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use froq_tools::implementations::skills::types::SkillScope;
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = froq_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
