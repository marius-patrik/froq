...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-froq-Token-Auth", GrokComConfig::default().token_header)
        .header("x-userid", &auth.user_id)
        .header("x-grok-client-version", froq_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    froq_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn froq_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn froq_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    /// Must include X-froq-Token-Auth so nginx auth subrequest routes to authenticate_froq_cli_token.
    /// See: crates/codegen/froq-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-froq-Token-Auth",
            required(&GrokComConfig::default().token_header, "X-froq-Token-Auth")?,
        );
        ...
        headers.insert(
            ...
            HeaderValue::from_static(froq_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = froq_file_utils::trace_context::inject_trace_context_into_request(
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(froq_telemetry::id::agent_id)
        .await
        .ok()?;
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.froq_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_froq_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set froq_API_KEY.".into(),
                    )
                })?;
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-froq-Token-Auth", "froq-cli")
                .header("x-userid", &auth.user_id)
                .header("x-grok-client-version", froq_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| froq_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(froq_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<froq_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<froq_sampling_types::CompactionsRemaining> {
...
mod tests {
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("froq-cli"));
    ...
    /// ApiKey → `froq_api_base_url` (ApiKey, public default when unset); a custom
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("GROK_DEPLOYMENT_KEY", "froq-token-ENTERPRISE") };
        let managed: toml::Value = toml::from_str(
            r#"[endpoints]
            deployment_key = "froq-token-ENTERPRISE"
            froq_api_base_url = "https://inference.acme-corp.example/xai/v1""#,
        )
        .unwrap();
