...
impl RelayConfig {
    ...
    /// (`is_froq_auth`: x.ai-issuer OIDC or external credential) with a
    ...
    pub(crate) fn for_session(
        ...
        if !session.is_froq_auth() || session.key.is_empty() {
            return None;
        }
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(froq_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                froq_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    #[test]
    fn for_session_builds_only_for_froq_issuer() {
        ...
        let xai = GrokAuth {
            ...
            ..test_auth("froq-bearer")
        };
        assert!(xai.is_froq_auth(), "precondition: is_froq_auth");
        assert!(builds(&xai));
        ...
        assert!(external_xai.is_froq_auth(), "precondition: is_froq_auth");
