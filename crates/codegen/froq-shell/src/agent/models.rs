...
use froq_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl ModelFetchAuth {
    ...
    ///
    /// A `deployment_key` outranks an ambient `froq_API_KEY` so a stray env key
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_froq_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
struct Inner {
    ...
    gateway: RwLock<Option<froq_acp_lib::AcpAgentGatewaySender>>,
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: froq_acp_lib::AcpAgentGatewaySender) {
...
impl ModelsCacheManager {
    ...
    fn load_fresh(
        ...
        if cache.grok_version.as_deref() != Some(froq_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            grok_version: Some(froq_version::VERSION.to_string()),
    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.froq_api_base_url.clone()),
                _ => None,
            };
...
/// explicit off switch must hold even when a stray login, `froq_API_KEY`, or
...
mod tests {
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use froq_sampling_types::ReasoningEffortOption;
    ...
    fn resolve_custom_endpoint_always_wins() {
        let _key = EnvGuard::set("froq_API_KEY", "test-key");
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        let _unset = EnvGuard::unset("froq_API_KEY");
        let _unset_legacy = EnvGuard::unset("GROK_CODE_froq_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    /// `deployment_key` outranks a stray `froq_API_KEY`, but session wins over both.
    #[test]
    #[serial]
    fn resolve_deployment_key_outranks_ambient_api_key() {
        let _key = EnvGuard::set("froq_API_KEY", "stray-env-key");
        let endpoints = config::EndpointsConfig {
            deployment_key: Some("deploy-key".to_owned()),
            ..config::EndpointsConfig::default()
        };
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Deployment,
            "managed deployment_key should outrank an ambient froq_API_KEY",
        );
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, true),
            ModelFetchAuth::Session,
    ...
    /// once — session auth, ambient `froq_API_KEY`, `deployment_key`, AND a
