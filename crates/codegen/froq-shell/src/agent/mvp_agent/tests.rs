...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(crate::auth::AuthMode::ApiKey, "froq-not-a-jwt");
    assert_eq!(
...
mod hunk_tracking_mode {
    ...
    use froq_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<froq_sampling_types::conversation::ConversationItem> {
    use froq_sampling_types::ToolCall;
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = froq_agent::AgentDefinition::from_json(&serde_json::json!(
...
fn inherited_harness_template_skips_nonstrict_model() {
    use froq_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        froq_agent::config::ModelOverride::Override("test-model-123".to_string()),
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use froq_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = froq_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: froq_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: froq_tools::implementations::grok_build::image_gen::ImageGenConfig::default(),
        video_gen_config: froq_tools::implementations::grok_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: froq_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = froq_hunk_tracker::HunkTrackerActor::spawn(
        ...
        hunk_event_tx,
        froq_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        hunk_tracker_handle,
        chat_state_handle: froq_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            froq_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(froq_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: froq_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use froq_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use froq_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<froq_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let froq_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
#[tokio::test(flavor = "current_thread")]
async fn auth_type_froq_api_key_no_current_returns_api_key() {
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::froq_API_KEY_METHOD_ID,
    ));
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    assert_eq!(agent.auth_type(), froq_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), froq_chat_state::AuthType::ApiKey,);
}
...
/// Deployment-key / managed-config user: `froq_API_KEY` resolves and the kill
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    use crate::agent::auth_method::{froq_API_KEY_ENV_VAR, froq_API_KEY_METHOD_ID};
    use froq_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("GROK_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(froq_API_KEY_ENV_VAR, "test-deployment-key");
    let agent = build_minimal_agent_for_tests();
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(froq_API_KEY_METHOD_ID),
        "deployment-key user (froq_API_KEY set, no kill switch) must fall \
         through to xai.api_key on a dead cached_token -- not interactive login",
    );
}
/// Forced-IdP deployment: even with `froq_API_KEY` present, the admin kill
...
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{GROK_COM_METHOD_ID, froq_API_KEY_ENV_VAR};
    use froq_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("GROK_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(froq_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        ...
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive grok.com so froq_API_KEY can't bypass forced IdP login",
    );
}
...
async fn cached_token_fallthrough_falls_to_grok_com_without_credentials() {
    use crate::agent::auth_method::{
        GROK_COM_METHOD_ID, LEGACY_froq_API_KEY_ENV_VAR, froq_API_KEY_ENV_VAR,
    };
    use froq_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("GROK_DISABLE_API_KEY_AUTH");
    let _new = EnvGuard::unset(froq_API_KEY_ENV_VAR);
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use froq_tools::implementations::grok_build::video_gen::{
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = froq_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = froq_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(froq_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<froq_workspace::permission::PermissionEvent>();
    agent
...
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<froq_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, GrokComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                froq_subagent_resolution::config::SubagentRole {
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _sim = EnvGuard::set(froq_version::TEST_VERSION_ENV, "0.0-sim");
...
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<froq_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let froq_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use froq_test_support::EnvGuard;
    use froq_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROK_HOME", home.path());
    let _sim = EnvGuard::set(froq_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, froq_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(froq_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use froq_test_support::EnvGuard;
    use froq_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROK_HOME", home.path());
    let _sim = EnvGuard::set(froq_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(froq_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> froq_announcements::RemoteAnnouncement {
    froq_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<froq_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = froq_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = froq_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<froq_acp_lib::AcpClientMessage>| {
            ...
            let froq_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
