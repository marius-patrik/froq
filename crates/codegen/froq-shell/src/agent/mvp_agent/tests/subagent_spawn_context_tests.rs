...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use froq_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = froq_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                froq_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    froq_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                froq_workspace::permission::AccessKind::Read(Some(".env".into())),
                froq_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        froq_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = froq_sampling_types::ToolOverrides {
        x_search: Some(froq_sampling_types::XSearchOptions {
            date_bound: Some(
                froq_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
