...
impl MvpAgent {
    ...
    fn has_proxy_credentials(&self) -> bool {
        self.cfg.borrow().endpoints.deployment_key.is_some()
            || self.auth_manager.current_or_expired().is_some_and(|a| a.is_froq_auth())
    }
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_froq_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            let (user_id, team_id) = if auth.is_froq_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            froq_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &froq_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<froq_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    froq_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    froq_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match froq_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => froq_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    pub(crate) fn auth_type(&self) -> froq_chat_state::AuthType {
    ...
    /// iff `should_advertise_froq_api_key`; otherwise `grok.com`. Returns `None`
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_froq_api_key(
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<froq_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate ::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == froq_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = froq_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == froq_chat_state::AuthType::ApiKey
    ...
    fn is_tier_restricted_capability(&self) -> bool {
        ...
        if !auth.is_froq_auth() || auth.team_id.is_some() {
            return false;
        }
        let tier = self
            .cfg
    ...
    ///
    /// Both BYOK and session (OAuth) users go direct to `froq_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> froq_tools::implementations::grok_build::image_gen::ImageGenConfig {
        use froq_tools::implementations::grok_build::image_gen::ImageGenConfig;
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.froq_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| froq_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("froq-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use froq_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig;
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> froq_tools::implementations::grok_build::video_gen::VideoGenConfig {
        use froq_tools::implementations::grok_build::video_gen::VideoGenConfig;
    ...
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> froq_tools::implementations::grok_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        let has_froq_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_froq_auth());
        let relay_sync_enabled = tui_mode && relay_config_enabled && has_froq_auth;
        ...
        } else if tui_mode && relay_config_enabled && !has_froq_auth {
            tracing::info!(
        ...
        let instance = Self {
            ...
            plugin_registry_handle: froq_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: froq_tools::implementations::grok_build::task::types::MonitorEventBuffer::default(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<froq_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> froq_tools::implementations::grok_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<froq_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<froq_hooks_plugins_types::HooksListResponse> {
    ...
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: froq_hooks_plugins_types::HooksAction,
    ) -> Option<froq_hooks_plugins_types::ActionOutcome> {
        if matches!(action, froq_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&froq_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub async fn execute_plugins_action(
        &self,
        session_id: &acp::SessionId,
        action: froq_hooks_plugins_types::PluginsAction,
    ) -> Option<froq_hooks_plugins_types::ActionOutcome> {
        let is_reload = matches!(action, froq_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(| o | & o.status),
            Some(froq_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<froq_agent::plugins::PluginRegistry>> {
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<froq_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<froq_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, froq_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(froq_version::VERSION.to_string()),
                workspace_type: None,
                sandbox: local_sandbox_telemetry(),
            };
            let capture = froq_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                froq_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    /// [`froq_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<froq_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> froq_agent::AgentDefinition {
        use froq_agent::AgentDefinition;
        ...
        let model_requires_strict_harness = model_agent_type
            .is_some_and(froq_agent::config::is_strict_harness_agent_type);
        if !grok_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = froq_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = froq_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let resolved = match agent_name.as_deref() {
            ...
            Some(name) => {
                froq_agent::discovery::by_name_in_cwd(name, cwd)
            ...
            && resolved.name != required
        {
            ...
            if let Some(def) = froq_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn froq_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (froq_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        let has_froq_auth = self.auth_manager.current().is_some_and(|a| a.is_froq_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_froq_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = froq_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = froq_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
                ...
                tokio::spawn(
                    froq_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = froq_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                froq_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            froq_agent::config::ModelOverride::Override(id) => {
            ...
            froq_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = froq_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = froq_tools::util::grok_home::grok_home()
                    .join("lsp.json");
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = froq_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                froq_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = froq_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = froq_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    froq_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        froq_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == froq_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        froq_hunk_tracker::LocAggregate::LinesReverted {
