...
//! verdict is [`froq_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = froq_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                froq_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a froq_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a froq_tools::types::CompatConfig,
