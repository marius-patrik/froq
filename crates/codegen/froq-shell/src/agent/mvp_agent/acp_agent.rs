...
) -> Option<froq_sampling_types::ToolOverrides> {
...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&froq_sampling_types::ToolOverrides>,
) {
...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        froq_telemetry::unified_log::info("agent initialized", None, None);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let opts = froq_fast_worktree::AutoGcOptions::from_resolved(auto_gc_policy);
            if let Err(e) = froq_fast_worktree::WorktreeDb::open_default()
                .and_then(|db| froq_fast_worktree::maybe_auto_gc(&db, &opts))
            {
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    froq_workspace::permission::cleanup_stale_permission_state(
        ...
        froq_workspace::trust::migrate_legacy_hook_trust();
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        if !self.cfg.borrow().grok_com_config.api_key_auth_disabled()
            && auth_method::read_froq_api_key_env().is_err()
            && let Some(api_key) = crate::auth::read_api_key(
                &crate::util::grok_home::grok_home(),
            )
        {
            unsafe { std::env::set_var("froq_API_KEY", &api_key) };
        ...
        let has_external_api_key = auth_method::should_advertise_froq_api_key(
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    ...
                    "agentVersion": froq_version::VERSION,
                    "agentId": agent_id(),
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::froq_API_KEY_METHOD_ID => {
                if self.cfg.borrow().grok_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_froq_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set froq_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(froq_telemetry::events::Login {
    ...
    async fn new_session(
        ...
        froq_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || froq_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = froq_telemetry::context::collect_git_context(&cwd_str);
                let ev = froq_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                froq_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match froq_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        froq_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        froq_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        froq_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => froq_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            froq_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = froq_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: froq_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use froq_workspace::session::git::RestoreKind;
            let outcome = froq_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = froq_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    froq_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = froq_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = froq_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(froq_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            froq_file_utils::trace_context::link_current_span_to_meta(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match froq_sampling_types::ToolOverridesUpdate::parse(value) {
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: froq_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                admission: None,
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<froq_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = froq_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = froq_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = froq_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            froq_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_froq_auth(
                    &self.auth_manager,
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                froq_telemetry::session_ctx::log_event(froq_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                froq_telemetry::session_ctx::log_event(froq_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| froq_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                froq_telemetry::session_ctx::log_event(froq_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                froq_telemetry::session_ctx::log_event(froq_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| froq_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == froq_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                froq_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            froq_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
