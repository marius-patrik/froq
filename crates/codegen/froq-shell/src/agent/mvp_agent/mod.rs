...
use froq_telemetry::id::{agent_id, agent_instance_id};
use froq_telemetry::session_ctx::log_event;
use froq_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use froq_workspace::permission::{ClientType, PermissionEvent};
...
use froq_workspace::session::git::GitDiscoveryResult;
use froq_hunk_tracker::HunkTrackerActor;
...
pub(crate) struct PromptResponseMeta {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<froq_sampling_types::ToolOverrides>,
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a froq_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[froq_announcements::RemoteAnnouncement]>,
    last_emitted: &[froq_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<froq_announcements::RemoteAnnouncement>> {
    let current = froq_announcements::filter_expired_at(
...
pub struct MvpAgent {
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<froq_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        froq_tools::implementations::grok_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: froq_tools::implementations::grok_build::task::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: froq_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<froq_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<froq_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &froq_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<froq_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !froq_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = froq_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use froq_agent::config::is_strict_harness_agent_type;
    match (
...
use froq_chat_state::conversation_util::replace_or_insert_system_head;
...
///  - `X-froq-Token-Auth` / `x-authenticateresponse` -- required by the
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<froq_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(froq_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<froq_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<froq_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<froq_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = froq_tools::types::TaskSnapshot {
    ...
    pub(super) async fn maybe_fetch_post_auth_settings(&self) {
        ...
        let is_froq_auth = auth.is_froq_auth();
        ...
        tracing::info!("post-auth remote_settings fetch succeeded");
        {
                ...
                && cfg.mode != crate::agent::config::AgentMode::Generic
            {
                ...
                if cfg.storage_mode == StorageMode::Writeback && !is_froq_auth {
