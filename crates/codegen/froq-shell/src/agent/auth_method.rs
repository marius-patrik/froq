...
pub const froq_API_KEY_ENV_VAR: &str = "froq_API_KEY";

/// Legacy env var name. Checked as a fallback when `froq_API_KEY` is not set,
/// so existing deployments that use the old name keep working.
pub const LEGACY_froq_API_KEY_ENV_VAR: &str = "GROK_CODE_froq_API_KEY";
...
pub fn has_froq_api_key_env() -> bool {
    read_froq_api_key_env().is_ok()
}
...
/// exist (no global `froq_API_KEY`). Deferring it made BYOK users hit the login
...
/// `froq_API_KEY` can't bypass a deployment's forced IdP login.
pub fn should_advertise_froq_api_key<'a, I>(disable_api_key_auth: bool, models: I) -> bool
where
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    has_froq_api_key_env() || models.into_iter().any(ModelEntry::has_own_credentials)
}
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// [`should_advertise_froq_api_key`]. When `preferred_method` is `Oidc`,
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    ...
    BuiltAuthMethods {
        methods: vec![froq_api_key_auth_method()],
        default_auth_method_id: Some(acp::AuthMethodId::new(froq_API_KEY_METHOD_ID)),
    }
}
...
fn build_unpinned(
    ...
    if has_external_api_key {
        methods.push(froq_api_key_auth_method());
        default_auth_method_id = Some(acp::AuthMethodId::new(froq_API_KEY_METHOD_ID));
    }
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            froq_API_KEY_METHOD_ID => Self::XaiApiKey,
...
/// `froq_API_KEY` is set, which demoted live OIDC sessions to non-refreshable
...
pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `grok login`, set froq_API_KEY, or add api_key to ~/.grok/config.toml.";
...
pub const PREFERRED_API_KEY_UNAVAILABLE: &str = "preferred_method=api_key but no API key is configured (set froq_API_KEY or model api_key/env_key in config.toml).";
...
pub const froq_API_KEY_METHOD_ID: &str = "xai.api_key";
pub fn froq_api_key_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(
            acp::AuthMethodId::new(froq_API_KEY_METHOD_ID),
            "xai.api_key".to_string(),
        )
        .description(Some(format!(
            "{froq_API_KEY_ENV_VAR} or api_key/env_key in config.toml"
        ))),
    )
}
...
mod tests {
    ...
    fn auth_method_kind_classifier_matrix() {
        ...
        let api_id = acp::AuthMethodId::new(froq_API_KEY_METHOD_ID);
    ...
    #[test]
    fn enterprise_byok_first_method_is_froq_api_key() {
    ...
    #[test]
    fn byok_with_cached_token_keeps_froq_api_key_first() {
        let inputs = AuthMethodsBuildInputs {
    ...
    #[test]
    fn enterprise_oidc_replaces_grok_com_but_froq_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
    ...
    /// the same predicate (`should_advertise_froq_api_key`) and the same
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        let _global = EnvGuard::unset(froq_API_KEY_ENV_VAR);
        ...
        // login method. Confirms the predicate isn't trivially true.
        {
            ...
            let has_external_api_key = should_advertise_froq_api_key(false, models.values());
    ...
    /// `froq_API_KEY` alone (no per-model creds) also triggers
    ...
    #[test]
    #[serial]
    fn global_external_api_key_advertises_froq_api_key_first() {
        let _set = EnvGuard::set(froq_API_KEY_ENV_VAR, "froq-external-key");
    ...
    #[test]
    #[serial]
    fn disable_api_key_auth_suppresses_froq_api_key_method() {
        let _set = EnvGuard::set(froq_API_KEY_ENV_VAR, "froq-external-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);

        // Flag off: today's behavior (advertised first).
        assert!(should_advertise_froq_api_key(false, models.values()));

        // Flag on: never advertised, regardless of credentials.
        let has_external_api_key = should_advertise_froq_api_key(true, models.values());
        assert!(!has_external_api_key);
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key,
            ..default_inputs()
    ...
    /// Legacy `GROK_CODE_froq_API_KEY` env var is accepted as a fallback
    /// when `froq_API_KEY` is not set, ensuring existing deployments keep working.
    #[test]
    #[serial]
    fn legacy_env_var_fallback_advertises_froq_api_key() {
        let _unset_new = EnvGuard::unset(froq_API_KEY_ENV_VAR);
        let _set_legacy = EnvGuard::set(LEGACY_froq_API_KEY_ENV_VAR, "froq-legacy-key");
        assert!(has_froq_api_key_env());
        assert_eq!(read_froq_api_key_env().unwrap(), "froq-legacy-key");
    ...
    /// When both `froq_API_KEY` and `GROK_CODE_froq_API_KEY` are set,
    /// the new name takes precedence.
    #[test]
    #[serial]
    fn new_env_var_takes_precedence_over_legacy() {
        let _new = EnvGuard::set(froq_API_KEY_ENV_VAR, "new-key");
        let _legacy = EnvGuard::set(LEGACY_froq_API_KEY_ENV_VAR, "old-key");
        assert_eq!(read_froq_api_key_env().unwrap(), "new-key");
    }
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        let _g2 = EnvGuard::unset(froq_API_KEY_ENV_VAR);
    ...
    fn pin_api_key_with_key_only_advertises_api_key() {
        ...
        assert_eq!(method_ids(&built), vec![froq_API_KEY_METHOD_ID]);
        assert_eq!(default_id(&built), Some(froq_API_KEY_METHOD_ID));
    }
