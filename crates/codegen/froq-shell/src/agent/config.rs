...
use std::sync::Arc;
use froq_agent::prompt::skills::SkillsConfig;
use froq_sampler::{AuthScheme, SamplerConfig};
use froq_sampling_types::{
...
pub const froq_API_BASE_URL_DEFAULT: &str = "https://api.x.ai/v1";
...
impl EndpointsConfig {
    ...
    /// the public default. NEVER falls back to `froq_api_base_url` — that is the
    ...
    /// cli-chat-proxy, never `froq_api_base_url`.
    ...
    /// else `proxy_url` + `/deployment/config`. Never `froq_api_base_url`, so the
    ...
    /// 3. `proxy_url` + `/traces`.
    /// Uses the proxy default (not the `froq_api_base_url` fallback) so
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            ...
            froq_api_base_url: std::env::var("GROK_froq_API_BASE_URL")
                .unwrap_or_else(|_| froq_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
...
pub use froq_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
/// (`froq_API_KEY`/legacy), and the credential-bearing `env_string(...)` reads in
...
pub(crate) const FIRST_PARTY_CREDENTIAL_ENV_VARS: &[&str] = &[
    crate::agent::auth_method::froq_API_KEY_ENV_VAR,
    crate::agent::auth_method::LEGACY_froq_API_KEY_ENV_VAR,
    "GROK_AUTH",
    "GROK_AUTH_PATH",
    "GROK_DEPLOYMENT_KEY",
    "GROK_EXTRA_AUTH_KEY",
...
fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> froq_chat_state::CompactionMode {
    use froq_chat_state::CompactionMode;
...
fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> froq_chat_state::CompactionDetail {
    use froq_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(froq_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: froq_tools::types::compat::CompatCell,
...
pub(crate) use froq_telemetry::config::env_telemetry_mode;
pub use froq_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                froq_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> froq_agent::plugins::discovery::DiscoveryConfig {
        froq_agent::plugins::discovery::DiscoveryConfig {
...
pub struct Config {
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<froq_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `froq_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<froq_agent::config::AgentDefinition>,
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, froq_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, froq_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<froq_tools::implementations::grok_build::video_gen::ZdrVideoOutputS3Config>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<froq_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<froq_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut froq_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut froq_agent::config::AgentDefinition) {
...
pub use froq_agent::config::AgentDefinition;
pub use froq_agent::config::Effort;
pub use froq_agent::config::PermissionMode;
pub use froq_shared::ui_config::{ContextualHints, UiConfig};
...
pub struct AgentSelectionConfig {
    ...
    /// Looked up via `froq_agent::discovery::by_name_in_cwd()`.
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(froq_version::VERSION.to_string()),
...
impl Config {
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<froq_sampling_types::DoomLoopRecoveryPolicy> {
        use froq_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
    ...
    /// `froq_fast_worktree::maybe_auto_gc`, not here.
    pub fn resolve_worktree_auto_gc(&self) -> froq_fast_worktree::ResolvedWorktreeAutoGc {
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        use froq_tools::implementations::grok_build::IMAGE_GEN_TOOL_NAME;
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use froq_tools::implementations::grok_build::IMAGE_EDIT_TOOL_NAME;
    ...
    pub(crate) fn resolve_video_gen(&self) -> Resolved<bool> {
        use froq_tools::implementations::grok_build::{
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> froq_chat_state::CompactionMode {
    ...
    fn resolve_compaction_detail(&self) -> froq_chat_state::CompactionDetail {
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        froq_config::load_merged_requirements().as_ref(),
...
pub fn resolve_external_otel_config(
    client: froq_telemetry::external::config::ExternalClientInfo,
) -> Option<froq_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<froq_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| froq_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            froq_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = froq_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = froq_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        froq_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = froq_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    froq_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
pub fn resolve_model_list(
    ...
    for (key, model_override) in &cfg.config_models {
        ...
        let session_bearer_unsafe = !crate::util::is_froq_api_bearer_url(&entry.info.base_url)
            || entry
                .api_base_url
                .as_deref()
                .is_some_and(|url| !crate::util::is_froq_api_bearer_url(url));
...
pub struct ModelEntryConfig {
    ...
    /// If not set, falls back to env_key, then froq_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// If not set, falls back to froq_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<froq_workspace::permission::ClassifierPromptType>,
...
pub struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: froq_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        (
            Some(key),
            info.base_url.clone(),
            froq_chat_state::AuthType::ApiKey,
        )
        ...
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            froq_chat_state::AuthType::SessionToken,
        )
    } else if let Ok(key) = crate::agent::auth_method::read_froq_api_key_env() {
        let url = model
            .api_base_url
            .clone()
            .unwrap_or_else(|| info.base_url.clone());
        (Some(key), url, froq_chat_state::AuthType::ApiKey)
    } else {
...
pub fn enforce_disable_api_key_auth(
    ...
    if disable_api_key_auth
        && creds.auth_type == froq_chat_state::AuthType::ApiKey
        && crate::util::is_froq_api_url(&creds.base_url)
    {
        creds.auth_type = froq_chat_state::AuthType::SessionToken;
        creds.api_key = session_key.map(str::to_owned);
        froq_telemetry::unified_log::debug(
...
pub use froq_telemetry::config::deployment_id_from_key;
...
pub fn resolve_aux_model_sampling_config(
    ...
    let froq_bearer = session_key
        .map(|s| s.to_owned())
        .or_else(|| crate::agent::auth_method::read_froq_api_key_env().ok())
        .or_else(|| endpoints.deployment_key.clone());
    if let Some(bearer) = froq_bearer {
...
pub fn stamp_session_local_sampler_fields(
    ...
    if crate::util::is_froq_api_bearer_url(&cfg.base_url) {
...
pub fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: froq_chat_state::AuthType,
) -> froq_chat_state::AuthType {
...
///
/// * cli-chat-proxy bases get `X-froq-Token-Auth` and
...
pub fn inject_url_derived_headers(
    ...
    if crate::util::is_cli_chat_proxy_url(base_url) {
        headers
            .entry("X-froq-Token-Auth".to_string())
            .or_insert_with(|| "froq-cli".to_string());
        headers
...
mod tests {
    ...
    use froq_test_support::EnvGuard;
    #[test]
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use froq_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn inject_url_derived_headers_adds_proxy_headers_for_cli_chat_proxy_url() {
        ...
        assert_eq!(
            headers.get("X-froq-Token-Auth").map(String::as_str),
            Some("froq-cli")
        );
        assert_eq!(
    ...
    fn inject_url_derived_headers_skips_proxy_headers_for_external_url() {
        ...
        assert!(headers.get("X-froq-Token-Auth").is_none());
    ...
    fn inject_url_derived_headers_does_not_overwrite_existing_entries() {
        let mut headers = IndexMap::new();
        headers.insert("X-froq-Token-Auth".to_string(), "caller-set".to_string());
        inject_url_derived_headers(&mut headers, None, crate::env::PROD_CLI_CHAT_PROXY_BASE_URL);
        assert_eq!(
            headers.get("X-froq-Token-Auth").map(String::as_str),
            Some("caller-set"),
    ...
    fn session_resolver_is_not_stamped_onto_third_party_samplers() {
        ...
        impl froq_sampler::BearerResolver for SessionResolver {
    ...
    async fn static_key_shadows_defined_provider_through_pipeline() {
        ...
        assert_eq!(creds.auth_type, froq_chat_state::AuthType::ApiKey);
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: froq_chat_state::AuthType::ApiKey,
    ...
    fn default_models_dual_endpoint_routing() {
        ...
        for (model_id, entry) in default_model_entries(&endpoints) {
            ...
            assert_eq!(
                api_key_creds.base_url, endpoints.froq_api_base_url,
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        use crate::agent::auth_method::{LEGACY_froq_API_KEY_ENV_VAR, froq_API_KEY_ENV_VAR};
        use froq_chat_state::AuthType;
        use froq_test_support::EnvGuard;
        let sentinel = "froq-global-sentinel-key";
        let primary = "GROK_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "GROK_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
        let _global = EnvGuard::set(froq_API_KEY_ENV_VAR, sentinel);
        let _legacy = EnvGuard::unset(LEGACY_froq_API_KEY_ENV_VAR);
        let mut model = test_model_entry("m", "https://inference.example/v1", None, None, None);
        model.env_key = Some(EnvKeys::new([primary, alias]));
        assert!(!model.has_own_credentials());
        let creds = resolve_credentials(&model, None);
    ...
    fn api_key_creds(base_url: &str) -> ResolvedCredentials {
        ResolvedCredentials {
            api_key: Some("froq-secret".to_string()),
            base_url: base_url.to_string(),
            auth_type: froq_chat_state::AuthType::ApiKey,
            auth_scheme: Default::default(),
        }
    ...
    fn enforce_disable_api_key_auth_blocks_first_party_only() {
        ...
        assert_eq!(creds.auth_type, AuthType::ApiKey);
        assert_eq!(creds.api_key.as_deref(), Some("froq-secret"));
        let mut creds = api_key_creds("https://api.x.ai/v1");
    ...
    fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
        ...
        assert_eq!(creds.api_key.as_deref(), Some("froq-model-key"));
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(
            creds.auth_type,
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = froq_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use froq_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use froq_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(froq_chat_state::CompactionDetail::default())
        );
        assert_eq!(
    ...
    /// froq-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    fn e2e_config_toml_model_overrides_default() {
        ...
        unsafe { std::env::set_var("froq_API_KEY", "froq-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
        unsafe { std::env::remove_var("froq_API_KEY") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn e2e_default_model_with_external_api_key_routes_to_api_xai() {
        ...
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        unsafe { std::env::set_var("froq_API_KEY", "froq-external-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("froq-external-key"));
        assert_eq!(
    ...
    fn e2e_credential_priority_model_key_beats_session_beats_env() {
        ...
        unsafe { std::env::set_var("froq_API_KEY", "env-key") };
    ...
    #[test]
    fn e2e_enterprise_custom_endpoint_skips_froq_defaults() {
        let mut cfg = Config::default();
    ...
    fn e2e_enterprise_endpoints_plus_partial_model_override() {
        ...
        let (_, models) = resolve_models_from_toml(
            ...
            froq_api_base_url = "https://enterprise-api.acme.com/v1"
    ...
    fn e2e_enterprise_endpoints_only_no_model_override() {
        ...
        assert_eq!(
            model.api_base_url.as_deref(),
            Some("https://enterprise-api.acme.com/v1"),
            "default model should use enterprise froq_api_base_url"
        );
    }
    ...
    /// `froq_api_base_url` — overriding ONLY inference keeps every aux endpoint on
    ...
    fn aux_endpoints_resolve_to_proxy_never_inference() {
        ...
        let cfg = EndpointsConfig {
            froq_api_base_url: inference.to_string(),
        ...
        assert_eq!(cfg.froq_api_base_url, inference);
    ...
    /// REGRESSION: the managed-config URL never follows `froq_api_base_url`
    ...
    fn loader_managed_config_url_never_follows_inference_endpoint() {
        ...
        let cfg = Config::new_from_toml_cfg(
            &toml::from_str(
                r#"[endpoints]
                froq_api_base_url = "https://inference.acme-corp.example/xai/v1""#,
            )
            .unwrap(),
        )
    ...
    fn worktree_auto_gc_section_parses_from_toml() {
        unsafe {
            std::env::remove_var(froq_fast_worktree::ENV_AUTO_GC);
            std::env::remove_var(froq_fast_worktree::ENV_AUTO_GC_DRY_RUN);
            std::env::remove_var(froq_fast_worktree::ENV_AUTO_GC_MAX_AGE);
        }
        let raw: toml::Value = toml::from_str(
            r#"
            [worktree.auto_gc]
        ...
        assert_eq!(
            p.max_age_by_kind
                .get(&froq_fast_worktree::WorktreeKind::Subagent),
            Some(&Some(3600))
        );
        assert_eq!(
            p.max_age_by_kind
                .get(&froq_fast_worktree::WorktreeKind::Manual),
            Some(&None)
        );
    }
...
"#,
    ...
    fn config_warns_on_section_typo() {
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoint]
            deployment_key = "froq-token-test"
        "#,
        )
        .unwrap();
    ...
    fn ext_client() -> froq_telemetry::external::config::ExternalClientInfo {
        froq_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
...
"#,
        ...
        froq_config::apply_version_overrides(&mut value, &v).unwrap();
