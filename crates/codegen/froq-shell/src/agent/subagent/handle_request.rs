...
pub(super) fn strip_task_tools_at_max_depth(
    ...
    use froq_tools::implementations::grok_build::task::MAX_SUBAGENT_DEPTH;
...
pub(super) fn canonical_total_tokens(totals: &froq_chat_state::UsageTotals) -> u64 {
    totals.total_tokens()
}
...
pub(crate) async fn handle_subagent_request(
    ...
    {
        use froq_tool_types::SubagentIsolationMode;
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(froq_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = froq_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != froq_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != froq_tool_types::SubagentIsolationMode::None
    {
        ...
        let creation_mode: froq_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
                let mut builder = froq_fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        froq_fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
                    .creation_mode(creation_mode)
                    .worktree_kind(froq_fast_worktree::WorktreeKind::Subagent)
    ...
    effective_runtime.capability_mode = froq_subagent_resolution::intersect_capability_modes(
    ...
    {
        let reminder = froq_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = froq_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            froq_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = froq_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = froq_chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<froq_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = froq_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        use froq_tools::implementations::opencode;
        let memory_tools: Vec<froq_tools::registry::types::ToolConfig> = vec![
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = froq_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == froq_hooks::event::HookEventName::Stop {
                            s.event = froq_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    froq_agent::config::McpServerRef::Named(name) => {
                    ...
                    froq_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            froq_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    froq_telemetry::session_ctx::log_event(froq_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            froq_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            froq_chat_state::CompactionMode::Summary,
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    froq_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                froq_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                ...
                froq_agent::config::PermissionMode::BypassPermissions
            ),
            false,
            None,
            ...
            Vec::new(),
            froq_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        froq_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        froq_telemetry::events::Outcome::Cancelled
    } else {
        froq_telemetry::events::Outcome::Error
    };
    froq_telemetry::session_ctx::log_event(froq_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
