...
use froq_workspace::file_system::AsyncFileSystem;
use froq_hunk_tracker::HunkTrackerHandle;
...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<froq_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn froq_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub sampling_config: froq_sampler::SamplerConfig,
    ...
    pub inherited_tool_overrides: Option<froq_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn froq_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<froq_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<froq_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: froq_tools::implementations::grok_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: froq_tools::implementations::grok_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: froq_tools::implementations::grok_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<froq_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<froq_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<froq_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<froq_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<froq_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<froq_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: froq_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<froq_sampler::SharedAttributionCallback>,
    ...
    pub parent_tool_snapshot: Option<Vec<froq_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<froq_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: froq_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: froq_tools::types::compat::CompatConfig,
    /// Shared completion reservations held by auto-wake prompts.
    pub task_completion_reservations:
        Option<froq_tools::reminders::task_completion::TaskCompletionReservations>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut froq_agent::config::AgentDefinition) {
...
use froq_subagent_resolution::ResumeSourceData;
...
use froq_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &froq_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (froq_sampler::SamplerConfig, acp::ModelId) {
    use froq_agent::config::ModelOverride;
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &froq_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (froq_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &froq_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &froq_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = froq_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(froq_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[froq_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<froq_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<froq_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        froq_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[froq_sampling_types::conversation::ConversationItem],
) -> bool {
    use froq_sampling_types::conversation::ConversationItem;
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<froq_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = froq_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        froq_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
    ...
    if let Some(source) = resume_source {
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens = froq_chat_state::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &froq_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use froq_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<froq_agent::config::AgentDefinition> {
    let mut def = froq_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<froq_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || froq_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = froq_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut froq_agent::config::AgentDefinition,
) {
...
fn summarize_tool_config(
    config: &froq_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
    use std::collections::HashMap;
...
fn resolve_subagent_permission_mode(
    requested: froq_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> froq_agent::config::PermissionMode {
    use froq_agent::config::PermissionMode;
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    froq_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn cancellation_error_message(
    category: Option<froq_file_utils::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    task_completion_reservations: &Option<
        froq_tools::reminders::task_completion::TaskCompletionReservations,
    >,
    ...
    let message = froq_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = froq_tools::reminders::wrap_reminder(&message);
