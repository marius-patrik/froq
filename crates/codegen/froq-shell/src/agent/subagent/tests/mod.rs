...
fn canonical_total_tokens_does_not_double_count_reasoning() {
    let totals = froq_chat_state::UsageTotals {
        input_tokens: 100,
        output_tokens: 40,
        reasoning_tokens: 25,
        ..Default::default()
...
async fn subagent_inherits_session_cli_overrides() {
    use froq_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    ...
    const PIN: &str = froq_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let broadcast_id = match gateway_rx.try_recv().expect("broadcast must fire") {
        froq_acp_lib::AcpClientMessage::ExtNotification(args) => {
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn froq_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = froq_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn dummy_tracker(
    ...
    let cwd = froq_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn froq_workspace::file_system::AsyncFileSystem> = Arc::new(
        froq_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
    );
    ...
    let handle = SessionHandle {
        ...
        max_turns: None,
        hunk_tracker_handle: froq_hunk_tracker::HunkTrackerHandle::noop(),
...
fn explicit_override_takes_precedence_over_role() {
    let overrides = SubagentRuntimeOverrides {
        ...
        capability_mode: Some(froq_tool_types::SubagentCapabilityMode::All),
        ..Default::default()
    };
    let role = froq_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
        resolved.capability_mode, Some(froq_tool_types::SubagentCapabilityMode::ReadOnly)
    );
}
#[test]
...
fn partial_override_fills_from_role() {
    ...
    assert_eq!(resolved.model.as_deref(), Some("explicit-model"));
    assert_eq!(
        resolved.capability_mode, Some(froq_tool_types::SubagentCapabilityMode::Execute)
    );
}
#[test]
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            froq_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = froq_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = froq_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(froq_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use froq_sampling_types::conversation::{
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(froq_sampling_types::synthesized_reasoning_item("thinking",)),
        ConversationItem::assistant("ack"),
    ];
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use froq_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
                    ...
                    ConversationItem::User(u) => {
                        Some(
                            u
                                ...
                                .iter()
                                .filter_map(|p| match p {
                                    froq_sampling_types::conversation::ContentPart::Text {
                                        text,
                                    } => Some(text.as_ref()),
                                    _ => None,
                                })
...
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    use froq_tools::registry::types::{ToolConfig, ToolServerConfig};
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    use froq_agent::config::BuiltinAgentName;
    let mut ctx = ctx_with_toggle(HashMap::new());
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    froq_test_utils::require_git!();
    use froq_test_utils::git::{git_commit_all, init_git_repo};
    ...
    froq_fast_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    froq_fast_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn subagent_auth_type_rule() {
    use crate::agent::auth_method::{CACHED_TOKEN_AUTH_METHOD_ID, froq_API_KEY_METHOD_ID};
    use froq_chat_state::AuthType;
    let session = acp::AuthMethodId::new(CACHED_TOKEN_AUTH_METHOD_ID);
    let api_key = acp::AuthMethodId::new(froq_API_KEY_METHOD_ID);
...
fn normalize_forked_context_empty_parent() {
    ...
    let (conv, prefix_len) = froq_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn test_sampling_config(model_slug: &str) -> froq_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> froq_chat_state::ChatStateHandle {
    let (mock, _persistence_rx) = froq_chat_state::MockChatPersistence::new();
    let (event_tx, _event_rx) = mpsc::unbounded_channel();
    let token = tokio_util::sync::CancellationToken::new();
    froq_chat_state::ChatStateActor::spawn(
        vec![],
