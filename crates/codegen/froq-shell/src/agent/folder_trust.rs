...
//! interactive prompt, and the durable [`froq_workspace::trust::TrustStore`]
//! reads/writes — lives in `froq-workspace` (client-side); this module keeps
...
//! on [`froq_workspace::folder_trust::decide`]; the consume-side nuance is
...
use froq_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) relocated to `froq-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `froq_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use froq_workspace::folder_trust::grant_folder_trust;
...
// (`froq_agent::plugins::TrustStore` at `~/.grok/trusted-plugins`, plus the
...
/// [`froq_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = froq_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`froq_workspace::folder_trust::repo_config_kinds`] /
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    froq_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: froq_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != froq_agent::config::AgentScope::Project || trusted()
}
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            froq_tools::implementations::lsp::config::LspServerConfig,
            froq_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, froq_tools::implementations::lsp::config::LspServerConfig>
{
    froq_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use froq_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(froq_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        let env = froq_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // GROK_FOLDER_TRUST unset so the default-on feature flag applies.
        use froq_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROK_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        use froq_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROK_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        .unwrap();
        let def = froq_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        let _unset_ver = EnvGuard::unset(froq_version::TEST_VERSION_ENV);
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        use froq_agent::plugins::discovery::DiscoveryConfig;
        use froq_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("GROK_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        let has_project_hook = |reg: &froq_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = froq_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use froq_tools::implementations::lsp::config::LspServerConfig;
        use froq_tools::types::config_source::ConfigSource;
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use froq_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use froq_tools::types::config_source::ConfigSource;
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(froq_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var(froq_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(froq_version::TEST_VERSION_ENV) };
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        let _sim = EnvGuard::unset(froq_version::TEST_VERSION_ENV);
        if option_env!("GROK_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        let _sim = EnvGuard::set(froq_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let remote = RemoteSettings {
