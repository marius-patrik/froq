...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(froq_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = froq_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`froq_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    froq_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    froq_telemetry::unified_log::set_version(froq_version::VERSION);
    ...
    froq_file_utils::queue::cleanup_orphaned_uploads(
        &grok_home::grok_home(),
        froq_file_utils::queue::DEFAULT_MAX_AGE,
    );
    ...
    // blocking stdin reads (see `froq_acp_lib::spawn_stdin_line_reader`): on
...
async fn run_headless_inner(
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        let auth_manager = Arc::new(AuthManager::new(&grok_home::grok_home(), ctx.clone()));
        if crate::agent::auth_method::has_froq_api_key_env()
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: froq_version::VERSION.to_string(),
    })
    ...
    local_set
        .run_until(async move {
            ...
            let initial_auth_key_hash = froq_config::user_grok_home()
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> GrokAuth {
        GrokAuth {
            ...
            oidc_issuer: Some(crate::auth::froq_OAUTH2_ISSUER.to_string()),
