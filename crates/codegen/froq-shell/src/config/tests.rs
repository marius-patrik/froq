...
fn apply_requirements_value_overrides_user_settings() {
    ...
    assert_eq!("https://managed-api.example/v1", cfg.endpoints.froq_api_base_url);
    assert_eq!(
...
fn managed_settings_disables_features_and_requirements_overrides() {
    use froq_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> froq_test_support::EnvGuard {
    froq_test_support::EnvGuard::set(froq_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
/// in `froq-agent`. An ABSOLUTE plugin path is used so the merged
...
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use froq_agent::plugins::{TrustStore, discover_plugins};
    use froq_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("GROK_HOME", home.path());
    let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
