...
//!       [--api-key <key> | $froq_API_KEY | <grok-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use froq_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// API key. Overrides `$froq_API_KEY` when set; falls back to
    /// `$froq_API_KEY`, then `<grok-home>/auth.json` (`xai::api_key`
