...
use froq_tools::implementations::grok_build::task::types::SubagentOwner;
...
/// (via [`froq_tool_types::format_resume_footer`]): trace tooling
...
pub(crate) fn build_subagent_trace_items(
    ...
    prompt: &str,
    output: &str,
) -> Vec<froq_sampling_types::conversation::ConversationItem> {
    use froq_sampling_types::conversation::{ConversationItem, ToolCall};
    ...
    let footer = froq_tool_types::format_resume_footer(subagent_id, subagent_type, None);
...
/// the phase via [`froq_chat_state::ChatStateHandle::flush_harness_trace_turn`]
...
pub(crate) fn record_subagent_trace(
    sink: Option<&(froq_chat_state::ChatStateHandle, String)>,
    subagent_id: &str,
    subagent_type: &str,
    description: &str,
    prompt: Option<&str>,
...
pub(crate) struct ChannelSpawner {
    ...
    pub(crate) trace_sink: Option<(froq_chat_state::ChatStateHandle, String)>,
...
mod tests {
    ...
    fn verifier_template_renders_per_agent_type_and_falls_back() {
        use froq_tools::implementations::grok_build::task::types::SubagentTypeSummary;
        let mut tool_names = std::collections::HashMap::new();
        tool_names.insert(
            froq_tools::types::tool::ToolKind::Read,
            "cursor_read".to_string(),
        );
        tool_names.insert(
            froq_tools::types::tool::ToolKind::ListDir,
            "cursor_ls".to_string(),
        );
        tool_names.insert(
            froq_tools::types::tool::ToolKind::Search,
        ...
        let grok = RoleToolNames::from_summary(&summary_with(&[
            (froq_tools::types::tool::ToolKind::Read, "read_file"),
            (froq_tools::types::tool::ToolKind::ListDir, "list_dir"),
            (froq_tools::types::tool::ToolKind::Search, "grep"),
        ]))
    ...
    fn cold_and_resume_renders_are_symmetric_for_an_index() {
        let summary = summary_with(&[
            (froq_tools::types::tool::ToolKind::Read, "cursor_read"),
            (froq_tools::types::tool::ToolKind::ListDir, "cursor_ls"),
            (froq_tools::types::tool::ToolKind::Search, "cursor_grep"),
        ]);
