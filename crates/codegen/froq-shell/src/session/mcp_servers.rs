...
use froq_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
use froq_mcp::servers as inner;
...
pub fn build_config_resolved_event(
    ...
    cwd: &Path,
) -> froq_file_utils::events::Event {
    let disabled: Vec<String> = crate::util::config::disabled_mcp_server_names(cwd)
        .into_iter()
        .collect();
    let servers = configs
        .iter()
        .map(|c| froq_file_utils::events::McpConfigServer {
        ...
        .collect();
    froq_file_utils::events::Event::McpConfigResolved { servers, disabled }
}
