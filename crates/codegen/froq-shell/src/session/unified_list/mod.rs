...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source_workspace_dir: Some("/Users/me/froq-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/froq-src");
    ...
    fn froq_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        ...
        am.hot_swap(crate::auth::GrokAuth {
            ...
            oidc_issuer: Some(crate::auth::froq_oauth2_issuer().to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        let addr = spawn_conversations_stub(
            ...
            .await;
        let _env = froq_test_support::EnvGuard::set(
            "GROK_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(froq_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "x.ai/facetFilters": { "kind": ["build"] },
            })),
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = froq_test_support::EnvGuard::unset("GROK_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = froq_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = froq_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        ...
        let _chat_off = froq_test_support::EnvGuard::unset(GROK_CHAT_MODE_ENV);
        let _desktop_off =
            froq_test_support::EnvGuard::unset("GROK_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                froq_test_support::EnvGuard::set("GROK_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = froq_test_support::EnvGuard::set(GROK_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = froq_test_support::EnvGuard::unset(GROK_CHAT_MODE_ENV);
        ...
        {
            let _on = froq_test_support::EnvGuard::set(GROK_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = froq_workspace::session::git::normalize_repo_url(this_repo).unwrap();
