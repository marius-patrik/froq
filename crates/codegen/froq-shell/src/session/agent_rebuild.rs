...
//! [`froq_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`froq_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`froq_agent::Agent`] owns an [`froq_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`froq_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use froq_agent::config::AgentDefinition;
use froq_agent::error::AgentBuildError;
use froq_agent::prompt::context::PromptAudience;
use froq_agent::prompt::skills::SkillsConfig;
use froq_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use froq_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use froq_tools::implementations::grok_build::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<froq_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<froq_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use froq_tools::implementations::grok_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(froq_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(froq_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(froq_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use froq_tools::implementations::grok_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            froq_tools::computer::local::LocalTerminalBackend::new_local(
                froq_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: froq_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(froq_tools::types::tool::ToolKind::Task)
