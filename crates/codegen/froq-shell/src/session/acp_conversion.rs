//! ACP conversion functions for `froq-tools`'s `ToolOutput`.
//!
//! These standalone functions convert `froq_tools::types::output::ToolOutput`
...
use froq_tool_types::{KillTaskOutput, TaskOutputOutput};
...
pub fn acp_tool_update(
    ...
    match output {
        ...
        ToolOutput::WebFetch(web_fetch_output) => {
            use froq_tools::types::output::WebFetchOutput;
        ...
        ToolOutput::Todo(todo_output) => {
            use froq_tools::types::output::TodoWriteOutput;
        ...
        ToolOutput::AskUserQuestion(ask) => {
            let message = match ask {
                froq_tools::types::output::AskUserQuestionOutput::UserAnswered { message } => {
                    message.clone()
                }
                froq_tools::types::output::AskUserQuestionOutput::QuestionsSent {
                    message,
                    ..
                } => message.clone(),
            };
        ...
        ToolOutput::EnterPlanMode(enter) => {
            let message = match enter {
                froq_tools::types::output::EnterPlanModeOutput::Entered { message, .. } => {
                    message.clone()
                }
            };
            Some(acp::ToolCallUpdate::new(
        ...
        ToolOutput::ExitPlanMode(exit) => {
            let message = match exit {
                froq_tools::types::output::ExitPlanModeOutput::PlanReady {
                    message, ..
                } => message.clone(),
                froq_tools::types::output::ExitPlanModeOutput::EmptyPlan {
                    message, ..
                } => message.clone(),
            };
            Some(acp::ToolCallUpdate::new(
...
///
/// This converts `froq-tools`' TodoItem (which has `id`, `content: Option<String>`,
...
mod tests {
    ...
    fn test_acp_tool_update_todo_returns_completed() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            ...
            todos: vec![],
            state: froq_tools::implementations::grok_build::todo::TodoState::default(),
        }));
        let update = acp_tool_update(&output, "call-1", None, None).unwrap();
        assert_eq!(update.fields.status, Some(acp::ToolCallStatus::Completed));
    }
    ...
    fn test_turn_end_plan_cleanup_preserves_semantics_and_priority() {
        use crate::tools::todo::plan_entry_from_todo_item;
        use froq_tools::implementations::grok_build::todo::{
    ...
    fn test_acp_plan_update_todo() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            summary_for_prompt: "tasks".to_string(),
            todos: vec![
                froq_tools::implementations::grok_build::todo::TodoItem {
                    content: "Task 1".to_string(),
                    priority:
                        froq_tools::implementations::grok_build::todo::TodoPriority::Medium,
                    status:
                        froq_tools::implementations::grok_build::todo::TodoStatus::Completed,
                    meta: None,
                },
            ],
            state: froq_tools::implementations::grok_build::todo::TodoState::default(),
