...
use froq_chat_state::StrictAppendAck;
...
impl JsonlStorageAdapter {
    ...
    /// [`froq_sampling_types::upgrade_legacy_reasoning`] and emit
    ...
    fn read_chat_history_sync(
        ...
        for (line_idx, line) in contents.split(|b| *b == b'\n').enumerate() {
            ...
            let siblings =
                froq_sampling_types::upgrade_legacy_reasoning(&raw, &mut sibling_btc_ids_seen);
            for sib in siblings {
                match &sib {
...
/// `froq-subagent-resolution/src/context.rs` (it counts turns in the same
...
impl JsonlStorageAdapter {
    ...
    pub fn copy_session_data_sync(
        ...
        if options.strip_reasoning {
            chat_to_copy = froq_chat_state::compaction_utils::strip_reasoning_blocks(chat_to_copy);
        }
...
async fn next_compaction_segment_index(compaction_dir: &std::path::Path) -> u64 {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        if let Some(n) = entry
            .file_name()
            .to_str()
            .and_then(froq_chat_state::compaction_transcript::parse_segment_index)
        {
            next = next.max(n + 1);
        }
    }
...
impl StorageAdapter for JsonlStorageAdapter {
    async fn init_session(&self, info: &Info, model_id: acp::ModelId) -> io::Result<Summary> {
        ...
        if Path::new(&summary_path).exists() {
            ...
            summary.sandbox_profile = froq_sandbox::configured_profile_name().map(String::from);
    ...
    async fn update_current_model_and_agent(
        ...
        reasoning_effort: Option<Option<froq_sampling_types::ReasoningEffort>>,
    ) -> io::Result<()> {
        self.apply_summary_patch(
            info,
            super::summary_write::SummaryPatch {
    ...
    async fn merge_rewind_points_from(&self, info: &Info, target_index: usize) -> io::Result<()> {
        let points = self.load_rewind_points(info).await?;
        let merged =
            froq_workspace::session::file_state::merge_rewind_points_from(points, target_index);
    ...
    async fn write_compaction_segment(
        ...
        use froq_chat_state::compaction_transcript::{
