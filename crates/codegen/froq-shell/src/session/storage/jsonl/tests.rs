...
async fn write_compaction_segment_numbers_and_indexes_resume_safely() {
    ...
    let seg = |summary: &str| CompactionSegmentFile {
        ...
        detail: froq_chat_state::CompactionDetail::Verbose,
    ...
    let base = adapter
        .session_dir(&info)
        .join(froq_chat_state::compaction_transcript::COMPACTION_DIR);
...
async fn load_session_without_updates_defers_rewind_points() {
    use froq_workspace::session::file_state::RewindPoint;
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use froq_paths::RelPathBuf;
    use froq_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use froq_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
#[tokio::test]
async fn test_froq_session_update_round_trip() {
    ...
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let froq_notification = XaiSessionNotification {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(froq_notification.clone())))
        .await
        .unwrap();
...
async fn copy_session_data_copies_compaction_segments_when_enabled() {
    ...
    assert!(
        ...
        .join(froq_chat_state::compaction_transcript::COMPACTION_DIR).exists()
    );
}
#[tokio::test]
...
#[tokio::test]
async fn test_copy_session_data_transforms_froq_updates() {
    use crate::extensions::notification::{
        DiffContent, SessionNotification as XaiSessionNotification,
        SessionUpdate as XaiSessionUpdateType,
    };
    ...
    adapter
        .append_update(&source_info, &SessionUpdate::Xai(Box::new(froq_notification)))
        .await
        .unwrap();
    let target_info = Info {
        id: acp::SessionId::new("fork-source-froq-abcd1234"),
    ...
    match &loaded.updates[0] {
        SessionUpdate::Xai(notification) => {
            assert_eq!(notification.session_id.0.as_ref(), "fork-source-froq-abcd1234");
        }
...
async fn assert_copy_clears_pending_relocation(fork_filter: bool) {
    ...
    if !fork_filter {
        ...
        assert!(
            ...
            froq_chat_state::StrictAppendAck::AlreadyPresent(item) if item.text_content()
            == "switch")
        );
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    froq_sandbox::set_configured_profile("workspace");
    let expected = froq_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_multi_tool_cycle_turn_with_reasoning() {
    ...
    let mut items = vec![
        ConversationItem::system("sys"), ConversationItem::user("q"),
        ConversationItem::Reasoning(froq_sampling_types::synthesized_reasoning_item("plan",)),
        ...
        ConversationItem::Reasoning(froq_sampling_types::synthesized_reasoning_item("reflect",)),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
        ...
        ConversationItem::Reasoning(froq_sampling_types::synthesized_reasoning_item("mid")),
...
/// path end-to-end (loader + serde + froq_sampling_types::
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let froq_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    ...
    let froq_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
async fn retry_after_lost_ack_converges_memory_and_disk_to_authoritative_item() {
            ...
            let result = storage
                ...
                .map_err(|error| match error {
                    crate::session::storage::AppendCwdSwitchError::NotCommitted(
                        error,
                    ) => froq_chat_state::StrictAppendError::NotCommitted(error),
                    crate::session::storage::AppendCwdSwitchError::Committed {
                        acknowledgement,
                        source,
                    } => {
                        froq_chat_state::StrictAppendError::Committed {
                            acknowledgement,
                            source,
                        }
                    }
    ...
    let chat = froq_chat_state::ChatStateActor::spawn(
        vec![],
    ...
    assert!(
        ...
        Err(froq_chat_state::StrictAppendError::Indeterminate(_)))
    );
...
async fn acknowledged_chat_append_preserves_existing_file_bytes_and_appends_once() {
    ...
    assert!(
        ...
        .unwrap(), froq_chat_state::StrictAppendAck::Appended)
    );
