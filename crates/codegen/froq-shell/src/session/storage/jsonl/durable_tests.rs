...
fn cwd_switch_retry_after_post_append_barrier_failure_is_already_present() {
    ...
    assert!(matches!(
        ...
        Err(crate::session::storage::AppendCwdSwitchError::Committed {
            acknowledgement: froq_chat_state::StrictAppendAck::Appended,
            ..
        })
    ));
    assert!(matches!(
...
async fn cwd_switch_retry_repairs_bookkeeping_without_duplicate() {
    ...
    assert!(matches!(
        adapter
            ...
            .await,
        Err(crate::session::storage::AppendCwdSwitchError::Committed {
            acknowledgement: froq_chat_state::StrictAppendAck::AlreadyPresent(authoritative),
            ..
...
async fn cwd_switch_retained_by_history_replacement_is_not_recounted() {
    ...
    assert!(matches!(
        adapter
            ...
            .await
            .unwrap(),
        froq_chat_state::StrictAppendAck::AlreadyPresent(authoritative)
