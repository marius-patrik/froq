...
pub(crate) const froq_SESSION_UPDATE_METHOD: &str = "_x.ai/session/update";
        ...
        S: serde::Serializer,
    {
        ...
        match self {
            ...
            SessionUpdate::Xai(notification) => {
                map.serialize_entry("method", froq_SESSION_UPDATE_METHOD)?;
                map.serialize_entry("params", notification)?;
            }
        }
        map.end()
...
impl SessionUpdateEnvelope {
    ...
    pub(crate) fn from_update(update: &SessionUpdate) -> Result<Self, serde_json::Error> {
        ...
        match update {
            ...
            SessionUpdate::Xai(notification) => Ok(Self {
                timestamp,
                method: froq_SESSION_UPDATE_METHOD.to_string(),
                params: serde_json::to_value(notification)?,
            }),
        }
    }
    ...
    pub(crate) fn into_update(self) -> Result<SessionUpdate, serde_json::Error> {
        if self.method == froq_SESSION_UPDATE_METHOD {
    ...
    pub(crate) fn from_str(line: &str) -> Result<SessionUpdate, serde_json::Error> {
        ...
        if let Ok(envelope) = serde_json::from_str::<BorrowedEnvelope<'_>>(line) {
            ...
            return if envelope.method == Some(froq_SESSION_UPDATE_METHOD) {
...
pub enum AppendCwdSwitchError {
    NotCommitted(io::Error),
    Committed {
        acknowledgement: froq_chat_state::StrictAppendAck,
        source: io::Error,
    },
}
...
pub trait StorageAdapter: Send + Sync {
    ...
    ) -> Result<froq_chat_state::StrictAppendAck, AppendCwdSwitchError> {
...
pub(crate) fn filter_rewind_lines<'a>(lines: Vec<&'a str>) -> Vec<&'a str> {
    ...
    for line in &lines {
        let (raw_params, is_xai) = if let Ok(env) = serde_json::from_str::<RawLinePeek<'_>>(line) {
            ...
            let xai = env.method == Some(froq_SESSION_UPDATE_METHOD);
            (raw, xai)
        } else {
            (*line, false)
        };
...
/// Returns only `SessionUpdate::Acp` updates (froq-extension updates like
...
mod tests {
    ...
    fn froq_envelope(session_update_json: &str) -> String {
        format!(
    ...
    #[test]
    fn froq_rewind_marker_yields_rewind_to() {
        let line = froq_envelope(
    ...
    #[test]
    fn froq_rewind_to_zero_yields_rewind_to_zero() {
        let line = froq_envelope(
    ...
    #[test]
    fn froq_diff_review_yields_not_user() {
        let line = froq_envelope(r#"{"sessionUpdate":"diff_review","content":[]}"#);
        assert_eq!(
            parse_prompt_extract_event(&line),
            PromptExtractEvent::NotUserMessage
        );
    ...
    fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
        ...
        // froq-style line persisted by an older binary: no _meta at all.
    ...
    #[test]
    fn from_str_unknown_froq_variant_deserializes_via_envelope() {
        ...
        let line = froq_envelope(r#"{"sessionUpdate":"git_branch_update","branch":"main"}"#);
    ...
    #[test]
    fn from_str_known_froq_variant_still_works() {
        let line = froq_envelope(r#"{"sessionUpdate":"memory_flush_started"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Xai(notif) => {
                assert_eq!(
