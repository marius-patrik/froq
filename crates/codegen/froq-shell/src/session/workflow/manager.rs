...
use froq_workflow::{Journal, WorkflowOutcome, WorkflowRunParams};
...
pub(crate) const WORKFLOW_DEFAULT_AGENT_BUDGET: u64 = froq_workflow::DEFAULT_AGENT_BUDGET;
...
impl WorkflowManager {
    ...
    pub(crate) fn launch(
        ...
        let (run_id, journal, state) = match &spec.resume_run_id {
            Some(run_id) => {
                ...
                if existing.status
                    ...
                    && existing.agents_used >= froq_workflow::MAX_AGENT_BUDGET
                {
                    return Err(LaunchError::NotResumable(
        ...
        let exec = tokio::task::spawn_blocking(move || {
            froq_workflow::run_workflow(WorkflowRunParams {
                script,
                args,
                journal,
                host_tx,
    ...
    pub(crate) fn test_bundle_with_session_dir(
        ...
        let notify = super::notify::WorkflowNotifySender::new(
            ...
            froq_acp_lib::AcpAgentGatewaySender::new(gateway_tx),
            persist_tx,
            store.clone(),
        );
    ...
    fn cancel_children_for_run(&self, run_id: &str) -> bool {
        ...
        self.subagent_event_tx
            .send(
                froq_tools::implementations::grok_build::task::types::SubagentEvent::Cancel(
                    froq_tools::implementations::grok_build::task::types::SubagentCancelRequest {
                        target: froq_tools::implementations::grok_build::task::types::SubagentCancelTarget::WorkflowRunId(
                            run_id.to_owned(),
                        ),
                        respond_to,
                    },
...
mod tests {
    ...
    type CancelLog = Arc<
        parking_lot::Mutex<
            Vec<froq_tools::implementations::grok_build::task::types::SubagentCancelTarget>,
        >,
    >;
    ...
    async fn pause_marks_user_paused_and_resume_replays() {
        ...
        use froq_tools::implementations::grok_build::task::types::SubagentEvent;
        ...
        use froq_tools::implementations::grok_build::task::types::SubagentResult;
    ...
    async fn workflow_spawns_await_to_completion() {
        ...
        assert_eq!(
            ...
            froq_tools::implementations::grok_build::task::types::ModelOverrideProvenance::Tool,
