...
use froq_workflow::{WorkflowMeta, extract_meta};
...
pub(crate) enum ResolveError {
    ...
    Meta(#[from] froq_workflow::MetaError),
}

pub(crate) fn project_root(session_cwd: &Path) -> PathBuf {
    froq_workspace::session::git::find_git_root_from_path(session_cwd)
...
mod tests {
    ...
    fn inline_resolution_validates_name_and_size() {
        ...
        assert!(matches!(
            ...
            Err(ResolveError::Meta(froq_workflow::MetaError::InvalidName))
        ));
        assert!(matches!(
