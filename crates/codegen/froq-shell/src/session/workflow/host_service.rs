...
use froq_workflow::{AgentOpts, AgentResult, BudgetState, HostError, WorkflowHostRequest};
...
pub(crate) const WORKFLOW_MAX_AGENT_RUNS: u32 =
    (froq_workflow::MAX_AGENT_BUDGET as u32) * (SCHEMA_CONTRACT_RETRIES + 1);
...
impl HostService {
    ...
    async fn spawn_agent(&self, mut opts: AgentOpts) -> Result<AgentResult, HostError> {
        ...
        let isolation = opts
            .isolation_worktree
            .then_some(froq_tool_types::SubagentIsolationMode::Worktree);
    ...
    async fn git_diff_since(&self, commit: &str) -> Result<String, HostError> {
        ...
        froq_tty_utils::detach_command(&mut cmd);
