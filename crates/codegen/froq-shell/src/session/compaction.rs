...
use std::sync::Arc;
use froq_chat_state::compaction_utils::{
...
use froq_sampling_types::{ApiBackend, ConversationItem};
...
mod two_pass_prefire_helper_tests {
    ...
    use froq_sampling_types::ConversationItem;
    #[test]
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        froq_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(froq_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`froq-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                froq_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
        ...
        self.send_froq_notification(XaiSessionUpdate::AutoCompactCompleted {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            froq_telemetry::session_ctx::log_event(
                froq_telemetry::events::AutoCompactSuppressed {
    ...
    pub(crate) async fn surface_compact_auth_failure(&self, err: acp::Error) -> acp::Error {
        ...
        acp::Error::auth_required().data(crate::sampling::error::terminal_error_data(
            message,
            Some(401),
            froq_sampler::SamplingErrorKind::Auth,
        ))
    }
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    froq_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    froq_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if froq_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: froq_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            froq_telemetry::events::CompactionTrigger::Manual => "manual",
            froq_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = froq_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            froq_hooks::event::HookEventName::PreCompact,
            froq_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            froq_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            froq_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            froq_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<froq_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            froq_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<froq_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(froq_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<froq_sampling_types::HostedTool> =
        ...
        let estimated_input_tokens =
            froq_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, froq_telemetry::events::CompactionTrigger::Auto);
        let wall_clock_budget_secs = self
            .agent
            .borrow()
            .compaction_policy()
        ...
        let fr_config = froq_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match froq_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(froq_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(froq_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(froq_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = froq_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    froq_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                froq_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                froq_tools::computer::types::TaskKind::Bash => {
                        ...
                        self.tool_context.subagent_event_tx
                    {
                        ...
                        use froq_tools::implementations::grok_build::task::types::{
                    ...
                    let connected_mcp_servers = {
                        ...
                        use froq_tools::implementations::search_tool::{
                    ...
                    let todos = {
                        ...
                        use froq_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn froq_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn froq_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        froq_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: froq_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            froq_hooks::event::HookEventName::PostCompact,
            froq_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) fn should_auto_compact(
        ...
        if froq_token_estimation::exceeds_threshold(
            ...
            let percentage = froq_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &froq_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = froq_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        froq_telemetry::session_ctx::log_event(froq_telemetry::events::AutoCompactFired {
            tokens_before: trigger_info.tokens_used,
            percentage: trigger_info.percentage,
        });
        self.signals_handle()
            .record_compaction(trigger_info.tokens_used);
        self.send_froq_notification(XaiSessionUpdate::AutoCompactStarted {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                froq_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                if self
                    ...
                    == SUPPRESS_NONE
                {
                    self.send_froq_notification(XaiSessionUpdate::AutoCompactFailed {
                        error: String::new(),
                    })
                    .await;
                }
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<froq_sampling_types::ToolSpec>,
        user_context: Option<&str>,
    ...
    fn persist_compaction_checkpoint(
        ...
        self.persist_froq_update_only(XaiSessionUpdate::CompactionCheckpoint(Box::new(info)));
        tracing::info!(
            prompt_index_at_compaction,
...
mod inline_auto_compact_flow_tests {
    ...
    use tokio::sync::mpsc;
    use froq_paths::AbsPathBuf;
    use froq_workspace::file_system::MockFs;
    use froq_workspace::permission::PermissionHandle;
    #[derive(Debug)]
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<froq_acp_lib::AcpClientMessage>,
        ...
        let hunk_tracker_handle = froq_hunk_tracker::HunkTrackerActor::spawn(
            "test-auto-compact".to_string(),
            cwd.to_path_buf(),
            hunk_tx,
            froq_hunk_tracker::TrackingMode::AgentOnly,
        ...
        let chat_state_handle = froq_chat_state::ChatStateActor::spawn(
            vec![],
            froq_sampling_types::SamplingConfig {
            ...
            Box::new(froq_chat_state::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: froq_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: froq_agent_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: froq_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: froq_sampler::SamplerHandle::noop(),
            ...
            workspace_ops: froq_workspace::WorkspaceOps::for_test(),
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<froq_acp_lib::AcpClientMessage>();
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use froq_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    fn api_error_with_context_window(context_window: u64) -> froq_sampler::SamplingErrorInfo {
        froq_sampler::SamplingErrorInfo {
            kind: froq_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_triggers_when_tokens_exceed_new_window() {
        ...
        local
            .run_until(async {
                let (gateway_tx, _) = mpsc::unbounded_channel::<froq_acp_lib::AcpClientMessage>();
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = froq_sampler::SamplingErrorInfo {
                    kind: froq_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = froq_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = froq_chat_state::CompactionMode::Summary;
