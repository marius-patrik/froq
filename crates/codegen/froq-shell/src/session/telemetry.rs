use froq_telemetry::events::SessionHarness;
use froq_workspace::permission::Decision;
...
pub(crate) fn format_hook_name(spec: &froq_hooks::config::HookSpec) -> String {
...
fn format_hook_source(spec: &froq_hooks::config::HookSpec) -> &'static str {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &froq_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: froq_telemetry::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: froq_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: froq_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names = froq_agent::prompt::agents_md::read_agents_config_with_paths(
            &self.cwd,
            self.compat,
        )
        .await
        ...
        .collect();
        let skill_names = froq_agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: froq_telemetry::context::collect_git_context(&self.cwd).is_git_repo,
