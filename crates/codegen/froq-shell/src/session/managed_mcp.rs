...
//! `froq_shell_session_support::managed_mcp` and is re-exported here so
//! `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use froq_shell_session_support::managed_mcp::*;
...
pub fn merge_managed_mcp_servers(
    ...
    plugin_registry: Option<&froq_agent::plugins::PluginRegistry>,
...
pub fn merge_managed_mcp_servers_with_policy(
    ...
    let allowlist = &froq_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
impl McpDisabledReason {
    ...
    pub fn for_blocked_server(
        policy: &froq_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &froq_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: froq_tools::util::grok_home::grok_home().join("config.toml"),
    };
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = froq_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = froq_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use froq_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
...
"#,
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use froq_agent::plugins::PluginRegistry;
        use froq_agent::plugins::PluginScope;
        use froq_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use froq_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: froq_agent::plugins::PluginOrigin::UserGrok,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use froq_agent::plugins::PluginScope;
        use froq_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use froq_agent::plugins::manifest::PluginManifest;
