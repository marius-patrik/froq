    ...
    builder
        ...
        .filter_entry(|entry| {
            ...
            entry
                ...
                .map(|name| {
                    name != ".git"
                        && !froq_file_utils::skip_dir_set().contains(name.to_lowercase().as_str())
                })
                .unwrap_or(true)
        });
...
mod tests {
    use super::*;
    use froq_sampling_types::{AssistantItem, UserItem};
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![froq_sampling_types::ContentPart::Text { text: text.into() }],
    ...
    async fn capture_changes_diff_walkdir_skips_all_well_known_directories() {
        let tmp = tempfile::tempdir().unwrap();
        let goal_created_at = now_unix_seconds() - 60;
        for sub in froq_file_utils::SKIP_DIR_NAMES {
