...
//!
//! Receives [`froq_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`froq_mcp::liveness`]),
//! - the [`froq_mcp::servers::GrokClientHandler`] (server-pushed
...
pub enum McpServerStatus {
    /// Client is in [`froq_mcp::servers::ClientStateKind::Ready`]
...
/// [`froq_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && froq_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///
/// `gateway` is a [`froq_acp_lib::AcpAgentGatewaySender`] (forwarded
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use froq_mcp::servers::{McpClient, McpState};
    ...
    fn discard_gateway() -> froq_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        froq_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = froq_acp_lib::AcpAgentGatewaySender::new(gw_tx);
