...
//!
//! Core worktree lifecycle logic lives in [`froq_workspace::worktree`].
...
use std::path::Path;
use froq_workspace::session::git::find_git_root_from_path;
pub use froq_workspace::worktree::*;
...
async fn create_worktree_for_resume(
    ...
    if find_git_root_from_path(source)
        .ok()
        .is_some_and(|root| froq_workspace::session::git::detect_vcs_kind(&root).is_jj())
    {
...
async fn cleanup_worktree_on_failure(source_cwd: &str, worktree_path: &str) {
    ...
    if is_jj {
        ...
        match tokio::task::spawn_blocking(move || froq_fast_worktree::remove_worktree(&wt_path))
            .await
        {
            Ok(Ok(_)) => {}
            Ok(Err(e)) => {
        ...
        if let Ok(root) = find_git_root_from_path(std::path::Path::new(source_cwd)) {
            let _ = froq_workspace::session::git::git_cli(&root, &["worktree", "prune"]).await;
        }
    }
}
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> froq_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return froq_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    froq_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<froq_workspace::session::git::RestoreDegree>,
}
/// Thin wire-format adapter over the shared
/// [`froq_workspace::session::git::build_restore_decision`] helper.
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &froq_workspace::session::git::CheckoutSessionOutcome,
    kind: froq_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = froq_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
pub async fn resume_session_in_worktree(
    ...
    ops: &froq_workspace::WorkspaceOps,
    ...
    use froq_workspace::session::git::effective_worktree_path;
    tracing::info!(
        target: WORKTREE_LOG,
    ...
    let restore_degree = if codebase_ok {
        Some(froq_workspace::session::git::RestoreDegree::Full)
    } else {
        None
    };
...
async fn resume_local_session_in_worktree(
    req: &ResumeSessionInWorktreeRequest,
    #[allow(unused_variables)] ops: &froq_workspace::WorkspaceOps,
    ...
    if req.restore_code.unwrap_or(restore_code_default) {
        let is_jj = find_git_root_from_path(std::path::Path::new(resolved_source_cwd))
            ...
            .is_some_and(|root| froq_workspace::session::git::detect_vcs_kind(&root).is_jj());
        if !is_jj {
            if froq_workspace::session::git::should_warn_registry_disabled(
                is_jj,
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(froq_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(froq_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(froq_workspace::session::git::RestoreDegree::HeadOnly)
        );
    ...
    fn make_wt_record(path: &str, source_repo: &str) -> froq_fast_worktree::WorktreeRecord {
        froq_fast_worktree::WorktreeRecord {
            id: format!("wt-{}", path.replace('/', "-")),
            path: std::path::PathBuf::from(path),
            source_repo: std::path::PathBuf::from(source_repo),
            repo_name: "repo".into(),
            kind: froq_fast_worktree::WorktreeKind::Session,
