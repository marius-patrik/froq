...
use froq_acp_lib::AcpAgentGatewaySender as GatewaySender;
use froq_agent::AgentDefinition;
use froq_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use froq_agent::prompt::skills::SkillsConfig;
use froq_sampler::SamplerConfig as SamplingConfig;
use froq_sampling_types::truncate_bytes;
...
use froq_tools::types::compat::CompatConfig;
use froq_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use froq_workspace::file_system::CodebaseIndexManager;
use froq_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use froq_workspace::session::file_state::{FileStateHandle, FileStateTracker};
const SESSION_LOG: &str = "froq_session";
#[path = "compaction.rs"]
mod compaction;
...
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<froq_sampling_types::ToolOverridesUpdate>,
...
impl froq_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        froq_tools::types::resources::ManagedGatewayToolCallResponse,
        froq_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| froq_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                froq_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                froq_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = froq_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| froq_tool_protocol::ToolId::new("use_tool").expect("valid"));
                froq_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            froq_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            froq_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, froq_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, froq_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, froq_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, froq_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `froq-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<froq_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: froq_chat_state::ChatStateHandle,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<froq_sampling_types::ToolOverrides>>,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<froq_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<froq_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<froq_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<froq_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<froq_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<froq_agent::Agent>,
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<froq_tools::implementations::grok_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, froq_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: froq_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<froq_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) vcs_kind: froq_workspace::session::git::VcsKind,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<froq_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<froq_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: froq_computer_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `froq-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: froq_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`froq_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`froq_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: froq_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: froq_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == froq_tools::implementations::grok_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == froq_tools::implementations::grok_build::workflow::WORKFLOW_TOOL_NAME
            }),
    ...
    async fn send_feedback_notification(&self, request: crate::session::feedback::FeedbackRequest) {
        self.send_froq_notification(XaiSessionUpdate::FeedbackRequest(request.into()))
            .await;
    }
}
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<froq_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &froq_agent::PromptContext) {
...
) -> Option<froq_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use froq_tools::types::output::{MCPOutput, ToolOutput};
    use froq_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl froq_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl froq_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> froq_tool_protocol::ToolId {
            froq_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::froq_tool_runtime::ListToolsContext,
        ) -> froq_tool_types::ToolDescription {
            froq_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            _ctx: froq_tool_runtime::ToolCallContext,
            _args: serde_json::Value,
        ) -> Result<ToolOutput, froq_tool_runtime::ToolError> {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use froq_tools::registry::types::ToolConfig;
    use froq_tools::tool_taxonomy::TOOL_META_KEY;
    use froq_workspace::permission::PermissionCommand;
...
mod managed_gateway_tool_tests {
    ...
    use froq_tools::types::output::{MCPOutput, ToolOutput};
    use froq_tools::types::tool::{ToolKind, ToolNamespace};
    use froq_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<froq_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
