...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &froq_tools::implementations::grok_build::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
pub(crate) struct PanelResolveCache {
    ...
    describe: std::collections::HashMap<
        String,
        froq_tools::implementations::grok_build::task::types::SubagentDescribeOutcome,
    >,
}
...
fn role_tool_names_from(
    ...
    use froq_tools::implementations::grok_build::task::types::SubagentDescribeOutcome;
...
impl SessionActor {
    async fn evaluate_goal_round(
        ...
        let plan = match plan_file {
            Some(path) => tokio::fs::read_to_string(path)
                .await
                .ok()
                .map(|text| froq_tools::util::truncate_str(&text, 16 * 1024).to_owned()),
            None => None,
        };
            ...
            let (client, model) = if requested_model == active_model {
                ...
                match self.resolve_aux_sampler_config(requested_model).await {
                    Some(mut config) => {
                        ...
                        match froq_sampler::SamplingClient::new(config) {
    ...
    fn record_goal_round_progress(&self, detail: &str, failed: bool) {
        let detail = froq_tools::util::truncate_str(detail.trim(), 500).to_owned();
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        if froq_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use froq_sampling_types::conversation::SyntheticReason;
...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<froq_tools::implementations::grok_build::update_goal::UpdateGoalEnvelope>,
    ) {
        use froq_tools::implementations::grok_build::update_goal::{
    ...
    async fn apply_classifier_outcome_legacy(
        ...
        outcome: crate::session::goal_classifier::GoalClassifierOutcome,
        notify: &crate::session::goal_orchestrator::GoalNotifySender,
    ) -> froq_tools::implementations::grok_build::update_goal::UpdateGoalAck {
        use crate::session::goal_classifier::GoalClassifierOutcome;
        use crate::session::goal_tracker::GoalClassifierVerdict;
        use froq_tools::implementations::grok_build::update_goal::UpdateGoalAck;
