...
//! its buffered/transient/direct variants, froq-notification handling, and
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, froq_chat_state::UsageTotals)],
    ...
    /// AutoCompactCompleted, etc.), use `send_froq_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_froq_update` vs `send_froq_notification`)
    /// mirrors the ACP-side split between `send_update` (high-frequency,
    /// buffered) and `emit_notification_direct` (low-frequency, direct).
    pub(super) async fn send_buffered_froq_update(&self, update: XaiSessionUpdate) {
        self.close_rewind_window().await;
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::Xai(n) => {
                self.log_outbound_froq_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    fn log_outbound_froq_buffered(&self, notification: &XaiSessionNotification) {
        if !matches!(
            notification.update,
        ...
        tracing::info!(
            target : "acp_event", event = "froq_buffered_notification_sent", session_id =
    ...
    pub(super) async fn handle_froq_session_notification(
        &self,
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    froq_hooks::event::HookEventName::SubagentStart,
                    None,
                    froq_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = froq_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        froq_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
    ...
    pub(super) fn persist_froq_update_only(&self, update: XaiSessionUpdate) {
        let notification = XaiSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            froq_hooks::event::HookEventName::Notification,
            None,
            froq_hooks::event::HookPayload::Notification {
    ...
    pub(super) async fn send_froq_notification(&self, update: XaiSessionUpdate) {
        self.send_froq_notification_with_extra_meta(update, None)
            .await;
    }
    /// [`Self::send_froq_notification`] with caller-supplied `_meta` keys merged
    /// into the standard eventId/timestamp meta. Caller keys win on collision.
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_froq_notification_with_extra_meta(
        &self,
...
#[cfg(test)]
mod froq_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_froq_event_id(
    ...
    /// `send_froq_notification` (own emission) and
    /// `handle_froq_session_notification` (inbound/forwarded, meta-less) —
    ...
    #[tokio::test]
    async fn actor_persisted_froq_lines_carry_event_id() {
        ...
        local
            .run_until(async {
                ...
                actor
                    .send_froq_notification(XaiSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_froq_event_id(&mut prx);
                assert!(own_id.starts_with("test-actor-"));
                actor
                    .handle_froq_session_notification(XaiSessionNotification {
                    ...
                    .await;
                let inbound_id = persisted_froq_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_froq_update_only(XaiSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_froq_event_id(&mut prx);
