...
impl SessionActor {
    ...
    pub(super) fn do_hooks_trust_project(cwd: &str) -> Result<std::path::PathBuf, String> {
        let root =
            froq_workspace::session::git::find_git_root_from_path(std::path::Path::new(cwd))
                .map_err(|_| {
    ...
    pub(super) async fn reseed_mcp_output_cap(&self) {
        ...
        let existing = resources
            .get::<froq_tools::types::resources::TruncationCfg>()
            .map(|c| c.0.clone());
        match (resolved, existing) {
            (resolved, Some(mut cfg)) => {
                if cfg.mcp_max_output_bytes != resolved {
                    cfg.mcp_max_output_bytes = resolved;
                    resources.insert(froq_tools::types::resources::TruncationCfg(cfg));
                }
            }
            (Some(v), None) => {
                resources.insert(froq_tools::types::resources::TruncationCfg(
    ...
    pub(super) async fn handle_hooks_action(
        ...
        use froq_hooks_plugins_types::{ActionOutcome, HooksAction, OutcomeStatus};
        ...
        match action {
            ...
            HooksAction::Disable { hook_name } => {
                match froq_hooks::trust::disable_hook(&hook_name) {
                    Ok(()) => ActionOutcome {
                        status: OutcomeStatus::Success,
            ...
            HooksAction::Enable { hook_name } => {
                match froq_hooks::trust::enable_hook(&hook_name) {
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        froq_hooks::trust::disable_hook(name).is_ok()
                    } else {
                        froq_hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
    ...
    pub(super) async fn handle_plugins_action(
        ...
        use froq_hooks_plugins_types::{ActionOutcome, OutcomeStatus, PluginsAction};
        ...
        match action {
            ...
            PluginsAction::Install { source } => {
                ...
                let install_source =
                    froq_agent::plugins::git_install::parse_install_source(&source, cwd);
                let registry = froq_agent::plugins::InstallRegistry::load();
                match froq_agent::plugins::git_install::install_from_source(
                    &install_source,
                    &registry,
                    crate::plugin::marketplace_require_sha(),
                ) {
                    Ok(result) => {
                        let repo = froq_agent::plugins::git_install::build_installed_repo(
                            &result,
                            &install_source,
                        );
            ...
            PluginsAction::Uninstall {
                ...
                let mut registry = froq_agent::plugins::InstallRegistry::load();
                match registry.find_plugin(plugin_name) {
                    ...
                    Some((repo_key, repo, _plugin)) => {
                        ...
                        if let Err(e) =
                            froq_agent::plugins::git_install::remove_repo_path(&repo_path)
                        {
            ...
            PluginsAction::Update { plugin_id } => {
                ...
                for (key, repo) in &repos_to_update {
                    match froq_agent::plugins::git_install::update_repo(
                        key,
                        repo,
                        crate::plugin::marketplace_require_sha(),
                    ) {
                        Ok(status) => {
                            use froq_agent::plugins::git_install::UpdateStatus;
                            match status {
    ...
    pub(super) async fn reload_hooks_impl(self: &std::sync::Arc<Self>) -> String {
        ...
        *self.hook_load_errors.borrow_mut() = errors.iter().map(|e| e.to_string()).collect();
        ...
        if let Some(ref pr) = plugin_registry_snapshot {
            for plugin in pr.active_plugins() {
                if let Some(ref hooks_path) = plugin.hooks_path {
                    let (specs, warnings) =
                        froq_agent::plugins::hooks_adapter::parse_plugin_hooks(
                            hooks_path,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
                ...
                if let Some(ref inline_value) = plugin.inline_hooks {
                    let (specs, warnings) =
                        froq_agent::plugins::hooks_adapter::parse_plugin_hooks_from_value(
                            inline_value,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
        ...
        // no Ref guard is alive across the suspension point.
        {
            ...
            self.send_froq_notification(XaiSessionUpdate::HooksChanged {
                hooks,
                project_trusted,
                load_errors,
            })
    ...
    pub(super) async fn reload_plugins_impl(
        self: &Arc<Self>,
        handle: &froq_agent::plugins::SharedPluginRegistryHandle,
        force: bool,
    ) -> String {
        let session_cwd = std::path::Path::new(&self.session_info.cwd);

        let sid = self.session_info.id.0.as_ref();
        froq_telemetry::unified_log::info("reload_plugins_impl: start", Some(sid), None);
    ...
    pub(crate) fn preserve_session_plugin_dirs(
        &self,
        incoming: Option<std::sync::Arc<froq_agent::plugins::PluginRegistry>>,
    ...
    pub(super) async fn apply_plugin_registry_snapshot(
        self: &Arc<Self>,
        new_registry_snapshot: Option<std::sync::Arc<froq_agent::plugins::PluginRegistry>>,
    ) -> (usize, bool, usize) {
        let sid = self.session_info.id.0.as_ref();
        let session_cwd = std::path::Path::new(&self.session_info.cwd);
        ...
        *self.plugin_registry.borrow_mut() = new_registry_snapshot.clone();
        ...
        if let Some(ref new_registry) = new_registry_snapshot {
            ...
            hooks_reloaded = new_specs.len();
            {
                ...
                if let Some(ref mut arc_reg) = *reg {
                    ...
                    let (mut new_reg, _) =
                        froq_hooks::discovery::load_hooks_from_sources(&[], &[]);
        ...
        let mcp_changed = if let Some(diff) = mcp_diff {
            ...
            {
                let _ = tx.send(froq_mcp::servers::McpClientEvent::ConfigDiff {
        ...
        // BorrowMutError panics when send_froq_notification dispatches
        ...
        let t_notify = std::time::Instant::now();
        {
            ...
            self.send_froq_notification(XaiSessionUpdate::PluginsChanged { plugins })
                .await;
        }
