        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: froq_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: froq_telemetry::memory_log::TARGET,
                ...
                event = chat_state_event_rx.recv() => {
                    match event {
                        ...
                        Some(froq_chat_state::ChatStateEvent::ImageBudget {
                            body_bytes,
                            trigger_bytes,
                        ...
                        Some(froq_chat_state::ChatStateEvent::TokensUpdated { .. }) => {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    let Some(cmd) = maybe_cmd else {
                        ...
                        let envelope = session.fire_hook(
                            froq_hooks::event::HookEventName::SessionEnd,
                            None,
                            froq_hooks::event::HookPayload::SessionEnd {
                        ...
                        if let Some(registry) = session.hook_registry.borrow().clone() {
                            ...
                            let results = froq_hooks::dispatcher::dispatch_non_blocking(
                                &registry,
                                froq_hooks::event::HookEventName::SessionEnd,
                                &envelope,
                                &ctx,
                        ...
                        if !session.startup_hints.is_subagent {
                            if let Some(storage) = session.memory.storage() {
                                ...
                                if let crate::session::memory::hooks::SessionEndResult::Written(ref path_str) = result {
                                    ...
                                    session.send_froq_notification(XaiSessionUpdate::MemorySessionSaved {
                                        path: path_str.clone(),
                                    }).await;
                                }
                            }
                        ...
                        }
                        {
                            ...
                            if let Some(signals) = session.signals_handle().snapshot().await {
                                froq_telemetry::session_ctx::log_event(
                                    froq_telemetry::events::SessionEnded {
                    ...
                    match cmd {
                        ...
                        SessionCommand::OverrideModelName { model_name, extra_headers, context_window } => {
                            ...
                            if let Some(mut cfg) = session.chat_state_handle.get_sampling_config().await {
                                ...
                                if let Some(r) = crate::agent::config::try_resolve_model_credentials(model_name.as_str(), existing.api_key.as_deref()) {
                                    session.chat_state_handle.update_credentials(froq_chat_state::Credentials {
                                        api_key: r.api_key,
                        ...
                        SessionCommand::GetHooksList { respond_to } => {
                            ...
                            let _ = respond_to.send(froq_hooks_plugins_types::HooksListResponse {
                                hooks,
                                project_trusted,
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        froq_tools::implementations::grok_build::task::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = froq_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::XaiSessionNotification { notification } => {
                            session.handle_froq_session_notification(notification).await;
                        }
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            {
                                let _ = tx.send(
                                    froq_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::ToggleMcpServer { server_name, enabled, server_config, respond_to } => {
                            session.events.emit(froq_file_utils::events::Event::McpServerToggled {
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                froq_hooks::event::HookEventName::SessionStart,
                                None,
                                froq_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
