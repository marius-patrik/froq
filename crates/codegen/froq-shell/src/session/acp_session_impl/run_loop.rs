...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<froq_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<froq_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            froq_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : froq_telemetry::memory_log::TARGET,
...
if let Some(timeout) = session.idle_flush_timeout { idle_flush_sleep
            ...
            froq_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(froq_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(froq_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            froq_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(froq_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(froq_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
...
if let Some(tx) = respond_to { let _ =
            ...
            .fire_hook(froq_hooks::event::HookEventName::SessionEnd, None,
            froq_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            froq_hooks::dispatcher::dispatch_non_blocking(& registry,
            froq_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            ...
            froq_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            session.send_froq_notification(XaiSessionUpdate::MemorySessionSaved { path :
...
{ let
            ...
            froq_telemetry::session_ctx::log_event(froq_telemetry::events::SessionEnded
            ...
            false; froq_telemetry::unified_log::info("shell.task_wake.gate_cleared",
...
if let Some(ref tp)
            ...
            froq_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(froq_chat_state::Credentials { api_key : r.api_key,
            ...
            .send(froq_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            .send_froq_notification(XaiSessionUpdate::PluginUpdatesInstalled { updates },)
            ...
            .push_capped(froq_tools::implementations::grok_build::task::types::MonitorEventNotification
            ...
            froq_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .handle_froq_session_notification(notification). await; }
            ...
            .send(froq_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(froq_file_utils::events::Event::McpServerToggled { server_name :
...
if let
            ...
            session.fire_hook(froq_hooks::event::HookEventName::SessionStart, None,
            froq_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            froq_hooks::dispatcher::dispatch_non_blocking(& registry,
            froq_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(froq_hooks::event::HookEventName::SessionEnd, None,
            froq_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            froq_hooks::dispatcher::dispatch_non_blocking(& registry,
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[froq_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = froq_chat_state::compaction_utils::extract_user_query(&raw);
