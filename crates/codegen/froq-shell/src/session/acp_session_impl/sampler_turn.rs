...
/// `SamplingError::is_auth_error` in froq-sampling-types: 403 is
...
pub(super) fn is_auth_tool_error(err: &froq_tool_runtime::ToolError) -> bool {
...
impl SessionTokenAuthGate {
    ...
    fn new(
        ...
        Self {
            ...
            model_byok,
            endpoint_is_first_party: crate::util::is_froq_api_url(base_url),
        }
    }
    fn active(self) -> bool {
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<froq_tools::types::output::ToolRunResult, froq_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                froq_tools::types::output::ToolRunResult,
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<froq_sampling_types::HostedTool>,
        froq_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = froq_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<froq_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }
    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<froq_sampling_types::HostedTool> {
    ...
    /// inherits it.
    pub(crate) fn set_tool_overrides(&self, overrides: froq_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<froq_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl froq_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = froq_file_utils::trace_context::current_traceparent()
        ...
        impl froq_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| froq_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> froq_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<froq_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, froq_workspace::permission::ClassifierFailure>,
            >,
        )>();
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    let (sampling_client, model) = match &aux_classifier_sampler {
                        ...
                        None => {
                            let client = session
                                ...
                                .await
                                .map_err(|e| froq_workspace::permission::ClassifierFailure::TransportError(
                                    e.to_string(),
                                ))?;
                            let model = session
                                .chat_state_handle
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            froq_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            froq_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            froq_workspace::permission::classifier_output_json_schema(),
                        ),
                        ...
                        x_grok_req_id: Some(
                            format!("froq-perm-auto-{}", uuid::Uuid::new_v4()),
                        ),
                        x_grok_session_id: Some(session_id),
                        x_grok_agent_id: Some(froq_telemetry::id::agent_id()),
                    ...
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| {
                            froq_workspace::permission::ClassifierFailure::Timeout
                        })?
                        .map_err(|e| froq_workspace::permission::ClassifierFailure::TransportError(
                            e.to_string(),
                        ))?;
        ...
        let clf =
            froq_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    /// catalog routing (Tier-1 catalog creds / Tier-2 froq-proxy via session token
    /// / `froq_API_KEY` / deployment key), gathering the session-local auth context
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<froq_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(froq_sampler::SamplingClient, String)> {
        ...
        let client = froq_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<froq_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            froq_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `froq-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: froq_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use froq_sampler::SamplingErrorKind;
        if self.tool_context.task_output_token_budget.is_some() {
        ...
        let client_version = froq_version::VERSION;
        ...
        let error_type = if froq_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    ///    `send_froq_notification(RetryState::Failed)`.
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = froq_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = froq_sampler::SamplingErrorInfo::from(&rich_err);
...
fn resolve_configured_cutoff(
    seed: Option<froq_sampling_types::ToolOverrides>,
    base: Option<&froq_sampling_types::ToolOverrides>,
) -> froq_sampling_types::ToolOverrides {
    use froq_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod configured_cutoff_tests {
    ...
    fn inherited_cutoff_agrees_with_the_wire_echo() {
        use froq_sampling_types::{HostedTool, apply_tool_overrides};
