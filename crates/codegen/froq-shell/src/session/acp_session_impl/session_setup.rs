...
impl SessionActor {
    ...
    ) -> Option<froq_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &froq_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ) -> Option<ConversationItem> {
        use froq_tools::types::skill_discovery_tracker::SkillUpdateKind;
        let is_cursor = self.is_cursor_harness();
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn froq_auth::AuthCredentialProvider> = Arc::new(
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = froq_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(froq_tools::types::resources::DenyReadGlobs(
    ...
    pub(super) async fn build_session_info(&self) -> SessionInfoData {
        ...
        let system_prompt_tokens = system_message
            .as_ref()
            .map(froq_chat_state::estimate_system_message_tokens)
            .unwrap_or(0);
        ...
        let tool_definitions_tokens = froq_chat_state::estimate_tool_definitions_tokens(&tool_defs);
        ...
        let free_tokens = froq_token_estimation::free_tokens(context_window, total_tokens);
        let usage_pct = froq_token_estimation::usage_percentage_u8(total_tokens, context_window);
