...
fn drop_cli_catchall_allows(
    rules: Vec<froq_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<froq_workspace::permission::types::PermissionRule>,
    Vec<froq_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if froq_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use froq_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use froq_workspace::permission::rules::parse_permission_rule;
    use froq_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: froq_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<froq_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: froq_chat_state::CompactionMode,
    ...
    preloaded_skills: Option<Vec<froq_tools::implementations::skills::types::SkillInfo>>,
    compat: CompatConfig,
    ...
    web_search_sampling_config: Option<froq_sampler::SamplerConfig>,
    web_fetch_config: froq_tools::implementations::grok_build::web_fetch::WebFetchConfig,
    image_gen_config: froq_tools::implementations::grok_build::image_gen::ImageGenConfig,
    video_gen_config: froq_tools::implementations::grok_build::video_gen::VideoGenConfig,
    app_builder_deployer_config: froq_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: froq_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<froq_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<froq_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<froq_workspace::permission::PermissionHandle>,
    api_key_provider: Option<froq_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<froq_hooks::discovery::HookRegistry>>,
    workspace_ops: froq_workspace::WorkspaceOps,
    cli_permission_rules: Vec<froq_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn froq_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        froq_tools::implementations::grok_build::scheduler::types::SchedulerHandle,
    >,
    ...
    froq_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(froq_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            froq_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = froq_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        froq_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(froq_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if froq_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    froq_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn froq_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            froq_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        froq_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            froq_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let chat_state_sampling_config = froq_sampling_types::SamplingConfig {
    ...
    let actor_pruning_config = froq_chat_state::PruningConfig {
    ...
    let chat_state_handle = froq_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let task_completion_reservations =
        froq_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        froq_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        froq_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        froq_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    ...
    let terminal_backend: std::sync::Arc<dyn froq_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn froq_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn froq_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(froq_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(froq_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || froq_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                froq_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                froq_telemetry::enums::PermissionMode::Auto
            } else {
                froq_telemetry::enums::PermissionMode::Ask
            },
    ...
    let compaction_policy = froq_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        froq_tools::implementations::grok_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn froq_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : froq_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : froq_telemetry::memory_log::TARGET, removed,
        ...
        froq_telemetry::session_ctx::log_event(
            froq_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        froq_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let resolved_task_output =
        froq_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        froq_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        froq_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<froq_tools::implementations::grok_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let resolved_workspace_root = froq_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = froq_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<froq_sampler::SamplingEvent>();
    let sampler_handle = froq_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<froq_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<froq_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = froq_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        froq_tools::implementations::grok_build::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        froq_tools::implementations::grok_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use froq_tools::implementations::grok_build::workflow::WorkflowLaunchAck;
            while let Some((req, ack)) = workflow_launch_rx.recv().await {
                ...
                if input.validate_only {
                    ...
                    let agent_budget = input
                        .agent_budget
                        .unwrap_or(froq_workflow::DEFAULT_AGENT_BUDGET);
                    tokio::spawn(async move {
                        let verdict = tokio::task::spawn_blocking(move || {
                            froq_workflow::validate_script_with_agent_budget(
                                &script,
                                probe_args,
                                agent_budget,
                            )
    ...
    let obs_bridge = {
        let sid = froq_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| froq_tool_protocol::SessionId::new("unknown").expect("valid"));
        froq_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: froq_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match froq_workspace::session::git::discover_git_root(root) {
                froq_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    froq_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => froq_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(froq_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(froq_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            froq_tools::implementations::grok_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                froq_tools::implementations::grok_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : froq_telemetry::memory_log::TARGET, files = files.len(),
                ...
                froq_telemetry::session_ctx::log_event(
                    froq_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use froq_tools::implementations::grok_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use froq_tools::implementations::grok_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = froq_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            froq_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        froq_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, froq_agent::AgentBuildError>,
    >();
            ...
            local.block_on(&rt, async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent" : tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = froq_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
