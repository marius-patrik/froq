...
) -> froq_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> froq_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use froq_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> froq_hooks::runner::RunContext<'_> {
        froq_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_annotation(&self, message: &str) {
        self.send_froq_notification(XaiSessionUpdate::HookAnnotation {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[froq_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, froq_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        use crate::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
        use froq_hooks::result::HookRunResult;
        ...
        self.send_froq_notification(XaiSessionUpdate::HookExecution {
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: froq_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            froq_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                froq_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    froq_telemetry::events::HookOutcome::Success,
                ),
                froq_hooks::result::HookRunResult::Blocked {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    froq_telemetry::events::HookOutcome::Blocked,
                ),
                froq_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    froq_telemetry::events::HookOutcome::Error,
                ),
                froq_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            froq_telemetry::session_ctx::log_event(froq_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: froq_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
