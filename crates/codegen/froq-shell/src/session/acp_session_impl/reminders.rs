...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> froq_agent::ReminderPolicy {
    let mut policy = froq_agent::ReminderPolicy::default();
...
fn workflow_completion_detail(detail: &str) -> std::borrow::Cow<'_, str> {
    ...
    if normalized == detail {
        froq_tools::util::truncate_str_with_marker(detail, WORKFLOW_RESULT_SUMMARY_REMINDER_CAP)
    } else {
        std::borrow::Cow::Owned(
            froq_tools::util::truncate_str_with_marker(
                &normalized,
...
impl SessionActor {
    pub(super) fn push_workflow_launch_reminder(
        ...
        let mut body = format!(
            ...
            froq_tools::util::truncate_str(&command_line, WORKFLOW_OBJECTIVE_REMINDER_CAP)
        );
        ...
        if !objective.is_empty() && !objective_redundant {
            body.push_str(&format!(
                "\nObjective: {}",
                froq_tools::util::truncate_str(&objective, WORKFLOW_OBJECTIVE_REMINDER_CAP)
            ));
        }
        body.push_str(&format!(
        ...
        if run.status.is_paused() {
            if let Some(msg) = run.pause_message.as_deref() {
                let _ = write!(
                    buf,
                    "\n  Paused: {}",
                    froq_tools::util::truncate_str(msg, WORKFLOW_RESULT_SUMMARY_REMINDER_CAP)
                );
            }
            let max_budget_exhausted = run.status
                == crate::session::workflow::tracker::WorkflowRunStatus::BudgetLimited
                && run.agents_used >= froq_workflow::MAX_AGENT_BUDGET;
...
fn format_workflow_completion_reminder(
    ...
    for run in runs {
        ...
        if let Some(summary) = run.result_summary.as_deref() {
            let capped =
                froq_tools::util::truncate_str(summary, WORKFLOW_RESULT_SUMMARY_REMINDER_CAP);
            buf.push_str("\n  Result:\n");
        ...
        if run.status == crate::session::workflow::tracker::WorkflowRunStatus::BudgetLimited {
            if run.agents_used >= froq_workflow::MAX_AGENT_BUDGET {
                let _ = writeln!(
                    buf,
...
pub(super) fn todo_gate_active(
    policy: &froq_agent::system_reminder::ReminderPolicy,
    audience: froq_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    /// in `froq_tools::reminders`.
    pub(super) fn reminder_wrapper_tag(&self) -> &'static str {
        froq_tools::reminders::DEFAULT_REMINDER_TAG
    }
    ...
    pub(super) async fn drain_between_turn_completions(&self) {
        ...
        if !bash_completions.is_empty() {
            ...
            if goal_loop_active {
                ...
                let task_output_name =
                    ...
                    .await;
                let read_tool_name =
                    froq_tools::reminders::task_completion::resolve_read_tool_name(&bridge)
                        .await;
                let reminder = froq_tools::reminders::task_completion::format_between_turn_bash_completions(
                    &bash_completions,
        ...
        let reminder =
            froq_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) async fn drain_between_turn_workflow_completions(&self, goal_loop_active: bool) {
        ...
        let read_tool_name =
            froq_tools::reminders::task_completion::resolve_read_tool_name(&bridge).await;
    ...
    pub(super) fn todo_gate_policy(
        &self,
    ) -> Option<froq_agent::system_reminder::TodoGateConfig> {
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(froq_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();
