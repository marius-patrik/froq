...
/// `list_dir`=`target_directory`. Grammar lives in `froq_chat_state`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<froq_chat_state::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    froq_chat_state::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = froq_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(froq_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from froq-sampling-types rather
