...
impl SessionActor {
    ...
    /// [`froq_prompt_queue::combine_prefix_len`]. `skip_ids` holds rows under
    ...
    pub(super) fn combine_front_pending_inputs(
        ...
        use froq_prompt_queue::{CombineGate, combine_prefix_len};
    ...
    fn combine_gate(item: &InputItem) -> froq_prompt_queue::CombineGate<'_> {
    ...
    fn joined_text_blocks(blocks: &[acp::ContentBlock]) -> String {
        use froq_prompt_queue::join_texts;
    ...
    fn append_text_to_prompt(item: &mut InputItem, extra: &str) {
        use froq_prompt_queue::TEXT_SEPARATOR;
    ...
    fn stamp_combined_display_texts_meta(item: &mut InputItem) {
        use froq_prompt_queue::stamp_combined_display_texts;
