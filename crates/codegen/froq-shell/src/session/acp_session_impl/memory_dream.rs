...
pub(super) struct MemoryFlushSnapshot {
    counts: froq_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &froq_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                froq_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                froq_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        froq_telemetry::session_ctx::log_event(
            froq_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_inner(
        ...
        self.send_froq_notification(XaiSessionUpdate::MemoryDreamCompleted {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_grok_req_id: Some(format!("froq-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: froq_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();

        self.send_froq_notification(XaiSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            froq_telemetry::session_ctx::log_event(
                froq_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                x_grok_req_id: Some(format!("froq-flush-{}", uuid::Uuid::new_v4())),
        ...
        tracing::info!(target: froq_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        froq_telemetry::session_ctx::log_event(
            froq_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => froq_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => froq_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => froq_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => froq_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        froq_telemetry::session_ctx::log_event(froq_telemetry::events::MemoryFlushed {
        ...
        self.send_froq_notification(XaiSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history = crate::sampling::conversation_to_chat_messages(
            froq_chat_state::compaction_utils::prepare_conversation_for_summarization(conversation),
        );
        MemoryFlushSnapshot {
            counts,
            chat_history,
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    froq_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                froq_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = froq_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = froq_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                froq_sampler::collect_response(events).await
            }
        };
