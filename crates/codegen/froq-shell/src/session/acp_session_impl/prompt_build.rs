...
fn partition_rules_by_scope(
    files: Vec<froq_agent::prompt::agents_md::AgentConfigFile>,
    grok_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<froq_agent::prompt::user_message::RuleEntry>,
    Vec<froq_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = froq_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use froq_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[froq_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(froq_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        ...
        use froq_agent::prompt::agents_md::read_agents_config_with_paths;
        use froq_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use froq_workspace::file_system::{git_status_short, jj_status};
        use froq_workspace::session::git::VcsKind;
    ...
    ) -> Vec<froq_agent::prompt::user_message::McpServerEntry> {
        use froq_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &froq_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = froq_sampler::SamplingClient::new(sampler_config).map_err(|e| {
