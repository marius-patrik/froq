...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &froq_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            froq_workspace::session::file_state::FlexiblePath,
            Option<String>,
        ...
        if wants_conversation_rewind {
            ...
            self.persist_froq_update_only(XaiSessionUpdate::RewindMarker {
                target_prompt_index: target_index,
    ...
    /// session: run `froq_chat_state::compaction_utils::repair_history` inside
    ...
    pub(super) async fn handle_repair_history(
        &self,
        dry_run: bool,
    ) -> anyhow::Result<froq_chat_state::compaction_utils::HistoryRepairReport> {
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(froq_chat_state::commands::RepairHistoryBlocked);
        }
