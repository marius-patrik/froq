...
impl SessionActor {
    ...
    pub(super) async fn execute_builtin_slash_command(
        ...
        froq_telemetry::session_ctx::log_event(froq_telemetry::events::SlashCommandUsed {
        ...
        match action {
            ...
            BuiltinAction::SetYolo { enabled } => {
                ...
                if let Some(actual) = yolo_toggle_report(was, actual) {
                    ...
                    froq_telemetry::session_ctx::log_event(
                        froq_telemetry::events::YoloToggled {
                            enabled: actual,
                            previous_state: was,
                            trigger: froq_telemetry::events::YoloTrigger::SlashCommand,
                        },
                    );
                    tracing::info_span!(
            ...
            BuiltinAction::HooksTrust => {
                let msg = match Self::do_hooks_trust_project(&self.session_info.cwd) {
                    Ok(root) => {
                        froq_telemetry::session_ctx::log_event(
                            froq_telemetry::events::HookTrusted { success: true },
                        );
                        format!("Trusted: {}.", root.display())
                    }
                    Err(e) => {
                        froq_telemetry::session_ctx::log_event(
                            froq_telemetry::events::HookTrusted { success: false },
                        );
                        e
                    }
                };
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_hooks_path(&path) {
                        Ok(()) => {
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::HookAdded { success: true },
                            );
                        ...
                        Err(e) => {
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::HookAdded { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        Ok(()) => {
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::HookRemoved { success: true },
                            );
                            self.send_host_turn_slash_command_output(&format!(
                        ...
                        Err(e) => {
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        froq_telemetry::session_ctx::log_event(
                            froq_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_host_turn_slash_command_output(&msg).await;
                    }
                    None => {
                        froq_telemetry::session_ctx::log_event(
                            froq_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_host_turn_slash_command_output(
            ...
            BuiltinAction::SessionInfo => {
                ...
                let context_pct = froq_token_estimation::usage_percentage(ctx.used, ctx.total);
            ...
            BuiltinAction::PluginsAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_plugin_path(&path_str) {
                        Ok(()) => {
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::PluginAdded {
                                    source: froq_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        Ok(()) => {
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::PluginRemoved { success: true },
                            );
                        ...
                        Err(e) => {
                            froq_telemetry::session_ctx::log_event(
                                froq_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsInstall { source, trust } => {
                if source.is_empty() {
                    ...
                    if !trust {
                        let install_source =
                            froq_agent::plugins::git_install::parse_install_source(
                                &source, cwd,
                            );
                        let source_desc = match &install_source {
                            froq_agent::plugins::git_install::InstallSource::Git {
                                url,
                                ..
                            } => {
                                format!("remote git repo: {url}")
                            }
                            froq_agent::plugins::git_install::InstallSource::Local {
                                path,
                                ..
                            } => {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                froq_telemetry::session_ctx::log_event(
                                    froq_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
            ...
            BuiltinAction::MemoryBrowse => {
                ...
                self.send_froq_notification(XaiSessionUpdate::MemoryFiles { files: file_infos })
                    .await;
                ok_end_turn(0, None)
            }
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn froq_tools::types::memory_backend::MemoryBackend,
            ...
            BuiltinAction::GoalClear => {
                ...
                self.clear_pending_classifier_completions();
                self.send_froq_notification(crate::session::goal_orchestrator::build_goal_cleared())
                    .await;
