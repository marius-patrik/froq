...
// froq-interjection-core crate so the server-side agent loop can adopt the
...
pub(crate) use froq_interjection_core::{InterjectionBuffer, drain_formatted, format_interjection};

/// Shell instantiation of the shared entry type: images are ACP content.
pub(crate) type PendingInterjection = froq_interjection_core::PendingInterjection<acp::ImageContent>;
