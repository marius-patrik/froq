...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &froq_tools::implementations::grok_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(crate) async fn normalize_images_with_notices(
        ...
        if !norm_result.compressed.is_empty() {
            ...
            let images = norm_result
                ...
                .collect();
            self.send_froq_notification(XaiSessionUpdate::ImageCompressed { images, message })
                .await;
        }
        if !norm_result.re_encode_fallbacks.is_empty() {
            ...
            self.send_froq_notification(XaiSessionUpdate::ImageCompressed {
                images: vec![],
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let turn_start_input = froq_agent_lifecycle::TurnStartInput::new(
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = froq_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        froq_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    froq_telemetry::session_ctx::log_event(
                        froq_telemetry::events::SlashCommandUsed {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        froq_telemetry::session_ctx::log_event(
                            froq_telemetry::events::PluginUsed {
        ...
        self.observability_bridge
            .emit(
                froq_tool_protocol::session_event::SessionEvent::TurnStarted {
                    turn_number,
                    model_id: model_id.clone(),
                    yolo_mode,
                },
            ...
            .await;
        self.send_before_turn_event(froq_tool_protocol::turn_hook::BeforeTurnPayload {
            turn_number: self.chat_state_handle.get_prompt_index().await as u64,
            model_id: model_id.clone(),
            yolo_mode: self.permissions.is_yolo_mode(),
            conversation_message_count: msg_count,
            session_relationship: froq_tool_protocol::turn_hook::DEFAULT_SESSION_RELATIONSHIP
                .to_string(),
        ...
        froq_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        froq_telemetry::session_ctx::begin_prompt_id();
        let origin = super::super::PromptOrigin::from_prompt_id(prompt_id);
        let mut chunk_meta = serde_json::Map::new();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = froq_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || froq_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = froq_telemetry::events::PromptSubmitted {
            ...
            froq_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(froq_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            froq_hooks::event::HookEventName::UserPromptSubmit,
            froq_hooks::event::HookPayload::UserPromptSubmit {
        ...
        self.observability_bridge
            .emit(froq_tool_protocol::session_event::SessionEvent::TurnEnded {
        ...
        match &result {
            Ok(TurnOutcome::Completed { refusal, .. }) => {
                ...
                if let Some(explanation) = refusal {
                    ...
                    self.dispatch_hook(
                        froq_hooks::event::HookEventName::StopFailure,
                        froq_hooks::event::HookPayload::StopFailure {
                            error: froq_hooks::event::StopFailureKind::InvalidRequest,
                ...
                self.send_after_turn_event(froq_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: froq_tool_protocol::turn_hook::TurnHookOutcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                ...
                .await;
                froq_telemetry::session_ctx::log_event(
                    froq_telemetry::events::TurnCompleted {
                        outcome: froq_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                self.send_after_turn_event(froq_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: froq_tool_protocol::turn_hook::TurnHookOutcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                froq_telemetry::session_ctx::log_event(
                    froq_telemetry::events::TurnCompleted {
                        outcome: froq_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                self.send_after_turn_event(froq_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: froq_tool_protocol::turn_hook::TurnHookOutcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                froq_telemetry::session_ctx::log_session_event(
                    froq_telemetry::events::ApiError {
                ...
                froq_telemetry::session_ctx::log_event(
                    froq_telemetry::events::TurnCompleted {
                        outcome: froq_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
                    contributor
                        .on_turn_done(&froq_agent_lifecycle::TurnDoneInput)
                        .await;
                }
            }
            Ok(TurnOutcome::Cancelled { .. }) | Ok(TurnOutcome::MaxTurnsReached { .. }) => {
                let input = froq_agent_lifecycle::TurnAbortInput::new(
                    froq_agent_lifecycle::TurnAbortReason::Interrupted,
                );
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
            ...
            Err(err) => {
                ...
                let input = froq_agent_lifecycle::TurnErrorInput { message: &message };
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = froq_tools::implementations::grok_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = froq_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = froq_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(super) async fn process_conversation_turn_with_recovery(
        ...
        loop {
            ...
            if attempt > recovery.max_retries {
                ...
                self.send_froq_notification(XaiSessionUpdate::AutoRecoveryExhausted {
                    attempts: attempt,
                    error: error_desc,
                })
                .await;
            ...
            self.send_froq_notification(XaiSessionUpdate::AutoRecoveryStarted {
                attempt,
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use froq_tools::types::memory_backend::MemoryBackend as _;
        ...
        tracing::info!(
            target : froq_telemetry::memory_log::TARGET, configured_min_score,
            "MEMORY_INJECT_SEARCH: results={result_count}"
        );
        froq_telemetry::session_ctx::log_event(
            froq_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<froq_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                froq_telemetry::session_ctx::log_event(froq_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            request.x_grok_agent_id = Some(froq_telemetry::id::agent_id());
            ...
            self.observability_bridge
                .emit(
                    froq_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: froq_tool_protocol::session_event::SessionPhase::Sampling,
                    },
                )
                .await;
            froq_telemetry::unified_log::info(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                froq_telemetry::session_ctx::log_event(
                    froq_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(froq_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    froq_sampling_types::ConversationItem::Assistant(_) => {
            ...
            self.observability_bridge
                .emit(
                    froq_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: froq_tool_protocol::session_event::SessionPhase::ToolExecution,
                    },
                )
                .await;
