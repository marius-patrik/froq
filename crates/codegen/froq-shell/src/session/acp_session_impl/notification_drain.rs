...
impl SessionActor {
    ...
    pub(super) async fn emit_session_idle_if_idle(&self) {
        ...
        for contributor in self.extension_registry.session_lifecycle_contributors() {
            contributor
                .on_session_idle(&froq_agent_lifecycle::SessionIdleInput)
                .await;
        }
    }
    ...
    pub(super) fn sweep_monitor_buffer_into_pending(
        ...
        for event in froq_tools::implementations::grok_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        ) {
            Self::push_pending_notification(
    ...
    fn notification_blocks(
        ...
        use froq_tools::implementations::grok_build::task::types::MonitorEventNotification;
        ...
        if let (Some(index), Some(batch)) = (
            monitor_section_idx,
            froq_tools::reminders::task_completion::format_monitor_events(
                &monitor_events,
