...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            froq_workspace::session::git::get_worktree_info(cwd),
            froq_workspace::session::git::get_branch(cwd),
        );
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<froq_tools::implementations::grok_build::task::types::SubagentOutstandingReply>
    {
    ...
    /// persists + forwards via `send_froq_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_froq_notification_with_extra_meta(
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use froq_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> froq_hooks::event::StopFailureKind {
        use froq_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &froq_agent::plugins::install_registry::InstallError,
    ) -> String {
