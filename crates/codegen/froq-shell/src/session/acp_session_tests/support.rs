...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> froq_computer_hub_sdk::ObservabilityBridge {
    froq_computer_hub_sdk::ObservabilityBridge::new(
        None,
        froq_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> froq_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<froq_sampling_types::HostedTool>,
) -> froq_agent::Agent {
    let base = test_agent_default().await;
    froq_agent::Agent::new(
        base.definition().clone(),
        froq_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        froq_agent::ReminderPolicy::default(),
        froq_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> froq_agent::Agent {
    use froq_tools::implementations::grok_build::update_goal::UpdateGoalTool;
...
#[cfg(test)]
pub(crate) async fn test_grok_build_agent_with_todo() -> froq_agent::Agent {
    use froq_tools::implementations::grok_build::todo::TodoWriteTool;
    use froq_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> froq_agent::Agent {
    use froq_tools::implementations::grok_build::enter_plan_mode::EnterPlanModeTool;
    use froq_tools::implementations::grok_build::exit_plan_mode::ExitPlanModeTool;
    use froq_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<froq_tools::registry::types::ToolConfig>,
) -> froq_agent::Agent {
    test_agent_from_config(
        froq_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        froq_agent::AgentDefinition::default_grok_build(),
        std::sync::Arc::new(froq_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: froq_agent::prompt::user_message::UserMessageTemplate,
) -> froq_agent::Agent {
    let mut definition = froq_agent::AgentDefinition::default_grok_build();
...
async fn test_agent_from_config(
    ...
    definition: froq_agent::AgentDefinition,
    backend: std::sync::Arc<dyn froq_tools::computer::types::TerminalBackend>,
) -> froq_agent::Agent {
    use froq_tools::computer::local::LocalFs;
    use froq_tools::computer::types::AsyncFileSystem;
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<froq_acp_lib::AcpClientMessage>,
    persistence_tx: tokio::sync::mpsc::UnboundedSender<PersistenceMsg>,
) -> (
    SessionActor,
    tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
) {
    let cwd = froq_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(froq_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(froq_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(froq_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: froq_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
...
pub(crate) async fn build_actor() -> (
    ...
    let (gateway_tx, gateway_rx) =
        tokio::sync::mpsc::unbounded_channel::<froq_acp_lib::AcpClientMessage>();
