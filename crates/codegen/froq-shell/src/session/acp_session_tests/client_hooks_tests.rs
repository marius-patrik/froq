...
fn install_client_hook(
    actor: &SessionActor,
    event: froq_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
fn spawn_deny_responder(
    gateway_rx: tokio::sync::mpsc::UnboundedReceiver<froq_acp_lib::AcpClientMessage>,
    reason: &'static str,
) {
    ...
    tokio::task::spawn_local(async move {
        while let Some(msg) = gateway_rx.recv().await {
            match msg {
                froq_acp_lib::AcpClientMessage::ExtMethod(args) => {
                ...
                froq_acp_lib::AcpClientMessage::SessionNotification(args) => {
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                froq_hooks::event::HookEventName::Stop,
                &["cb_0"],
            );
            ...
            actor.fire_hook(
                froq_hooks::event::HookEventName::Stop,
                None,
                froq_hooks::event::HookPayload::Stop {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                froq_tools::registry::types::ToolConfig::for_tool::<
                    froq_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        froq_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
fn file_registry_with_stop_spec(
    event: froq_hooks::event::HookEventName,
    script: &str,
) -> froq_hooks::discovery::HookRegistry {
    let (mut registry, _) = froq_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![froq_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() =
                Some(std::sync::Arc::new(file_registry_with_stop_spec(
                    froq_hooks::event::HookEventName::SubagentStop,
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                froq_hooks::event::HookEventName::SubagentEnd,
                None,
                froq_hooks::event::HookPayload::SubagentStop {
                    phase: froq_hooks::event::SubagentStopPhase::Observe,
