...
fn test_json_round_trip() {
    let ctx = froq_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: froq_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = froq_agent::PromptContext::default();
    ...
    let loaded: froq_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: froq_agent::PromptContext = serde_json::from_str(
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<froq_agent::PromptContext, _> = serde_json::from_str(&json);
