...
/// attribution.
fn auth_error() -> froq_sampler::SamplingErrorInfo {
    froq_sampler::SamplingErrorInfo {
        kind: froq_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: froq_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        froq_chat_state::AuthType::SessionToken => "cached_token",
        froq_chat_state::AuthType::ApiKey => "xai.api_key",
    };
...
/// `ApiKey` (session-token cache miss + `froq_API_KEY`).
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(froq_chat_state::Credentials {
...
/// `froq_API_KEY`) must NOT attempt an OIDC session-token refresh. The
...
fn model_not_found_error() -> froq_sampler::SamplingErrorInfo {
    froq_sampler::SamplingErrorInfo {
...
fn unauthorized_401_error() -> froq_sampler::SamplingErrorInfo {
    froq_sampler::SamplingErrorInfo {
            kind: froq_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.grok.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_froq_token_auth=froq-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
async fn switch_to_first_party_model_drops_minted_provider_token() {
    ...
    local
        .run_until(async {
            ...
            let (actor, _rx) =
                make_actor_with_auth_and_credentials(None, froq_chat_state::AuthType::ApiKey, token)
                    .await;
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = froq_sampler::SamplingErrorKind::Api;
