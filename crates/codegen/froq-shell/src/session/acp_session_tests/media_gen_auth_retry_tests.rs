...
use froq_tools::types::output::{ToolOutput, ToolRunResult};
...
fn ok_result(text: &str) -> Result<ToolRunResult, froq_tool_runtime::ToolError> {
    Ok(ToolRunResult {
...
fn err(msg: &str) -> Result<ToolRunResult, froq_tool_runtime::ToolError> {
...
fn http_err(status: u16, msg: &str) -> Result<ToolRunResult, froq_tool_runtime::ToolError> {
    Err(
        froq_tool_runtime::ToolError::new(froq_tool_runtime::ToolErrorKind::Custom, msg.to_owned())
            .with_details(
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, froq_tool_runtime::ToolError)> = vec![
        ...
        (
            true,
            froq_tool_runtime::ToolError::new(
        ...
        // the inference path's gate in froq-sampling-types/src/error.rs:
        ...
        (
            true,
            froq_tool_runtime::ToolError::invalid_arguments("response: invalid api key for project"),
        ),
        // Fallback path: OAuth 2.0 `invalid_token` payload (RFC 6749)
        // surfaced as raw JSON without a structured status code.
        (
            true,
            froq_tool_runtime::ToolError::invalid_arguments(r#"{"error":"invalid_token"}"#),
        ),
        // Fallback path: case-insensitive "unauthorized" anywhere in
        // the message body.
        (
            true,
            froq_tool_runtime::ToolError::invalid_arguments("UNAUTHORIZED"),
        ),
        // Negative: transport failure must not trigger a token refresh.
        (
            false,
            froq_tool_runtime::ToolError::invalid_arguments("Image generation timed out after 60s"),
        ),
        // Negative: structural not-found error; not a network response.
        (
            false,
            froq_tool_runtime::ToolError::not_found(
                froq_tool_protocol::ToolId::new("image_gen").expect("valid"),
        ...
        (
            false,
            froq_tool_runtime::ToolError::invalid_arguments("request id req_401abc failed"),
        ),
    ];
