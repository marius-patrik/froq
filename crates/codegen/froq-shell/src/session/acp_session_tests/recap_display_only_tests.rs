        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_grok_req_id.starts_with("froq-recap-"),
                        "req id: {}",
...
async fn manual_recap_over_budget_trims_persisted_request_and_is_display_only() {
    ...
    local
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    let est = froq_chat_state::estimate_conversation_tokens(&artifact.chat_history);
                    assert!(
                        est <= PROMPT_BUDGET,
...
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    ...
    use froq_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use froq_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    let msg = froq_sampling_types::build_messages_request(&req);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = froq_sampling_types::ApiBackend::Responses;
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use froq_sampling_types::HostedTool;
    use froq_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = froq_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use froq_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use froq_test_support::MockInferenceServer;
