use super::support::*;
use super::*;
use froq_tools::reminders::task_completion::consumed_completion_ids;
use froq_tools::types::output::{BashOutput, TextOutput, ToolOutput};
use froq_tool_types::{
...
async fn cancel_barrier_rejects_task_completion_wake_without_reporting_it() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                res.get::<froq_tools::types::resources::State<
                    froq_tools::reminders::task_completion::ReportedTaskCompletions,
                >>()
                .is_none(),
                "declined admission must not report before user re-engagement"
            );
            drop(res);
            let reminder = froq_tools::reminders::TaskCompletionReminder;
            let reminders = froq_tools::types::tool::Reminder::collect_reminders(
                &reminder,
                resources,
        ...
        .run_until(async {
            ...
            assert!(
                resources
                    .lock()
                    .await
                    .get::<froq_tools::types::resources::State<
                        froq_tools::reminders::task_completion::ReportedTaskCompletions,
                    >>()
                    .is_none(),
...
async fn genuine_user_start_consumes_deferred_completions_without_notification_turn() {
    ...
    local
        .run_until(async {
            ...
            let body = froq_tools::reminders::task_completion::format_monitor_completion(
                &froq_tools::types::TaskSnapshot {
                    ...
                    signal: None,
                    completed: true,
                    kind: froq_tools::computer::types::TaskKind::Monitor,
                    block_waited: false,
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use froq_tools::reminders::task_completion::ReportedTaskCompletions;
...
struct OneTaskTerminal {
    tasks: Vec<froq_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl froq_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: froq_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        froq_tools::computer::types::TerminalRunResult,
        froq_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<froq_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> froq_tools::computer::types::KillOutcome {
        froq_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<froq_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<froq_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> froq_tools::computer::types::TaskSnapshot {
    froq_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
