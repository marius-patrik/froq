...
fn x_search_cutoff_update() -> froq_sampling_types::ToolOverridesUpdate {
    froq_sampling_types::ToolOverridesUpdate {
        x_search: Some(Some(froq_sampling_types::XSearchOptions {
            date_bound: Some(
                froq_sampling_types::SearchDateBound::new(None, Some("2024-03-15".to_string()))
                    .unwrap(),
            ),
        })),
        web_search: None,
...
async fn tool_overrides_update_applies_at_promotion_never_at_enqueue() {
    ...
    local
        .run_until(async {
            let (actor, _rx) = build_actor().await;
            let options = froq_sampling_types::XSearchOptions {
                date_bound: Some(
                    froq_sampling_types::SearchDateBound::new(
                        None,
            ...
            let set_update = || froq_sampling_types::ToolOverridesUpdate {
                x_search: Some(Some(options.clone())),
                web_search: None,
            };
            let expected = froq_sampling_types::ToolOverrides {
            ...
            actor.apply_tool_overrides_update(Some(froq_sampling_types::ToolOverridesUpdate {
                x_search: Some(None),
                web_search: None,
            }));
            assert_eq!(
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_backend_search(vec![froq_sampling_types::HostedTool::XSearch {
                    options: None,
                }])
                .await;
            ...
            *actor.tool_overrides.borrow_mut() = Some(expected.clone());
            ...
            assert_eq!(
                ...
                vec![froq_sampling_types::HostedTool::XSearch {
...
async fn agent_rebuild_republishes_the_configured_cutoff() {
    ...
    local
        .run_until(async {
            ...
            let seed = froq_sampling_types::ToolOverrides {
                x_search: Some(froq_sampling_types::XSearchOptions {
                    date_bound: Some(
                        froq_sampling_types::SearchDateBound::new(
                            None,
            ...
            let mut seeded = froq_agent::AgentDefinition::default_grok_build();
...
async fn set_tool_overrides_publishes_the_inheritance_cell_before_any_turn() {
    ...
    local
        .run_until(async {
            ...
            let cutoff = froq_sampling_types::ToolOverrides {
                x_search: Some(froq_sampling_types::XSearchOptions {
                    date_bound: Some(
                        froq_sampling_types::SearchDateBound::new(
                            None,
