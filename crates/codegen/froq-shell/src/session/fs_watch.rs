//! Session-level fs-watch policy over [`froq_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `froq_fsnotify`. This module
...
use froq_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> froq_workspace::file_system::FileIndexDelta {
    use froq_workspace::file_system::FileIndexDelta;
...
async fn refresh_codebase_graph_after_head_change(
    ...
    cmd.envs(froq_tools::util::pager_env());
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = froq_workspace::session::git::get_branch(&self.cwd).await;
        let commit = froq_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = froq_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = froq_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: froq_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                froq_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || froq_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = froq_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(froq_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: froq_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
