...
pub use froq_chat_state::compaction_utils::{
...
pub(crate) use froq_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_grok_req_id = Some(format!("froq-compact-{}", uuid::Uuid::new_v4()));
            message.x_grok_session_id = Some(sid);
            message.x_grok_agent_id = Some(froq_telemetry::id::agent_id());
            tracing::info!(
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = froq_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, froq_sampling_types::StopReason::Length);
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                ...
                x_grok_req_id: Some(format!("froq-compact-{}", uuid::Uuid::new_v4())),
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            froq_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            froq_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: froq_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            froq_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, froq_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        froq_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            froq_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            froq_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            froq_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            froq_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            froq_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            froq_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            froq_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            froq_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    /// `froq-chat-state`, rendering the system-reminder synchronously (no
    ...
    async fn test_compacted_history_raw_strings() {
        ...
        let formatted_summary =
            froq_chat_state::compaction_utils::format_compact_summary_content(compaction_summary);
        assert_eq!(
    ...
    fn fallback_minimal_history_has_no_tool_results() {
        use froq_chat_state::compaction_utils::validate_compacted_history;
