...
//! shell-specific types (`froq_tools::MemoryBackend`, memory context).
//!
//! The core [`CompactionStateContext`] struct and its builder live in
//! `froq_chat_state::compaction_utils`. This module adds system-reminder
//! rendering that requires dependencies not available in `froq-chat-state`.
//!
//! The three **common** active-agent sections (background tasks, TODO list,
//! running subagents) are formatted by
//! [`froq_compaction::reminder`] so grok-chat and grok-build stay in lockstep.
...
pub fn to_system_reminder_sync(
    ...
    skills: &[froq_tools::implementations::skills::types::SkillInfo],
...
pub async fn to_system_reminder(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[froq_tools::implementations::skills::types::SkillInfo],
    memory_backend: Option<&dyn froq_tools::types::memory_backend::MemoryBackend>,
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
...
fn to_system_reminder_inner(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[froq_tools::implementations::skills::types::SkillInfo],
    memory_results: &[froq_tools::types::memory_backend::MemorySearchResult],
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
    ...
    if let Some(listing) =
        froq_tools::types::skill_discovery_tracker::format_compaction_skill_listing(skills)
    {
    ...
    if !ctx.connected_mcp_servers.is_empty() {
        use froq_tools::implementations::search_tool::format_compaction_server_line;
