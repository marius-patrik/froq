...
//! (`froq_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](froq_compaction::sample_full_replace_summary);
...
//! [`FullReplaceError`](froq_compaction::FullReplaceError).
...
use froq_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use froq_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    froq_telemetry::session_ctx::log_event(CompactionRetryDegraded {
