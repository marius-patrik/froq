...
// Re-export from froq-chat-state — canonical definition lives there.
pub use froq_chat_state::compaction_utils::extract_user_query;
    ...
    match result {
        Ok(Ok(status)) => {
            ...
            let status = if vcs_kind.is_jj() {
                ...
                froq_agent::prompt::user_message::normalize_git_status(&status)
            };
...
// Tests for extract_user_query now live in froq_chat_state::compaction_utils.

#[cfg(test)]
mod tests {
    use super::*;
    use froq_workspace::file_system::FsError;
