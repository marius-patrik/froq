...
//! `froq-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use froq_file_utils::events::tracker::EventTracker;
pub(crate) use froq_file_utils::events::types::{
...
) -> Option<froq_sampling_types::PriorTurnInterrupt> {
    use froq_sampling_types::PriorTurnInterrupt;
    match category {
...
/// (`froq-file-utils`). The two enums are intentional mirrors and live in
