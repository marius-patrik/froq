...
//! both matched by the agent ([`froq_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use froq_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use froq_telemetry::events::ClientHookGateOutcome;
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| {
            froq_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value)
        })
...
impl SessionActor {
    ...
    fn client_gate_responses<'a>(
        ...
        let default_timeout =
            if envelope.hook_event_name.traits().gate == froq_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                froq_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    froq_telemetry::session_ctx::log_event(
                        froq_telemetry::events::ClientHookGate {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> froq_hooks::dispatcher::StopDispatchResult {
        use froq_hooks::result::HookRunResult;

        let mut out = froq_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed)) in ordered {
            ...
            let detail = froq_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                froq_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use froq_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    froq_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use froq_hooks::matcher::HookMatcher;
