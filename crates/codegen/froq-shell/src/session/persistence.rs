...
pub enum PersistenceMsg {
    ...
    AppendCwdSwitchAndAck {
        ...
        respond_to: tokio::sync::oneshot::Sender<
            Result<froq_chat_state::StrictAppendAck, froq_chat_state::StrictAppendError>,
        >,
    },
...
pub use froq_shared::session::session_dir;
...
impl Summary {
    pub fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            froq_workspace::session::git::resolve_persisted_session_git_metadata_sync(
...
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = froq_config::grok_home().join("logs").join("mcp");
...
pub fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "froq_shell::session::persistence",
