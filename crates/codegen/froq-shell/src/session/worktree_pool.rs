...
use froq_fast_worktree::{WorktreeBuilder, WorktreeSync};

const WORKTREE_POOL_LOG: &str = "froq_worktree_pool";
use crate::util::config::PoolConfig;
use crate::util::grok_home::grok_home;

use froq_tty_utils::git_command;
...
async fn warm_source_repo(repo_path: &Path) {
    ...
    let _ = tokio::task::spawn_blocking(move || {
        ...
        for &(key, val) in froq_tty_utils::GIT_AUTH_SUPPRESSION_ENVS.iter() {
            cmd.env(key, val);
        }
        let result = cmd
...
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(froq_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(froq_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                froq_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || froq_fast_worktree::remove_worktree(&path_owned)).await;
