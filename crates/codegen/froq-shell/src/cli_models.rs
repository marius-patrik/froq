...
impl AuthStatus {
    ...
    /// [`crate::agent::auth_method::should_advertise_froq_api_key`] so
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if crate::agent::auth_method::should_advertise_froq_api_key(
...
mod tests {
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            EnvGuard::unset(froq_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_froq_API_KEY_ENV_VAR),
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(froq_API_KEY_ENV_VAR, "froq-test-key");
    ...
    fn resolve_legacy_api_key_env() {
        let (_dir, _g) = isolate_auth_sources();
        let _key = EnvGuard::set(LEGACY_froq_API_KEY_ENV_VAR, "legacy-key");
        assert_eq!(AuthStatus::resolve(&Config::default()), AuthStatus::ApiKey);
    }
