//! Shim — see `froq_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `froq_shell::instrumentation_timer!`). Keeping the macro here
...
pub use froq_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `froq_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `froq_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `froq_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `froq_telemetry::instrumentation`.
#[macro_export]
