...
/// Resolved absolute path to bash. On Unix uses the `froq_config` shell
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        froq_config::shell::unix_shell_path(froq_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `froq-tools`.
pub use froq_tools::util::pager_env;
...
/// froq-pager which renders its own UI and doesn't need raw ANSI codes).
///
