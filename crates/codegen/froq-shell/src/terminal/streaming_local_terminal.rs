...
/// after we stop waiting. Mirrors the bound the `froq-tools` local terminal
...
#[async_trait::async_trait]
impl SessionNotificationSender for froq_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
...
fn spawn_shell_command(
    ...
    #[cfg(not(unix))]
    {
        let inv = froq_config::shell::shell_command_argv(command);
...
fn spawn_with_argv(
    ...
    #[cfg(unix)]
    {
        ...
        let mut cmd = CommandWrap::with_new(program, |cmd| {
            ...
            #[cfg(target_os = "linux")]
            if froq_sandbox::should_restrict_child_network() {
                // SAFETY: single prctl syscall (async-signal-safe).
                unsafe {
                    cmd.pre_exec(|| froq_sandbox::child_net::install_child_network_filter());
                }
            }
        });
