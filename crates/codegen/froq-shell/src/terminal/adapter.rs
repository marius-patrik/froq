//! AcpTerminalAdapter: implements `froq-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates froq-tools' `TerminalBackend` trait into ACP protocol calls:
...
/// Wraps froq-shell's ACP gateway to satisfy froq-tools' TerminalBackend.
