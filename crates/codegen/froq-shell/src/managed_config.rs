...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    froq_config::MANAGED_CONFIG_FILENAME,
    froq_config::REQUIREMENTS_FILENAME,
    froq_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    froq_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            froq_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", froq_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            froq_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `froq-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = froq_config::signed_policy::stored_envelope_nonce(
        &crate::util::grok_home::grok_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            froq_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
fn apply_fetched(
    ...
    let verified = if froq_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(froq_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        froq_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(froq_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            froq_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(froq_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &froq_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<froq_config::signed_policy::SignatureEnvelope> {
    use froq_config::signed_policy::now_unix;
    ...
    let claim = match froq_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &froq_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
fn bump_managed_rollback_floor() {
    ...
    if !froq_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::grok_home::grok_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            froq_config::bump_rollback_floor(&home);
        }
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    if source.is_some()
        && froq_config::signed_policy::verification_active()
