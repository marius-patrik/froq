...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // froq_ROOT/target/tmp (which is read-only inside the sandbox). Outside
    // Bazel, prefer froq_ROOT's shared cache dir (monorepo behavior) and fall
    // back to OUT_DIR for standalone checkouts where froq_ROOT is not a thing.
    ...
    } else if let Ok(froq_root) = env::var("froq_ROOT") {
        PathBuf::from(froq_root).join("target/tmp/grok-shell-bundle-rg")
    } else {
        PathBuf::from(env::var("OUT_DIR")?)
    };
