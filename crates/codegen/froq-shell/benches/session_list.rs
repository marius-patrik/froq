...
//!
//! Run: `cargo bench -p froq-shell --bench session_list`
...
use froq_fast_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use froq_shell::session::info::Info;
use froq_shell::session::persistence::Summary;
use froq_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use froq_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = froq_shell::util::grok_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = froq_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = froq_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(froq_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn bench_session_list(c: &mut Criterion) {
    ...
    assert_eq!(froq_shell::util::grok_home::grok_home(), home.path());
