...
//! script the test writes to disk). The harness's `froq_API_KEY` stands in for
...
//! ```bash
//! cargo test -p froq-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
"#,
    ...
    // `test-key-for-ci` is the harness's froq_API_KEY; it stands in for the
