...
use froq_acp_lib::{AcpAgentGatewaySender, AcpClientMessage};
...
/// `froq-config` only allows one value per process, so all tests share
...
async fn dont_ask_policy_denies_without_prompting() {
    use froq_workspace::permission::types::{PermissionConfig, PromptPolicy};
