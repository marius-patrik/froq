...
use froq_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as XaiSessionUpdate,
};
use froq_shell::session::info::Info as SessionInfo;
use froq_shell::session::persistence::default_model_id;
use froq_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that xAI session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_froq_session_notification_storage_roundtrip() {
    ...
    let froq_notification = SessionNotification {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Xai(Box::new(froq_notification.clone())),
        )
        .await
        .unwrap();
...
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(froq_notification)))
        .await
        .unwrap();
...
#[test]
fn test_froq_session_notification_serialization() {
