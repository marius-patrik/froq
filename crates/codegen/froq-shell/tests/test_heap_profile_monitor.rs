...
use froq_shell::auth::{AuthManager, AuthMode, GrokAuth, GrokComConfig};
use froq_shell::heap_profile::{
...
use froq_test_support::{EnvGuard, MockInferenceServer};
use froq_version::TEST_VERSION_ENV;
...
fn assert_storage_auth(uploads: &[froq_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
