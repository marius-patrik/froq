...
//!
//! Run:
//!   cargo test -p froq-shell --test session_load_perf -- --nocapture
//!   cargo test -p froq-shell --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use froq_shell::session::info::Info;
use froq_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use froq_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
async fn full_session_load_e2e() {
    ...
    let server = froq_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    unsafe {
        ...
        std::env::set_var("GROK_froq_API_BASE_URL", server.url());
        std::env::set_var("froq_API_KEY", "test-key-for-ci");
    ...
    let _ = tracing_subscriber::registry()
        .with(froq_shell::instrumentation::layer::<Registry>())
        .try_init();
    ...
    local
        .run_until(async move {
            ...
            let _ = froq_shell::instrumentation::finalize();
