...
use froq_shell::config::ServingIdentity;
use froq_test_support::spawn_counting_server;
...
fn write_team_auth_expiry(home: &std::path::Path, team_id: &str, expires_at: &str) {
    let scope = froq_shell::auth::GrokComConfig::default().auth_scope();
...
async fn team_sync_writes_files() {
    ...
    let wrote = froq_shell::managed_config::sync()
        .await
...
async fn marker_dir_squat_is_cleared_and_marker_written() {
    ...
    let marker_path = home.join(froq_config::MANAGED_CONFIG_CACHE_FILE);
...
async fn padded_team_id_is_one_identity() {
    ...
    assert_eq!(
        froq_shell::managed_config::current_serving_identity(),
        team_identity("team-007"),
        "the serving identity must be the trimmed team id"
    );
    froq_shell::managed_config::sync()
        .await
        .expect("sync should succeed");
    let marker =
        std::fs::read_to_string(home.join(froq_config::MANAGED_CONFIG_CACHE_FILE)).unwrap();
    let v: serde_json::Value = serde_json::from_str(&marker).unwrap();
    assert_eq!(
        v["principal"].as_str(),
        Some("team-007"),
    ...
    assert_eq!(
        froq_config::confirmed_team_switch("team-007"),
        None,
...
async fn team_switch_evicts_prior_teams_policy() {
    ...
    assert!(!froq_shell::config::is_managed_config_stale_for(
...
async fn withdrawn_artifact_is_removed_on_next_sync() {
    ...
    assert!(
        !froq_shell::config::is_managed_config_stale_for(&team_identity("team-007")),
...
async fn empty_dk_response_with_failing_team_leaves_team_policy_intact() {
    ...
    let err = froq_shell::managed_config::sync()
        .await
...
async fn served_then_deleted_refetches_best_effort() {
    ...
    let auth_manager = std::sync::Arc::new(froq_shell::auth::AuthManager::new(
        &home,
        froq_shell::auth::GrokComConfig::default(),
    ));
    froq_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
    assert!(
        home.join("requirements.toml").exists(),
...
async fn expired_refreshable_team_token_heals_after_auth_refresh() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        !froq_shell::managed_config::has_principal(),
...
async fn managed_policy_gate_fails_closed_on_deleted_policy_offline() {
    ...
    assert!(
        froq_shell::managed_config::managed_policy_gate().is_ok(),
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        froq_shell::managed_config::managed_policy_gate().is_err(),
    ...
    write_config(&home, &url);
    write_team_auth(&home, "team-007");
    froq_shell::config::mark_managed_config_synced(froq_shell::config::SyncMarker {
...
async fn bootstrap_fails_closed_when_managed_policy_compromised() {
    ...
    let cfg = froq_shell::agent::config::Config::default();
    let auth_manager = std::sync::Arc::new(froq_shell::auth::AuthManager::new(
        &home,
        froq_shell::auth::GrokComConfig::default(),
    ));
    // `bootstrap`'s Ok type isn't `Debug`, so match rather than `expect_err`.
    let err = match froq_shell::agent::init::bootstrap(&cfg, &auth_manager, None) {
        Err(e) => e,
        Ok(_) => {
...
async fn managed_policy_gate_fails_closed_on_deployment_key_switch_offline() {
    ...
    let gate_same_key = froq_shell::managed_config::managed_policy_gate();
    ...
    let gate_switched_key = froq_shell::managed_config::managed_policy_gate();
...
async fn identity_change_permits_offline_team_switch_and_purges_prior_team() {
    ...
    for f in froq_shell::managed_config::MANAGED_ARTIFACT_FILES
        .into_iter()
        .chain([froq_config::MANAGED_CONFIG_CACHE_FILE])
    {
        assert!(
            !home.join(f).exists(),
...
async fn gate_purge_retries_past_a_transient_lock_holder() {
    ...
    assert!(
        !home
            .join(froq_config::MANAGED_CONFIG_CACHE_FILE)
            .exists(),
...
async fn fail_closed_env_cannot_disarm_the_gate() {
    ...
    let gate = froq_shell::managed_config::managed_policy_gate();
    unsafe {
        std::env::remove_var("GROK_MANAGED_CONFIG");
...
async fn logout_clears_team_config() {
    ...
    froq_shell::managed_config::clear_orphan();
...
async fn setup_lock_skip_is_not_reported_as_no_config() {
    ...
    let outcome = froq_shell::managed_config::run_setup().await;
    lock.unlock().unwrap();

    assert!(
        matches!(
            outcome,
            froq_shell::managed_config::SetupOutcome::Skipped
        ),
...
async fn shared_client_reuses_pooled_connection() {
    ...
    let client = froq_shell::http::shared_client();
...
async fn managed_config_opt_out_makes_no_requests() {
    ...
    unsafe { std::env::set_var("GROK_MANAGED_CONFIG", "0") };
    let outcome = froq_shell::managed_config::post_login_sync(None).await;
    unsafe { std::env::remove_var("GROK_MANAGED_CONFIG") };

    assert_eq!(
        outcome,
        froq_shell::managed_config::ManagedConfigSync::Skipped
    );
...
async fn post_login_pins_authenticated_team_over_disk() {
    ...
    let pinned: froq_shell::auth::GrokAuth = serde_json::from_value(serde_json::json!({
        "key": "pinned-token",
        "auth_mode": "oidc",
        "create_time": "2026-01-01T00:00:00Z",
        "expires_at": FAR_FUTURE,
    ...
    let outcome = froq_shell::managed_config::post_login_sync(Some(pinned)).await;
    assert_eq!(
        outcome,
        froq_shell::managed_config::ManagedConfigSync::Updated { is_team: true }
    );
    assert_eq!(
        auths.lock().unwrap().last().map(String::as_str),
...
async fn post_login_sync_is_latency_bounded() {
    ...
    assert_eq!(
        outcome,
        froq_shell::managed_config::ManagedConfigSync::Failed
    );
    assert_eq!(
        *count.lock().unwrap(),
        2,
...
async fn purge_crash_prefixes_stay_armed_and_converge() {
    let home = test_home().clone();
    let artifacts = froq_shell::managed_config::MANAGED_ARTIFACT_FILES;
    ...
    for prefix_len in 0..=artifacts.len() {
        ...
        assert!(
            home.join(froq_config::MANAGED_CONFIG_CACHE_FILE)
                .exists(),
        ...
        write_team_auth(&home, "team-b");
        assert_eq!(
            froq_config::confirmed_team_switch("team-b").as_deref(),
            Some("team-a"),
...
async fn contended_sync_writes_no_marker() {
    ...
    let lock = std::fs::OpenOptions::new()
        ...
        .unwrap();
    lock.lock().unwrap();
    let synced = froq_shell::managed_config::sync()
        .await
...
async fn dk_synced_marker_survives_config_blip_with_team_signed_in() {
    ...
    write_team_auth(&home, "team-b");
    assert_eq!(
        froq_config::confirmed_team_switch("team-b"),
        None,
