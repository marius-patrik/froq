...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p froq-shell --test test_leader_soak -- --ignored --nocapture
//! ```
...
/// Copied from `froq-codebase-graph/tests/memory_integration.rs`.
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    local
        .run_until(async {
            ...
            // the same wiring: froq-pager/src/app/leader_cluster/mod.rs
