...
//! ```bash
//! cargo test -p froq-shell --test test_stop_hook_e2e -- --ignored
//! ```

use froq_test_support::env::test_env_cmd_tokio;
...
async fn stop_block_loop_ends_at_continuation_cap() {
    ...
    assert_eq!(
        run.invocations(),
        froq_shell::session::MAX_STOP_HOOK_CONTINUATIONS_PER_TURN,
