//! Built-binary end-to-end tests for the grok (froq-pager) binary.
//!
...
//! ```bash
//! cargo test -p froq-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
...
async fn test_headless_json_reports_server_cost() {
    use froq_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        froq_test_support::scripted::ScriptedResponse::sse(vec![
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        froq_test_support::scripted::ScriptedResponse::sse(
            froq_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
