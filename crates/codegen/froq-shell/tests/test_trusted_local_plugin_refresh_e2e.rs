...
//!
//! ```bash
//! cargo test -p froq-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p froq-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    assert_eq!(
        plugin.scope,
        froq_agent::plugins::PluginScope::CliOverride
    );
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    .kill_on_drop(true);
    froq_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), &home);
    cmd.env("HOME", &home);
