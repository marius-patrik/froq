...
//! Run locally:
//! ```bash
//! cargo test -p froq-shell --test test_agent_type_invariant -- --ignored
//! ```
...
async fn test_grok_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            froq_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            ...
            let incoming = froq_acp_lib::LineBufferedRead::spawn_local(incoming);
