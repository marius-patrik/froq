...
/// All tests share this as GROK_HOME (the `OnceLock` in froq-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = froq_shell::config::load_effective_config().unwrap();
    let effective_cfg = froq_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    froq_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = froq_shell::util::config::load_config_from_toml(&user_toml);
