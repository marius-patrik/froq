...
use froq_tools::util::grok_home::grok_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `froq_agent::config::MemoryScope` (agent memory dir).
...
/// named `{project-slug}-{hash8}`, e.g. `~/.grok/memory/froq-a3f7b2c9/`.
#[derive(Debug, Clone)]
...
impl MemoryStorage {
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see froq_sqlite_journal::JournalMode::open_readonly.
        froq_sqlite_journal::JournalMode::for_db_path(&db_path)
