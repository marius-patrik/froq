...
use froq_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use froq_pager::client_identity::PAGER_CLIENT_VERSION;
use froq_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use froq_shell::agent::config::Config as AgentConfig;
use froq_shell::leader::{
...
use froq_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_agent_endpoint_args(
    agent_args: &froq_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.froq_api_base_url {
        config.endpoints.froq_api_base_url = v.clone();
    }
}
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use froq_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(froq_telemetry::sampling_log::layer())
        .with(froq_telemetry::instrumentation::layer())
        .with(froq_telemetry::hooks_log::layer())
        .with(froq_telemetry::otel_layer::build_otel_layer(
            froq_telemetry::otel_layer::OtelClientInfo {
                client_name: "grok-pager",
                client_version: froq_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            froq_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    froq_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    froq_telemetry::external::init(
        froq_shell::agent::config::resolve_external_otel_config(
            froq_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: froq_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `grok setup`: rendering + exit codes only; fetch logic lives in `froq_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use froq_shell::managed_config::{self, SetupOutcome};
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = froq_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !froq_shell::util::is_grok_process(pid) {
        ...
        if let Err(e) = froq_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(froq_shell::env::GrokBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, froq_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = froq_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = froq_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&froq_shell::leader::ControlPayload>,
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&froq_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<froq_shell::util::config::RemoteSettings> {
    join_early_prefetch(froq_shell::agent::models::start_early_prefetch(None))
}
...
async fn workspace_control(
    ...
    let raw_config = froq_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use froq_shell::auth::ensure_authenticated;
    froq_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = froq_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    froq_telemetry::sentry::flush_on_shutdown();
    froq_telemetry::otel_layer::shutdown_otel();
    froq_telemetry::debug_log::flush();
...
async fn run_agent_command(
    agent_args: Box<froq_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use froq_pager::app::signal_handler::next_signal_code;
    ...
    let _otel_guard = froq_telemetry::otel_layer::otel_guard();
    froq_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => froq_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(error = %e, "--trust: failed to resolve cwd; folder not trusted")
            }
        }
    }
    let early_prefetch = froq_shell::agent::models::start_early_prefetch(None);
    froq_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Grok Build (pager) - v{}",
            froq_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                froq_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(froq_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = froq_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = froq_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&froq_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &froq_shell::util::grok_home::grok_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = froq_shell::leader::LeaderEnvUrls::from(&agent_config.grok_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = froq_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                            biased;
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = froq_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            froq_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(froq_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = froq_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = froq_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !froq_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<froq_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(froq_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<froq_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(froq_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    froq_shell::heap_profile::install(froq_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "frog-build {}\n",
        froq_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), channel_label,)
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = froq_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    if let Some(code) = froq_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = froq_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    froq_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    froq_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        froq_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        froq_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    froq_pager::memory_trace::start(
        froq_shell::util::grok_home::grok_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = froq_config::validate_requirements() {
    ...
    let _sentry_guard = froq_telemetry::sentry::init(froq_telemetry::sentry::Config {
        client: "grok-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: froq_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    froq_pager::docs::extract_user_guide_docs(&froq_shell::util::grok_home::grok_home());
    froq_crash_handler::install_terminal_restore_only();
    if froq_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = froq_shell::util::grok_home::grok_home().join("crash");
        if let Some(report) = froq_crash_handler::check_previous_crash(&crash_dir) {
        ...
        if !froq_crash_handler::install(froq_crash_handler::CrashHandlerConfig {
    ...
    let crashed = froq_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    froq_telemetry::debug_log::flush();
    if let Err(e) = result {
        froq_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if args.chat() {
        unsafe {
            std::env::set_var(froq_shell::agent::chat_modes::GROK_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(froq_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    ...
    if let Some(Command::Completions { shell }) = &args.command {
        froq_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return froq_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        froq_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        froq_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    froq_shell::config::apply_sandbox(
        None,
    ...
    froq_shell::http::set_client_name(if is_interactive {
        froq_workspace::permission::ClientType::GrokPager
    } else {
        froq_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": froq_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
            ...
            Command::Inspect { json } => {
                ...
                froq_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return froq_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = froq_telemetry::otel_layer::otel_guard();
                return froq_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = froq_telemetry::otel_layer::otel_guard();
                let config = froq_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return froq_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return froq_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return froq_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return froq_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return froq_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return froq_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return froq_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                froq_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                froq_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = froq_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                froq_shell::auth::run_cli_logout(&config)?;
                froq_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = froq_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(froq_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == froq_pager::headless::OutputFormat::Plain
        {
            args.output_format = froq_pager::headless::OutputFormat::Json;
        }
        return froq_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            froq_pager::headless::HeadlessOptions {
    ...
    let result = froq_pager::app::run(args, bg_update_rx).await;
    froq_sandbox::flush();
    match result {
        Ok(true) => {
...
fn build_update_config() -> UpdateConfig {
    let environment = froq_shell::env::GrokBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                froq_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("GROK_NPM_REGISTRY"))
        .ok()
        .or_else(froq_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = froq_shell::config::load_effective_config_disk_only()
        && let Some(ch) = froq_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn is_managed_install(exe: Option<std::path::PathBuf>, grok_home: &std::path::Path) -> bool {
    ...
    let managed = froq_config::grok_application_in(grok_home);
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in froq_shell::leader::discover_leaders().await {
        if d.classification != froq_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match froq_shell::leader::LeaderClient::connect(
            socket_path,
            "grok-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(froq_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(froq_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn version_output_writer_preserves_channel_aware_contract() {
        for (label, expected_suffix) in [
            ...
            assert!(output.starts_with("frog-build "));
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            froq_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: froq_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            froq_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!froq_shell::heap_profile::prof_available());
            return;
        }
        assert!(froq_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(froq_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(froq_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        froq_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    ...
    fn workspace_command_gate_resolution() {
        use froq_shell::util::config::RemoteSettings;
