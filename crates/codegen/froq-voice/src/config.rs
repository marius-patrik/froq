...
/// `[endpoints].froq_api_base_url` so enterprise proxies need no second knob.
...
impl VoiceConfig {
    ...
    /// `api_base`: non-empty `[voice].api_base`, else `[endpoints].froq_api_base_url`
    ...
    pub fn from_config_table(root: &toml::Table, resolved_endpoints_base: Option<&str>) -> Self {
        ...
        .or_else(|| {
            non_empty_str(
                root.get("endpoints")
                    .and_then(|e| e.get("froq_api_base_url"))
                    .and_then(|v| v.as_str()),
            )
        })
...
mod tests {
    ...
    #[test]
    fn froq_v1_base_preserves_prefix() {
        let cfg = VoiceConfig {
    ...
    fn inherits_endpoints_when_voice_api_base_unset() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
froq_api_base_url = "https://proxy.example.com/xai/v1"
"#,
        )
        .unwrap();
...
"#,
    ...
    fn table_endpoints_beat_resolved_endpoints_base() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
froq_api_base_url = "https://config.example.com"
"#,
        )
        .unwrap();
