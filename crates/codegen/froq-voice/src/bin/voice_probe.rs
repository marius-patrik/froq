...
//! ```bash
//! export froq_API_KEY=...
//! cargo run -p froq-voice --bin voice-probe -- --seconds 5
//! ```
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(code) = froq_voice::maybe_run_capture_subprocess() {
...
async fn run() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,froq_voice=debug".into()),
        )
        .init();

    let args = parse_args(std::env::args().skip(1).collect());

    let auth = std::env::var("froq_API_KEY")
        .ok()
        .and_then(StaticVoiceAuth::shared)
        .ok_or_else(|| {
            anyhow::anyhow!("set froq_API_KEY (standalone probe has no login session)")
        })?;
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                froq_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("GROK_froq_API_BASE_URL").ok();
...
Environment:
  froq_API_KEY     required
  RUST_LOG        optional (default info,froq_voice=debug)
