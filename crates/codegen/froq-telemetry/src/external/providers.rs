...
//! `Authorization`/`X-froq-Token-Auth`/`x-userid`;
...
mod tests {
    ...
    /// `X-froq-Token-Auth`, `x-userid`, or `x-teamid` unless customer-supplied
    ...
    fn exporter_headers_are_exactly_customer_headers() {
        ...
        for forbidden in ["Authorization", "X-froq-Token-Auth", "x-userid", "x-teamid"] {
            assert!(
    ...
    fn exporter_metadata_is_customer_headers_only() {
        ...
        for forbidden in ["x-froq-token-auth", "x-userid", "x-teamid"] {
