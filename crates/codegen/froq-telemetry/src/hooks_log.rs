...
//! targets `froq_hooks` or `froq_agent::plugins` at key lifecycle
...
/// Filters to hooks (`froq_hooks`) and plugins (`froq_agent::plugins`) targets.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "froq_hooks=debug,froq_agent::plugins=debug",
    );
