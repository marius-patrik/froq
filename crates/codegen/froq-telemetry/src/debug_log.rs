...
///
/// Lives here (not in `froq-pager`) so the firehose directives below and
...
/// safe. See `froq-pager/src/tracing.rs` for the consumer side.
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,froq_pager=debug,froq_shell=debug,froq_tools=debug,froq_telemetry=debug,froq_agent=debug,froq_mcp=debug,froq_acp_lib=debug,sampling_log=off";
...
mod tests {
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "froq_shell", "inside session");
            });
            tracing::info!(target: "froq_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `froq_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "froq_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "froq_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "froq_shell", "one first");
                tracing::info!(target: "froq_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "froq_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
