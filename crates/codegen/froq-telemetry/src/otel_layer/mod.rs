...
//!
//! `froq-pager` uses this module to set up OTLP
...
///
/// Built by the binaries (`froq-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `froq_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-froq-Token-Auth` header (typically `"froq-cli"`).
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-froq-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &froq_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-froq-Token-Auth".to_string(), value.to_string());
    }
...
mod tests {
    ...
    use std::sync::Mutex;
    use froq_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("froq-cli"),
