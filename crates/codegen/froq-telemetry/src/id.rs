...
//!
//! Extracted from `froq-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$GROK_HOME` is resolved through `froq-config::grok_home`.
...
fn load_or_compute_agent_id() -> String {
    let cache_path = froq_config::grok_home().join("agent_id");
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    froq_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
/// Returns true when workspace marker env vars (`froq_ROOT` and `froq_USER`) are set.
///
...
pub fn has_workspace_env_markers() -> bool {
    std::env::var("froq_ROOT").is_ok() && std::env::var("froq_USER").is_ok()
}
