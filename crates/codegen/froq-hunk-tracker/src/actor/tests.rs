...
impl TestHarness {
    ...
    fn populate(&self, files: usize, files_per_dir: usize) {
        froq_test_utils::git::write_fanout_tree(&self.working_dir, files, files_per_dir);
    ...
    fn feature_branch(&self, picks: usize) -> String {
        froq_test_utils::git::make_feature_branch(&self.working_dir, picks)
    }
}
...
async fn refresh_storm_scan_count_during_rebase() {
    ...
    let t = Instant::now();
    froq_test_utils::git::run_git_with_env(
        &wd,
        &["rebase", "-i", &base],
