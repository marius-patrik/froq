...
//! terminal's *native* scrollback (via `froq_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `froq-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`froq_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`froq-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `froq-pager`.
///
...
/// subsequent calls are ignored (see [`froq_pager::minimal_hook`]).
pub fn install() {
    froq_pager::minimal_hook::install(froq_pager::minimal_hook::MinimalHooks { draw });
}
