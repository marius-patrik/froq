...
use froq_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (froq_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    froq_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use froq_shell::tools::{TodoItem, TodoPriority};
