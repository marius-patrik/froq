...
use froq_pager::app::PagerTerminal;
use froq_pager::app::app_view::{ActiveView, AppView};
use froq_pager::minimal_api;
use froq_pager::render::Renderable;
use froq_pager::scrollback::state::ScrollbackState;
use froq_pager::scrollback::wrappers::EntryRenderer;
use froq_pager::theme::Theme;
use froq_pager::views::prompt_widget::PromptStyle;
use froq_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &froq_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut froq_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        froq_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub(super) fn prompt_style(
    appearance: &froq_pager::appearance::AppearanceConfig,
) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    froq_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        agent.active_pane = froq_pager::app::agent_view::AgentPane::Prompt;
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                froq_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
            froq_pager::views::btw_overlay::btw_panel_height(
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            froq_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(froq_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a froq_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &froq_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_advance_phase_timer(
    agent: &mut froq_pager::app::agent_view::AgentView,
) -> Option<froq_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &froq_pager::app::agent_view::AgentView,
    activity: &Option<froq_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = froq_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = froq_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use froq_pager::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = froq_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<froq_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> froq_pager::app::agent_view::AgentView {
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use froq_pager::app::agent::AgentState;
        use froq_pager::scrollback::RenderBlock;
        use froq_pager::scrollback::entry::ScrollbackEntry;
        use froq_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &froq_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use froq_pager::acp::tracker::TurnActivity;
        use froq_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        froq_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        froq_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            froq_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(froq_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &froq_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use froq_pager::app::app_view::PendingAction;
        use froq_pager::input::key::KeyShortcut;
