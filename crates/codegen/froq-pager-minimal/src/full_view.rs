...
use froq_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = froq_pager::appearance::cache::load_show_thinking_blocks();
        ...
        froq_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: froq_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(froq_pager::scrollback::block::RenderBlock::system(
        ...
        Err(e) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                agent.scrollback.push_block(
                    froq_pager::scrollback::block::RenderBlock::system(format!(
...
mod tests {
    use super::*;
    use froq_pager::scrollback::block::RenderBlock;
