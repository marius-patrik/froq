...
//! image bytes.
//!
//! Both the TUI ([`froq_pager::prompt_images`]) and the server-side
...
//! [`froq_pager::prompt_images::display_text`]. The shape of this
...
/// position (the two diverge — see `AttachedImages` in `froq-tools`).
...
/// [`AttachedImages`](froq_tools::types::resources::AttachedImages))
...
///   separator — see [`froq_pager::prompt_images::display_text`].
...
/// [`froq_pager::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    froq_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// `froq_pager::prompt_images::display_text`. A
