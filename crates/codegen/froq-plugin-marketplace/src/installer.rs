...
use froq_agent::plugins::manifest::{ManifestLoadResult, load_manifest, name_from_dirname};
...
fn clone_repo_to_path(
    ...
    let mut cmd = froq_tty_utils::git_command();
...
mod tests {
    ...
    fn transactional_update_save_failure_rolls_back_install_and_disk_registry() {
        with_test_registry(|registry| {
            ...
            unsafe { std::env::set_var("froq_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE", "1") };
            let result = update_from_marketplace_entry_transactional(
                marketplace.path(),
                &entry,
                provenance(marketplace.path(), "plugins/demo"),
            ...
            unsafe { std::env::remove_var("froq_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE") };
