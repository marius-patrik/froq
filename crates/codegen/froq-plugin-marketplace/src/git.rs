...
pub fn sync_source_cache_with_mode(
    ...
    let url = froq_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(froq_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    froq_config::grok_home().join("marketplace-cache")
}
...
mod tests {
    ...
    fn cache_hash_is_deterministic() {
        let url = "https://github.com/xai-org/froq-plugin-marketplace.git";
