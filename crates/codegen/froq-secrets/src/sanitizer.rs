...
/// Vendor API keys with `sk-`/`sk_` prefixes and xAI (`froq-`) keys. `\b`-anchored so
/// `task-`/`disk-`/`risk-` don't fold a stray `sk-`.
static API_KEY_PREFIX_REGEX: LazyLock<Regex> =
    LazyLock::new(|| compile(r"\b(?:sk[-_]|froq-)[A-Za-z0-9_-]{20,}"));
...
mod tests {
    ...
    fn redacts_known_secret_shapes() {
        let cases = [
            (
                fixture(&["key: froq-", "abc123XYZdef456GHIjkl789"]),
                "xai api key",
            ),
            (
