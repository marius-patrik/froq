...
//! `froq-workspace`. Wire types live in
//! `froq_workspace_types::rpc`; this crate adds the connected-state
...
use froq_computer_hub_sdk::harness::ToolHarness;
use froq_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use froq_workspace_types::rpc::code_nav::{
...
use froq_workspace_types::rpc::fs::{
...
use froq_workspace_types::rpc::git::{
...
use froq_workspace_types::rpc::hunks::{
...
use froq_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use froq_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use froq_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
use froq_workspace_types::rpc::workspace::{
...
use froq_workspace_types::rpc::worktree::{
...
use froq_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
use froq_tool_runtime::{ToolCallContext, ToolStreamItem, TypedToolOutput};
...
pub async fn consume_stream_terminal(
    stream: &mut froq_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, froq_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](froq_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &froq_tool_runtime::ToolError) -> bool {
    match err.kind {
        froq_tool_runtime::ToolErrorKind::NetworkError => true,
        froq_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = froq_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    ...
    use froq_computer_hub_sdk::harness::LocalRegistry;
    use froq_workspace_types::rpc::skills::SkillScope;
    use froq_tool_protocol::{SessionId, ToolId};
    use froq_tool_runtime::{Tool, ToolError};
    ...
    impl froq_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::froq_tool_runtime::ListToolsContext) -> ToolDescription {
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = froq_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: froq_tool_runtime::ToolStream<TypedToolOutput> =
            froq_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = froq_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
