...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `froq-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `froq-shell::session::events`.
    ...
    /// discriminator constants in `froq-shell::session::events`.
    ...
    /// `froq-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `froq-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `froq-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `froq-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `froq-shell`'s `GoalPauseReason`. The two types
