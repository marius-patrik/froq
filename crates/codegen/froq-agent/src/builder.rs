...
use std::sync::Arc;
use froq_tools::bridge::ToolBridge;
use froq_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use froq_tools::notification::ToolNotificationHandle;
use froq_tools::registry::types::SessionContext;
...
/// tool registry in `froq-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    froq_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    parent_scheduler_handle:
        Option<froq_tools::implementations::grok_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn froq_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: froq_tools::implementations::web_search::WebSearchConfig,
    ...
    lsp: Option<std::sync::Arc<dyn froq_tools::implementations::lsp::LspBackend>>,
    ...
    app_builder_deployer_config:
        froq_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: froq_tools::types::compat::CompatConfig,
    ...
    attribution_callback: Option<froq_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in froq-shell).
...
fn ensure_plan_mode_tools(tool_config: &mut froq_tools::registry::types::ToolServerConfig) {
...
fn merge_tool_params(
    tool_config: &mut froq_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(froq_tools::computer::local::LocalFs),
            notification_handle,
            ...
            system_reminder_tag: froq_tools::reminders::DEFAULT_REMINDER_TAG,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<froq_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn froq_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: froq_tools::implementations::grok_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: froq_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    pub fn with_web_fetch_config(
        mut self,
        config: froq_tools::implementations::grok_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn froq_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: froq_tools::implementations::grok_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: froq_tools::implementations::grok_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: froq_tools::implementations::grok_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: froq_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            if self.memory_backend.is_some() {
                use froq_tools::implementations::memory;
                tool_config
                    .tools
            ...
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&froq_tools::implementations::grok_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&froq_tools::implementations::grok_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&froq_tools::implementations::grok_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&froq_tools::implementations::grok_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&froq_tools::implementations::grok_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&froq_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let grok_build_ns = froq_tools::types::tool::ToolNamespace::GrokBuild.to_string();
            let mem_search_id = format!(
                "{grok_build_ns}:{}",
                froq_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{grok_build_ns}:{}",
                froq_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                froq_tools::types::tool::ToolNamespace::GrokBuild,
            );
        ...
        if task_stripped {
            use froq_tools::types::tool::ToolNamespace;
        ...
        if let froq_tools::implementations::grok_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                froq_tools::types::resources::TruncationCfg(
                    froq_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(froq_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(froq_sampling_types::HostedTool::XSearch { options: None });
            }
            froq_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
/// CLI naming for the shared [`froq_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: froq_tool_types::TaskToolNaming<'static> = froq_tool_types::TaskToolNaming {
...
/// CLI [`froq_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: froq_tool_types::SubagentToolNaming<'static> =
    froq_tool_types::SubagentToolNaming {
...
/// shared [`froq_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => froq_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => froq_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => froq_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`froq_tool_types::SubagentDescriptor`] and defers to
/// [`froq_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<froq_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            froq_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = froq_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        froq_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: froq_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(froq_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(froq_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: froq_tools::types::config_source::ConfigSource::Project {
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use froq_tools::types::template_renderer::TemplateRenderer;
        use froq_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use froq_tools::computer::local::LocalTerminalBackend;
        use froq_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use froq_tools::computer::local::LocalTerminalBackend;
        use froq_tools::implementations::grok_build::ask_user_question::AskUserQuestionParams;
        use froq_tools::notification::ToolNotificationHandle;
        use froq_tools::types::resources::Params;
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use froq_tools::computer::local::LocalTerminalBackend;
        use froq_tools::implementations::grok_build::bash::BashParams;
        use froq_tools::notification::ToolNotificationHandle;
        use froq_tools::types::resources::Params;
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use froq_tools::computer::local::LocalTerminalBackend;
        use froq_tools::implementations::grok_build::web_fetch::WebFetchConfig;
        use froq_tools::implementations::web_search::WebSearchConfig;
        use froq_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_grok_build();
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<froq_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use froq_tools::computer::local::LocalTerminalBackend;
        use froq_tools::implementations::web_search::WebSearchConfig;
        use froq_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, froq_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, froq_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = froq_sampling_types::XSearchOptions {
            date_bound: Some(
                froq_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(froq_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&froq_sampling_types::HostedTool::XSearch {
