...
//! live in `froq_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use froq_tools::implementations::skills::types::skill_name_from_path;
pub use froq_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `froq_tools` directly.
pub use froq_tools::types::compat::CompatConfig;

use froq_tools::implementations::skills::discovery::{
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &froq_tools::util::grok_home::grok_home(),
        compat,
    )
    .await;
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            froq_tools::types::config_source::ConfigSource::ConfigToml {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill.body.as_ref().filter(|b| !b.is_empty()).map(|body| {
        froq_tools::implementations::skills::skill::build_skill_message(skill, body)
    })
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || froq_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match froq_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(froq_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        assert!(
            matches!(
                ...
                Some(froq_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    // these functions now live in froq-tools::implementations::skills::discovery
    // and froq-tools::types::skill_discovery_tracker, tested there.
