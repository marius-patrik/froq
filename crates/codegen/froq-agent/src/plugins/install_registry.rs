...
/// Lives here (not in froq-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MarketplaceProvenance {
    ...
    /// Plugin subdirectory within marketplace (e.g., "plugins/froq-code-review").
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("froq_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    pub fn resolve_install_dir() -> PathBuf {
        ...
        froq_config::grok_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
