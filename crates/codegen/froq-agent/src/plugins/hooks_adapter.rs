...
//! `froq-hooks` runtime.  It pre-filters unsupported events from plugin
...
//! `froq-hooks` crate's parser and runtime.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use froq_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        if let Some(cmd) = &spec.command {
            ...
            // `froq-hooks::runner::command` for env vars whose
            // values are already known at load time.
            let substituted = substitute_env_vars(&cmd_str, plugin_root, plugin_data);
            let expanded = froq_config::expand_env_vars_in_string(&substituted);
            if expanded != cmd_str {
...
mod tests {
    ...
    /// `froq-hooks::runner::command`, which can fail for hooks
