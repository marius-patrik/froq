...
pub enum HostedTool {
    ...
    /// This is froq-specific — not part of the OpenAI Responses API, so it's
...
// engine in `crates/common/froq-compaction` operate over grok-build's
...
impl froq_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> froq_compaction::CompactionRole {
        use froq_compaction::CompactionRole;
    ...
    fn attachment_refs(&self) -> Vec<froq_compaction::CompactionFileRef> {
...
impl froq_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/froq-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`froq-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/froq-sampler/src/stream/chat_completions.rs])
...
fn build_responses_tools(req: &ConversationRequest) -> Vec<rs::Tool> {
    ...
    for hosted in &req.hosted_tools {
        match hosted {
            ...
            // XSearch is froq-specific — not in async_openai's rs::Tool enum.
...
/// Return raw JSON tool definitions for froq-specific hosted tools that
...
/// ([crates/codegen/froq-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use froq_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
...
if image_url.url == "https://example.com/image.png"
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in froq-chat-state. Here the assistant
