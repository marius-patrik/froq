...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] froq_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `froq-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `froq_tools::ToolError`) happens
