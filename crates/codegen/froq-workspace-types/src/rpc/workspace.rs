...
/// over `froq_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `froq_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// One TODO list item (slim DTO over `froq_tools`'s `TodoState`). `status`
...
/// in `froq-workspace/src/hub_server.rs`.
