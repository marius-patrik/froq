...
//!
//! `froq_hooks` pulls in `git2`/`reqwest`/`froq-tools`, too heavy for
...
/// Wire mirror of `froq_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `froq_hooks::config::HookSpec`.
///
...
/// `froq-hooks`, so a server-side test (`froq-workspace`'s
...
/// Wire mirror of `froq_hooks::event::HookEventName`.
///
