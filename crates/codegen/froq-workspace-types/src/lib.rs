//! Wire types for the `froq-workspace` API.
//!
...
//! `froq-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `froq-workspace-types` (instead of `froq-workspace` or
/// `froq-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `froq_workspace::permission`
