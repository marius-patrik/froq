...
//!
//! This crate provides the common logic used by `froq-shell` and
//! `froq-pager` for handling announcements (banner notifications).
...
fn announcements_state_path() -> PathBuf {
    froq_tools::util::grok_home::grok_home().join("announcements.json")
}
