...
//!
//! Extracted from `froq-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `froq-chat-state::compaction_utils::strip_system_tags` which
...
/// `froq-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use froq_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(froq_sampling_types::synthesized_reasoning_item(text))
    }
