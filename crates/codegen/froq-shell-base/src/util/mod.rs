...
pub mod tips;
pub mod uname;
pub use froq_shared::clipboard;
pub use froq_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True for froq-operated endpoints (`*.x.ai`, cli-chat-proxy, and optional
...
/// *attach* a credential, use [`is_froq_api_bearer_url`].
pub fn is_froq_api_url(url: &str) -> bool {
    is_froq_api_url_impl(url, false)
}
/// Like [`is_froq_api_url`], but requires `https` on every arm, so a
...
pub fn is_froq_api_bearer_url(url: &str) -> bool {
    is_froq_api_url_impl(url, true)
}
fn is_froq_api_url_impl(url: &str, require_https: bool) -> bool {
    if require_https {
...
mod tests {
    ...
    #[test]
    fn test_is_froq_api_url() {
        assert!(is_froq_api_url("https://api.x.ai/v1"));
        assert!(is_froq_api_url("https://api.x.ai/v1/chat/completions"));
        assert!(is_froq_api_url("https://x.ai"));
        assert!(is_froq_api_url(
            "https://cli-chat-proxy.grok.com/v1/chat/completions"
        ));
        assert!(!is_froq_api_url("https://api.openai.com/v1"));
        assert!(!is_froq_api_url("https://api.anthropic.com/v1"));
        assert!(!is_froq_api_url("https://generativelanguage.googleapis.com"));
        assert!(!is_froq_api_url("https://api.x.ai.evil.example/v1"));
        assert!(!is_froq_api_url("https://evil-x.ai.attacker.com/v1"));
        assert!(!is_froq_api_url("https://prefixx.ai/v1"));
        assert!(!is_froq_api_url("not-a-url"));
        assert!(!is_froq_api_url(""));
        assert!(is_froq_api_url("http://api.x.ai/v1"));
        assert!(is_froq_api_url("http://localhost:11434/v1"));
    }
    #[test]
    fn test_is_froq_api_bearer_url() {
        assert!(is_froq_api_bearer_url("https://api.x.ai/v1"));
        assert!(!is_froq_api_bearer_url("http://api.x.ai/v1"));
        assert!(!is_froq_api_bearer_url("http://localhost:11434/v1"));
        {
            assert!(!is_froq_api_bearer_url("https://localhost:11434/v1"));
            assert!(!is_froq_api_bearer_url("https://127.0.0.2:11434/v1"));
            assert!(!is_froq_api_bearer_url("https://[::1]:11434/v1"));
        }
        assert!(is_froq_api_bearer_url("https://API.X.AI/v1"));
        assert!(!is_froq_api_bearer_url(
            "https://api.x.ai@attacker.example/v1"
        ));
        assert!(!is_froq_api_bearer_url("https://х.ai/v1"));
    }
    #[test]
    fn test_truncate() {
