//! `paste-latency` — clipboard paste latency benchmark for `froq-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p froq-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p froq-pager-pty-harness --bench paste_latency -- \
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for froq-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
