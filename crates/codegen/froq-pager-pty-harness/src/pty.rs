...
use froq_test_support::{TestProcessTree, TestSandbox};
#[cfg(unix)]
use froq_test_support::process_has_exited_without_reap;
...
/// (`froq-pager-render/src/terminal/mod.rs`:
...
/// [`froq_test_support::TestSandbox`], which always clears inheritance.
...
mod tests {
    ...
    fn apply_child_env_remove_deletes_sandbox_credential() {
        ...
        apply_child_env(&mut cmd, Some(&sandbox), &[EnvOp::remove("froq_API_KEY")]);

        assert_eq!(cmd.get_env("froq_API_KEY"), None);
        assert_eq!(
            cmd.get_env("GROK_froq_API_BASE_URL")
                .and_then(|v| v.to_str()),
