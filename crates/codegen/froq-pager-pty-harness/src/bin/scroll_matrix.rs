//! `scroll-matrix` — scroll validation matrix sweep for froq-pager.
//!
...
use froq_pager_pty_harness::pager_binary;
use froq_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against froq-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_froq-pager,
