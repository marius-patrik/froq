...
//! [`MockInferenceServer`] from `froq-test-support` and provides the
...
pub use froq_test_support::mock_server::LogEntry;
pub use froq_test_support::mock_server::MockModelEntry as MockModel;
pub use froq_test_support::mock_server::StorageUpload;
pub use froq_test_support::sse;
pub use froq_test_support::{
...
impl ContentController {
    ...
    ///
    /// Mirrors `froq_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ...
            ("GROK_froq_API_BASE_URL".into(), self.url()),
            ("froq_API_KEY".into(), "test-key-for-ci".into()),
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(get("GROK_froq_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("froq_API_KEY").as_deref(), Some("test-key-for-ci"));
