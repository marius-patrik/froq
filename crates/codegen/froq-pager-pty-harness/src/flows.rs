...
/// shell has session auth (the harness's `froq_API_KEY` is ApiKey/BYOK mode
...
/// [`ContentController::env_for_pager`] minus `froq_API_KEY`, so the entry
