...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("froq-pager{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    cmd.current_dir(workspace_root()?)
        ...
        .stdin(Stdio::null())
        .envs(froq_tty_utils::pager_env());
    froq_tty_utils::detach_std_command(&mut cmd);
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build froq-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build froq-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    if !binary.exists() {
        bail!(
            "froq-pager build completed but binary missing at {}",
            binary.display()
        );
    }
    Ok(())
...
/// 2. `CARGO_BIN_EXE_froq-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p froq-pager-bin` (the composition-
///    root package that owns the `froq-pager` binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_froq-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
