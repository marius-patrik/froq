//! Unified PTY harness for froq-pager.
//!
...
//!    `froq-pager/tests/scenarios/`) — assert screen contents and
...
impl PtyHarness {
    ...
    /// Spawn from a canonical [`froq_test_support::TestSandbox`] baseline
    ...
    pub fn new_in_sandbox(
        ...
        cols: u16,
        args: &[&str],
        sandbox: &froq_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use froq_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
