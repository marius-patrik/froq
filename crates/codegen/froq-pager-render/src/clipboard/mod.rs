...
//! `froq-ratatui-textarea`, and adds [`SystemClipboard`] backed by `arboard`.
//!
...
pub use froq_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
/// Documented in `froq-pager/docs/internal/22-environment-variables.md`.
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(froq_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(froq_shared::clipboard::is_containerized_without_display)
}
...
/// clipboard (see `froq-pager`'s `pty_wrap` module). It advertises this to
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if froq_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = froq_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        let mut cmd = Command::new("tmux");
        cmd.args(["load-buffer", "-"])
            .stdin(Stdio::from(stdin))
            .stdout(Stdio::null())
            .stderr(Stdio::null());
        froq_tty_utils::detach_std_command(&mut cmd);
        let mut child = cmd.spawn()?;
        // Bounded wait: a wedged tmux server must not freeze the UI thread.
        let status = froq_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`froq_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        froq_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = froq_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match froq_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    ...
    froq_config::user_grok_home().map(|grok_home| grok_home.join("last-copy.txt"))
}
...
fn log_clipboard_copy_event(
    ...
    if !froq_telemetry::client::is_enabled() {
        return;
    }
    froq_telemetry::session_ctx::log_event(froq_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    froq_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !froq_shared::clipboard::x11_display_env_present() {
        return None;
    }
    froq_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && froq_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    froq_telemetry::session_ctx::log_event(
        froq_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    froq_telemetry::session_ctx::log_event(froq_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match froq_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use froq_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    froq_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    froq_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    froq_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(froq_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match froq_shared::clipboard::get_image() {
        Ok(img) => {
...
mod tests {
    ...
    fn default_copy_fallback_path_is_grok_home() {
        ...
        let expected = froq_config::user_grok_home()
    ...
    /// the `GROK_HOME`-override integration test in `froq-pager`).
    #[test]
