...
use froq_ratatui_textarea::ElementId;
...
pub fn decode_image_dimensions(bytes: &[u8]) -> Option<(u32, u32)> {
    ...
    froq_tools::util::image_validate::validate_image_bytes_unrestricted(bytes, false)
        .ok()
        .map(|(w, h, _)| (w, h))
}
...
impl ImageViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        let mime_type = froq_shared::clipboard::mime_from_bytes(&bytes).to_owned();
...
impl VideoViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        froq_tty_utils::detach_std_command(&mut ffmpeg_cmd);
...
use froq_shared::clipboard::mime_to_extension;
...
///
/// Delegates to [`froq_shared::clipboard::mime_to_extension`].
...
fn read_image_at_path(path: &std::path::Path) -> Option<PastedImage> {
    ...
    let mime_type = froq_shared::clipboard::mime_from_bytes(&data);
...
pub fn session_images_dir(
    ...
    let sid = session_id?;
    let info = froq_shared::session::info::Info {
        id: sid.clone(),
        cwd: cwd.to_string_lossy().into_owned(),
    };
    Some(froq_shared::session::session_dir(&info).join("images"))
}
...
pub fn session_mermaid_dir(
    ...
    Some(froq_shared::session::session_dir(&info).join("mermaid"))
}
...
///   [`froq_shell::session::placeholder_images::load_placeholder_image`]
...
pub fn build_content_blocks_with_workspace(
    ...
    let allowed: Option<Vec<std::path::PathBuf>> =
        workspace_cwd.map(froq_shared::placeholder_images::default_allowed_prefixes);
...
pub fn build_content_blocks_with_prefixes(
    ...
    build_content_blocks_with_prefixes_and_caps(
        ...
        images,
        allowed_prefixes,
        froq_shared::placeholder_images::MAX_PLACEHOLDER_AGGREGATE_BYTES,
    )
}
...
/// [`froq_shell::session::placeholder_images::recover_orphan_placeholders_with_prefixes_and_caps`].
...
pub fn build_content_blocks_with_prefixes_and_caps(
    ...
    // attachment.
    let rewritten_text =
        froq_shared::placeholder_images::strip_paths_from_image_placeholders(rewritten_text);
    ...
    for img in &images {
        ...
        blocks.push(ContentBlock::Image(
            ImageContent::new(data, mime_type)
                ...
                .meta(Some(
                    froq_shared::placeholder_images::display_number_meta(img.display_number),
                )),
        ));
    }
...
fn resolve_orphan_placeholders(
    ...
    let placeholders = froq_shared::placeholder_images::extract_placeholders(&text);
    ...
    for ph in &placeholders {
        ...
        match froq_shared::placeholder_images::load_placeholder_image(&ph.path, allowed) {
            Ok(loaded) => {
                ...
                recovered.push(
                    ImageContent::new(data, loaded.mime_type)
                        ...
                        .meta(Some(
                            froq_shared::placeholder_images::display_number_meta(
                                ph.display_number,
                            ),
                        )),
                );
...
mod tests {
    ...
    fn build_blocks_sets_display_number_meta() {
        ...
        assert_eq!(
            froq_shared::placeholder_images::display_number_from_meta(ic.meta.as_ref()),
            Some(3),
