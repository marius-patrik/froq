...
//! [`froq_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use froq_ratatui_inline::LinkSpan;
...
pub type PagerTerminal = froq_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
#[derive(Debug)]
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || -> std::io::Result<()> {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = froq_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(payload) = rx.recv() {
                ...
                let result = {
                    let _guard = froq_shared::stderr::stderr_lock();
