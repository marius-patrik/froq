...
//! frame diff (see `froq_ratatui_inline::Terminal::flush_with_links`).
...
mod tests {
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/froq-pager/src/render` should
        // NOT produce a link for the `/froq-pager/src/render` substring.
        let line = make_line("find crates/codegen/froq-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
