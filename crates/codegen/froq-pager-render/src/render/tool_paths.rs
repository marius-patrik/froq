...
fn resolve_tool_path_with_home(
    ...
    let display_path = target
        .as_deref()
        .map(froq_paths::normalize_lexically)
        .unwrap_or_else(|| PathBuf::from(path));
    let relative_to_cwd = target.as_ref().and_then(|_| {
        let cwd = froq_paths::normalize_lexically(cwd?);
...
mod tests {
    ...
    fn shorten_path_fish_style() {
        let result = shorten_path("crates/codegen/froq-pager/src/views/foo.rs", 25);
    ...
    fn shorten_path_front_truncate() {
        let result = shorten_path(
            "crates/codegen/froq-pager/src/views/very_long_filename.rs",
            20,
        );
    ...
    fn home_relative_target_preserves_filesystem_spelling_for_io() {
        ...
        assert_eq!(
            ...
            froq_paths::normalize_lexically(&home.join("notes.md"))
        );
    }
