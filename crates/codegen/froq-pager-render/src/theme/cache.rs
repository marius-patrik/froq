...
//! `froq_shell::util::config::set_theme()` (and friends), invoked
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    froq_markdown::set_color_level_cap(if locked {
        froq_markdown::ColorLevel::Basic
    } else {
        froq_markdown::ColorLevel::TrueColor
    });
    froq_markdown::set_polarity_safe_syntax(locked);
}
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `froq_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = froq_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = froq_config::load_effective_config_disk_only() else {
...
mod tests {
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!froq_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                froq_markdown::polarity_safe_syntax(),
