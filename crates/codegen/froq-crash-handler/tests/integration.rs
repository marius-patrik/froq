//! Integration tests for froq-crash-handler.
//!
...
fn subprocess_entry() {
    ...
    let config = froq_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    froq_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob = froq_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report =
        froq_crash_handler::check_previous_crash(tmp.path()).expect("should produce a crash report");
...
fn clean_exit_does_not_produce_crash_report() {
    ...
    let report = froq_crash_handler::check_previous_crash(tmp.path());
