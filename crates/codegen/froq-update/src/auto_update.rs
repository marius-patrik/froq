...
use froq_shell::util::grok_home::{grok_application, grok_home};
...
fn reinstall_hint(installer: &str) -> String {
    match installer {
        ...
        "gh-release" => "Please reinstall via GitHub Releases:\n  gh release download --repo marius-patrik/froq --pattern 'froq-*' --output froq && chmod +x froq".to_string(),
...
/// Download and install froq from GitHub Releases (marius-patrik/froq).
///
...
fn warn_if_other_grok_processes_running() {
    ...
    cmd.args(["-f", "grok"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    froq_tools::util::detach_std_command(&mut cmd);
...
fn install_npm(target: Option<&str>, channel: &str, npm_registry: Option<&str>) -> Result<()> {
    ...
    let version_arg = match target {
        Some(ver) => format!("@froq-official/grok@{ver}"),
        None => {
...
async fn refresh_deployment_config() {
    if !froq_shell::managed_config::has_principal() {
        return;
    }
    if !froq_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    froq_shell::managed_config::clear_orphan();
    if !froq_shell::config::is_managed_config_stale_for(
        &froq_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match froq_shell::managed_config::sync().await {
...
mod tests {
    ...
    fn test_reinstall_hint_npm_mentions_npm_command() {
        ...
        assert!(
            hint.contains("@froq-official/grok"),
    ...
    fn test_reinstall_hint_gh_release_mentions_gh_command() {
        ...
        assert!(
            hint.contains("marius-patrik/froq"),
