...
use froq_shell::env::GrokBuildEnvironment;
use froq_shell::util::grok_home::grok_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@froq-official/grok";
pub const GH_RELEASE_REPO: &str = "marius-patrik/froq";
...
impl UpdateConfig {
    pub fn from_environment(env: &GrokBuildEnvironment) -> Self {
        Self {
            ...
            auth_scope: froq_shell::auth::GrokComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
pub use froq_version::installed as get_installed_grok_version;
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = froq_shell::util::grok_home::grok_application();
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(froq_version::VERSION, &stable)
    })
}
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(froq_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
