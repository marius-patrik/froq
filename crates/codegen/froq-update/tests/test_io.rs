...
//! `froq-config`), `GROK_TEST_VERSION`, and `NPM_TOKEN` — so they
...
use froq_update::write_version_cache;
...
async fn get_installed_version_uses_env_var_override() {
    ...
    let v = froq_update::version::get_installed_grok_version();
    assert_eq!(v, "9.9.9");
    unsafe {
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        froq_update::version::get_installed_grok_version()
    };

    unsafe {
        std::env::set_var("GROK_TEST_VERSION", "0.0.0-test");
    }
    let overridden = froq_update::version::get_installed_grok_version();
