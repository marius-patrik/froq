...
//! `froq-shell` and `froq-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `froq-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `froq-agent`. Optional on [`PluginInfo`]
...
/// Hook event type.
///
/// Maps from `HookEventName` in `froq-hooks`. The source type's
