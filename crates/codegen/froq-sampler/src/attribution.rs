...
//! it" buckets.
//!
//! `froq-sampler` is intentionally decoupled from `froq-shell`
...
/// tools live in `froq-tools`
/// (`crates/codegen/froq-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `froq_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `froq_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `froq-shell/src/auth/manager.rs` to match, otherwise the
