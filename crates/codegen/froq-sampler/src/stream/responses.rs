...
pub(crate) fn stream_responses_tracked<'a>(
    ...
    async_stream::stream! {
        ...
        let mut items = froq_sampling_types::response_to_conversation_items(response);
        froq_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use froq_sampling_types::doom_loop::{
