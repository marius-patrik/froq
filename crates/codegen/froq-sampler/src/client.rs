...
use froq_sampling_types::error::{try_parse_stream_error, user_facing_api_error_message};
...
pub use froq_sampling_types::ApiBackend;
...
/// Deserialize a Responses API SSE event, with a fallback for froq-specific
...
/// when the API emits the froq-specific `context_details` field.
    ...
    let mut event = match serde_json::from_str::<rs::ResponseStreamEvent>(data) {
        ...
        Err(first_err) => {
            ...
            if let Ok(mut value) = serde_json::from_str::<serde_json::Value>(data) {
                ...
                // (e.g., froq-specific "x_search"). Instead of maintaining a
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = froq_sampling_types::reported_cost_ticks(
        value
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<froq_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub async fn create_response(
        ...
        froq_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = grok_headers
    ...
    pub async fn create_response_stream(
        ...
        // Inject froq-specific fields not in async-openai's CreateResponse type.
        if self.defaults.stream_tool_calls {
            request_body["stream_tool_calls"] = serde_json::json!(true);
        }
        // Inject froq-specific tools (e.g., x_search) that can't be expressed
    ...
    pub async fn conversation_stream_responses(
        ...
        // Collect froq-specific tools that can't be expressed via rs::Tool
        // (e.g., x_search). These are injected as raw JSON after serialization.
        let extra_tools = froq_sampling_types::extra_tool_entries(&request.hosted_tools);
...
mod tests {
    ...
    use froq_sampling_types::types::ChatRequestMessage;
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-froq-token-auth".to_string(), "froq-cli".to_string());
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = froq_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
