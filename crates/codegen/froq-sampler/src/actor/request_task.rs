...
async fn run_one_attempt(
    ...
    event_tx: &mpsc::UnboundedSender<SamplingEvent>,
    cancel_token: &CancellationToken,
    doom_check: Option<froq_sampling_types::DoomLoopRecoveryPolicy>,
        ...
        tokio::select! {
            ...
            next = l2.next() => match next {
                Some(SamplingEvent::Completed { response, metrics, .. }) => {
                    ...
                    if response.stop_reason == Some(froq_sampling_types::StopReason::Length) {
                        return AttemptOutcome::Failed {
                    ...
                    let content_filtered = response.stop_reason
                        == Some(froq_sampling_types::StopReason::ContentFilter);
...
fn build_empty_context(
    reason: froq_sampling_types::EmptyReason,
