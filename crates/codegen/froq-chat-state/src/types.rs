...
/// emitter in `froq-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    ...
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, froq_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `froq_sampling_types::SamplingConfig` (which is secret-free).
