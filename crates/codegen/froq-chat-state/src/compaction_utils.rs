...
//! no I/O, no actor state. They live in `froq-chat-state` so that both
//! this crate and `froq-shell` can share them without duplication.
use std::collections::BTreeSet;
use froq_sampling_types::{ContentPart, ConversationItem, ToolResultItem};
...
///
/// Stored here (rather than only in `froq-shell`) so that query-extraction
...
/// `ActiveSubagentSummary` in froq-tools. The mapping between them
/// happens in `run_compact_inner()` (froq-shell).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (froq-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in froq-tools.
...
/// handled by the consumer (e.g. `froq-shell`), which has access to
...
/// `froq-chat-state` and `froq-shell` share the same format.
...
/// `run_compact_inner` in `froq-shell` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = froq_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = froq_sampling_types::repair_dangling_tool_calls(
        items,
        froq_sampling_types::DanglingToolCallReason::HarnessHalted {
...
mod tests {
    use super::*;
    use froq_sampling_types::SyntheticReason;
    #[test]
...
actual user question";
    ...
    fn call(id: &str) -> froq_sampling_types::ToolCall {
        froq_sampling_types::ToolCall {
            id: id.into(),
            name: "read_file".to_string(),
            arguments: "{}".into(),
        }
    ...
    ///
    /// Simulates the exact conversation shape produced by froq-shell:
    ///
    ...
    async fn build_compacted_history_multi_turn_with_parallel_tool_calls() {
        use froq_sampling_types::{AssistantItem, ToolCall};
        let conversation = vec![
            // [0] System prompt
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use froq_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use froq_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use froq_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
    ...
    fn fit_counts_user_images_against_budget() {
        use froq_sampling_types::ContentPart;
