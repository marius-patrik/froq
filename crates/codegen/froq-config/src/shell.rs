...
///
/// The probe is spawned via `froq_tty_utils::detach_std_command` so that
...
fn is_executable(path: &std::path::Path) -> bool {
    ...
    // Nix fallback. Detach from the controlling TTY via froq_tty_utils so
...
mod tests {
    ...
    fn is_command_available_detects_present_and_absent() {
        ...
        assert!(!is_command_available(
            "froq-definitely-not-a-real-command-xyz"
        ));
    }
