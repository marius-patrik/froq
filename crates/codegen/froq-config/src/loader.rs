...
/// (`froq_shell::util::config::load_effective_config`) when remote campaigns
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match froq_version::installed_semver() {
...
mod tests {
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        ...
        std::fs::write(
            &path,
            "api_key = \"froq-secretmustnotleak\"\napi_key = \"froq-secretmustnotleak2\"\n",
        )
        .unwrap();
        ...
        assert!(
            !msg.contains("froq-secretmustnotleak"),
