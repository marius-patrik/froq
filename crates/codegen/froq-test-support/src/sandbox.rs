...
impl TestSandbox {
    ...
    pub fn git_command(&self) -> Command {
        ...
        cmd.stdin(Stdio::null()).envs(froq_tty_utils::pager_env());
        for &(key, value) in &froq_tty_utils::GIT_AUTH_SUPPRESSION_ENVS {
            cmd.env(key, value);
        }
...
fn apply_mock_url(env: &mut BTreeMap<OsString, OsString>, url: String) {
    ...
    env.insert("froq_API_KEY".into(), TEST_API_KEY.into());
}
...
mod tests {
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "froq_API_KEY").as_deref(),
