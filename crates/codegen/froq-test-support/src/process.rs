...
pub struct TestProcessTree {
    pid: u32,
    label: String,
    group: Option<froq_tty_utils::ProcessGroup>,
...
impl TestProcessTree {
    ...
    pub fn try_attach(pid: u32, label: impl Into<String>) -> io::Result<Self> {
        ...
        let mut group = froq_tty_utils::ProcessGroup::new()?;
    ...
    pub fn attach(pid: u32, label: impl Into<String>) -> Self {
        let label = label.into();
        let (group, attachment_error) = match froq_tty_utils::ProcessGroup::new() {
    ...
    fn from_group(pid: u32, label: String, group: froq_tty_utils::ProcessGroup) -> Self {
        Self {
            pid,
            label,
            group: Some(group),
...
impl TestProcess {
    ...
    pub fn spawn(
        ...
        cmd.envs(froq_tty_utils::pager_env())
            .envs(config.env)
...
mod tests {
    ...
    fn non_reaping_exit_observation_preserves_wait_status() {
        ...
        froq_tty_utils::detach_std_command(&mut command);
