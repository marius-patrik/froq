...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build froq-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build froq-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
...
/// Resolve grok binary: `GROK_BINARY` env (CI) or a locally built `froq-pager` binary.
