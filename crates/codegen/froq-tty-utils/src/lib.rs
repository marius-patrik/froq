...
//!
//! ```rust,no_run
//! use froq_tty_utils::{detach_command, pager_env};
...
//!
//! ```rust,no_run
//! use froq_tty_utils::{detach_std_command, pager_env};
//! use std::process::Stdio;
//!
...
mod tests {
    ...
    fn stderr_redirect_roundtrip_subprocess() {
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("__froq_STDERR_REDIRECT_SUBPROCESS", "1")
            .status()
    ...
    fn stderr_redirect_roundtrip_body() {
        if std::env::var("__froq_STDERR_REDIRECT_SUBPROCESS").is_err() {
