...
pub struct McpState {
    ...
    event_writer: froq_file_utils::events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: froq_file_utils::events::EventWriter::noop(),
    ...
    pub fn set_event_writer(&mut self, writer: froq_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &froq_file_utils::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `froq-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use froq_telemetry::enums::McpInitStrategy;

/// Parse a non-empty `server__tool` ID with one overlap-aware delimiter and
/// valid [`froq_tool_protocol::ToolId`] syntax.
pub fn parse_mcp_qualified_name(name: &str) -> Option<(froq_tool_protocol::ToolId, &str, &str)> {
    ...
    Some((froq_tool_protocol::ToolId::new(name).ok()?, server, tool))
}
...
impl McpError {
    ...
    pub fn error_category(&self) -> froq_file_utils::events::McpErrorCategory {
        use froq_file_utils::events::McpErrorCategory;
        match self {
...
/// `froq_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl froq_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> froq_tool_protocol::ToolId {
        ...
        froq_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| froq_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        froq_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(froq_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(froq_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            froq_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(froq_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
...
if mime_type
        ...
        event_writer.emit(froq_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        froq_telemetry::session_ctx::log_event(froq_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &froq_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, froq_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| froq_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(froq_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(froq_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(froq_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(froq_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(froq_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(froq_file_utils::events::Event::McpTransportDecodeError {
...
/// External-config overrides for an MCP server, surfaced to froq-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: froq_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            ...
            Implementation::new(
                ...
                froq_version::VERSION.to_string(),
            ),
        )
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            froq_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = froq_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = froq_config::grok_home().join("logs").join("mcp");
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = froq_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = froq_telemetry::instrumentation::timer("mcp_stdio_spawn");
            ...
            froq_tools::util::detach_command(&mut cmd);
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        froq_telemetry::session_ctx::log_event(
                            froq_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: froq_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    froq_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            froq_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = froq_telemetry::instrumentation::timer("mcp_start_servers");
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            froq_file_utils::events::EventWriter::noop(),
        );
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use froq_computer_hub_sdk::LocalRegistry;
        use froq_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = froq_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = froq_file_utils::events::EventWriter::open(tmp.path());
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            froq_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
