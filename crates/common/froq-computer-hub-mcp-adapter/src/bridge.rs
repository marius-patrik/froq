...
use froq_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use froq_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`froq_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`froq_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl froq_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            froq_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == froq_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
