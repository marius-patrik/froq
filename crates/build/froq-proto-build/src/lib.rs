...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        let dep_path = out_dir.join("froq-proto-build-deps.d");
        let desc_path = out_dir.join("froq-proto-build-descriptor.bin");
    ...
    pub fn compile_protos(
        ...
        let file_descriptor_set_path: Option<PathBuf> =
            if let Some(file_descriptor_set_path) = file_descriptor_set_path {
                ...
                let file_descriptor_set_path = tempfile.path().join("froq-proto-build.pbbin");
